...
```bash
sqlplus '用户名/密码@//主机地址:端口/ServiceName'
```

数据库连接规则：
- Claude Code 可以使用以上连接信息访问开发数据库。
- 默认只允许执行只读查询。
- 所有写操作必须执行人工审批流程。
- 不得自行切换到其他数据库、用户或 Schema。
- 数据库连接失败时必须停止并报告，不得自行修改连接参数。
- 任务另有表白名单时，只需访问白名单中的业务表，非白名单中的业务表，无需关注。
- 可以任何必要的，有权限的 Oracle 数据字典视图。