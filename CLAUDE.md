...
当任务涉及查询列表页的**新建、结构性调整、公共交互修改，或既有页面迁移评估**时，必须先完整阅读：
...
- 目录登记与模板接受**不等同于**页面迁移授权：迁移既有页面前必须确认项目负责人是否已**明确授权**；未授权时只能评估，**不得**实施迁移，也**不得**更新任何页面的迁移状态；
- 新建查询列表页或对查询列表页做结构性调整时，应先评估并**优先复用**该模板与公共组件；若业务差异确实不适用，必须记录差异与理由，**不得**静默复制一套平行实现；
- 若该模板与某个 Feature 的已批准基线冲突，必须停止并报告，**不得**由模板静默覆盖功能基线。
