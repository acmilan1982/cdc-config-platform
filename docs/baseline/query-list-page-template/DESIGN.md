...
后续的**公共组件详细设计** `SHARED_COMPONENT_DESIGN.md`（状态
`APPROVED`，**已批准、尚未实现**）在此基础上逐个候选给出**唯一结论**
（保留实现 / 合并 / 延后 / 否决）、最终建议名称与目录、完整
Props / Slots / Emits 契约、样式令牌、稳定滚动条槽的唯一方案与阶段一范围。
其 R2 结果提交为 `6f3c3517821d4b7e43300180033c5b5ef7c628f9`，
批准收口任务为 `QUERY-LIST-PAGE-SHARED-COMPONENT-DESIGN-APPROVAL-001`。

两者关系必须按以下口径理解，不得混用：

- 本节的候选条目是**基线的候选池**，不是最终组件清单；
- 最终建议名称、职责边界与取舍理由以该详细设计为准；该详细设计**已批准**，
  但**尚未实现**，因此在**独立授权的实现任务**启动之前**不得**据以开始实现；
- 该详细设计**未**修改本节任何已批准内容，也**未**修改本文件集其余三份文档的规范内容；
