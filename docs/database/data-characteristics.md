...
**状态**：已确认，程序层已兼容。项目负责人确认"最好都是大写"。

### 2. CDC_DATA_SOURCE_EXTEND 测试数据

mock7、mock8、mock9 为测试数据。项目负责人确认"不用理会"。

### 3. TABLE_NAME_SUFFIX 占位值

`_fucking` 出现在5条记录中。项目负责人确认"为测试数据内容，你无需理会"。

### 4. CDC_LOG_CORRECT 空表

**状态**：已确认。该表仍在使用，保存成功同步的数据。因表空间不够且日志量大，之前被清空。

### 5. CDC_LOG_ERROR 仅1条记录

**状态**：已确认。同样因表空间不足被清空，剩余的1条为清理后新产生的错误记录。

### 6. CDC_DATA_SOURCE.DATA_SOURCE_DOMAIN 全部为空

**状态**：已确认。项目负责人确认"暂时不用理会"。
