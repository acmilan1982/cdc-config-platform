...
> 新权威文档：`docs/database/DATA_PROFILE.md`（数据现状与规模画像）、`docs/database/CODE_VALUES.md`（公共码值）。
> 本快照为 2026-07-03 记录数/分布（开发库当时瞬时状态），其中记录数、分布及“因表空间不足清空”等描述仅代表当时快照；当前行数与码值分布以 `DATA_PROFILE.md` / `CODE_VALUES.md` 为准。
