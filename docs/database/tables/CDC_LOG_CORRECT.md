...
| 项 | 值 |
|---|---|
| 表用途 | 同步正确日志表：记录 CDC 源库到目标库同步成功的每一条数据日志（含偏移量、时间戳、落盘详情等） |
| 表类型 | 普通堆表（NON-PARTITIONED） |
| 主键 | `PK_CDC_LOG_CORRECT`（`CDC_LOG_ID`） |
| 外键 | 无 |
| 分区 | 无 |
...
| 名称 | 唯一性 | 类型 | 字段（顺序） | 状态 |
|---|---|---|---|---|
| PK_CDC_LOG_CORRECT | UNIQUE | NORMAL | CDC_LOG_ID (1) | VALID |
...
| 字段名 | LOB段 | 表空间 | CHUNK |
|---|---|---|---|
| RAW_MESSAGE | SYS_LOB0000106857C00016$$ | USERS | 8192 |
...
| 日志查询 | `logquery/mapper/LogQueryMapper.xml` | selectLogList / selectLogDetail / selectRawMessage（`${tableName}` 由 LogTypeEnum 封闭枚举限定为 CDC_LOG_CORRECT） | 只读 |
| 日志查询 | `logquery/enums/LogTypeEnum.java` | CORRECT → "CDC_LOG_CORRECT" 白名单映射 | 只读 |
...
- 大表成本原则：本表为大规模日志表，行数按统计信息评估（`OBSERVED_ESTIMATED`，约 3,819,479 行，LAST_ANALYZED 2026-08-12），见 `DATA_PROFILE.md`；未做大表全表 COUNT 精确统计。
- 既有旧快照 `docs/database/table-detail.md`（2026-07-03）记录 RESULT_CODE/OFFSET 等字段类型需核对；本次核验确认 RESULT_CODE、OFFSET 为 `NUMBER(10)`（精度/小数见 §2）。
