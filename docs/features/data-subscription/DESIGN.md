...
| 文档状态 | `APPROVED`（正式验收前 UI 交互基线对齐内容已经获得 ChatGPT 对结果提交 `26094c6b6d8f9b8d5971ef38648851611799adee` 的正式复审 `APPROVED`，批准收口完成，已纳入当前批准设计基线；上一正式批准设计版本为 R4（R4-R1 完成状态元数据收口），ChatGPT 对 R4-R1 结果提交 `ba7feddff426e369e1e73791b8d75e2ab62934e9` 正式复审结论 `APPROVED`，该批准历史保留；本次 UI 交互基线对齐在已实现并经 ChatGPT 正式复审通过的前端最终交互事实基础上，把弹窗尺寸、空间优先级、目标库卡片白色主体四态与 Shift 连选同步为正式批准设计基线，不新增后端接口、不修改 API 契约；“批准不代表实现或验收通过”表述设计批准当时的历史含义；其后本 Feature 已完成实现并正式验收，当前实现状态 `IMPLEMENTED_ACCEPTED`、正式验收状态 `ACCEPTED`、验收执行状态 `PASS`，见“实现状态”/“正式验收状态”行） |
| 设计正式复审状态 | `APPROVED`（R1 正式复审结论 `CHANGES_REQUIRED`；R2 定向修订已完成且四项修订目标通过正式复核；R3 已按已批准“取消并发保护”需求完成定向修订，ChatGPT 对 R3 结果提交 `ac4954401b79e04c56a8bbf9daec871fd194f19c` 正式复审结论 `CHANGES_REQUIRED`；本 R4 按正式复审发现定向修正三个确定问题（DELETE 影响多行错误码统一为 `50041`；DELETE 接口删除前多源库异常后端防护统一为强制普通读取→存在性与异常判定→普通 DELETE；修正“NULL 被 split 成 `['']`”的 Java 语义错误为“对 null 调用 split 抛 `NullPointerException`”）；R4/R4-R1 为上一正式批准设计版本和历史批准链基础（ChatGPT 对 R4-R1 结果提交 `ba7feddff426e369e1e73791b8d75e2ab62934e9` 正式复审结论 `APPROVED`，该批准历史保留）；当前正式批准设计基线已纳入正式验收前 UI 交互对齐内容，即 ChatGPT 对提交 `26094c6b6d8f9b8d5971ef38648851611799adee` 正式复审通过的 UI 交互对齐内容；设计正式复审状态仍为 `APPROVED`；“批准不代表实现或验收通过”表述设计批准当时的历史含义；其后本 Feature 已完成实现并正式验收，当前实现状态 `IMPLEMENTED_ACCEPTED`、正式验收状态 `ACCEPTED`） |
...
| 依据的已批准需求基线 | `docs/features/data-subscription/REQUIREMENTS.md`（当前文档状态 `APPROVED`，正式验收前 UI 交互基线对齐内容已经获得 ChatGPT 对结果提交 `26094c6b6d8f9b8d5971ef38648851611799adee` 的正式复审 `APPROVED`，已作为当前批准设计基线依据；107 条 `DSUB-REQ-001` ~ `DSUB-REQ-107`；上一正式批准需求版本为“取消并发保护”需求调整版本，正式复审依据提交 `43a909773aec63fe8c4de2957074f113910f4686`，该批准历史保留；前序含逗号数据源 ID 查询兼容批准版本批准依据提交 `afc5765956cac3c8f66d8857ff17565472d0c746`、英文句点保留分隔符批准版本批准依据提交 `bb8716c26d5181edf84ba1f07d4e60e8f1c1918a` 作为历史事实保留；旧 Git 基线提交 `8331fbb6e17b8e2165b788d972f651aa980bf227` 仅作为历史记录保留，不作为当前依赖基线） |
| 依据的已批准验收基线 | `docs/features/data-subscription/ACCEPTANCE.md`（当前文档状态 `APPROVED`，正式验收前 UI 交互基线对齐内容已经获得 ChatGPT 对结果提交 `26094c6b6d8f9b8d5971ef38648851611799adee` 的正式复审 `APPROVED`，已作为当前批准设计基线依据；126 条 `DSUB-AC-001` ~ `DSUB-AC-126`（设计基线批准当时全部 `NOT_RUN`，为当时历史状态；其后正式验收已执行并经 ChatGPT 正式复审批准，当前 126 条全部 `PASS`、验收执行状态 `PASS`、正式验收状态 `ACCEPTED`）；上一正式批准验收标准版本为“取消并发保护”验收标准调整版本，该批准历史保留；旧 Git 基线提交 `8331fbb6e17b8e2165b788d972f651aa980bf227` 仅作为历史记录保留，不作为当前依赖基线） |
| 创建日期 | 2026-08-30 |
| R1 修订日期 | 2026-08-30 |
| R2 修订日期 | 2026-08-31 |
| R3 修订日期 | 2026-08-31 |
...
> 下表保证 `DSUB-REQ-001` ~ `DSUB-REQ-107` 每条至少映射到一个 R3 修订后真实存在的设计章节（DESIGN=本文，API=API.md，UI=UI.md，DATABASE=DATABASE.md）。映射为“设计覆盖”，表示该需求已在本设计草案中得到实现层决策；是否验收通过由 126 条验收用例另行判定（设计基线形成时 126 条验收用例仍全部 `NOT_RUN`，为当时历史状态；其后正式验收已执行并经 ChatGPT 正式复审批准，当前 126 条全部 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`）、正式验收状态 `ACCEPTED`）。所有引用章节均为 R3 修订后的真实标题编号，无未来不存在的章节号。
...
- 映射为“设计覆盖”不代表验收执行结果；设计基线形成时 126 条验收用例全部 `NOT_RUN`（为当时历史状态，不代表当前），其后正式验收已执行并经 ChatGPT 正式复审批准，当前 126 条全部 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`）、正式验收状态 `ACCEPTED`，验收是否通过由验收执行与正式复审确认。

---

*文档状态：`APPROVED`。本文件为数据订阅 Feature 正式设计基线（上一批准版本 R4，R4-R1 完成状态元数据收口，批准依据提交 `ba7feddff426e369e1e73791b8d75e2ab62934e9`，该批准历史保留）；正式验收前 UI 交互基线对齐草案（`DATA-SUBSCRIPTION-PRE-ACCEPTANCE-UI-BASELINE-ALIGNMENT-001`）及 R1 状态文字修订已获得 ChatGPT 对结果提交 `26094c6b6d8f9b8d5971ef38648851611799adee` 正式复审 `APPROVED`，§7.7 内容（最终弹窗尺寸、空间优先级、目标库卡片白色主体四态与 Shift 连选）已纳入当前批准设计基线，设计正式复审状态保持 `APPROVED`；当时实现状态为 `IMPLEMENTED_REVIEW_APPROVED_PENDING_FORMAL_ACCEPTANCE`（该批准收口阶段的历史状态，不代表当前），批准不代表当时实现或正式验收通过，126 条验收在该阶段仍全部 `NOT_RUN`（当时历史状态）。其后正式验收 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001` 及 R1、R1-R1 补验已执行并经 ChatGPT 正式复审：126 条验收用例全部 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`），ChatGPT 对最终结果提交 `ae66d90e4415ce51be54f8be2523bb44b55b78a2` 的正式复审结论为 `APPROVED`，任务 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-CLOSEOUT-001` 完成最终接受收口，当前实现状态 `IMPLEMENTED_ACCEPTED`、正式验收状态 `ACCEPTED`、验收执行状态 `PASS`，本 Feature 不再有待验收复审入口。R3 已按已批准“取消并发保护”需求完成定向修订，R4 已按 ChatGPT 对 R3 正式复审 `CHANGES_REQUIRED` 定向修正 DELETE 错误码、DELETE 删除前多源库异常后端防护与 Java null/split 语义三个确定问题。ChatGPT 对批准收口结果提交 `dc97ffd...` 正式复核结论为 `CHANGES_REQUIRED`，本 R1（`DATA-SUBSCRIPTION-PRE-ACCEPTANCE-UI-BASELINE-ALIGNMENT-APPROVAL-001-R1`）已定向修正 DESIGN/UI 依赖基线状态、当前批准版本说明与四份文档实现状态“草案”文字，业务语义零变化，设计正式复审状态仍为 `APPROVED`。*
