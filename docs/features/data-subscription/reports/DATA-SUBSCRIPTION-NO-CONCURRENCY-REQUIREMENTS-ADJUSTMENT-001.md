...
> **R1 定向修订说明**：本报告后经 ChatGPT 对提交 `27a27e3de22cf2ea03c378bf6d39f58549c0c6fa` 正式复审（结论 `CHANGES_REQUIRED`）进行 R1 定向修订：修正 `DSUB-AC-048` 并发保护残留，并更正设计 R2 三类数据源 ID 查询语义的错误描述。R1 修订详情见 `reports/DATA-SUBSCRIPTION-NO-CONCURRENCY-REQUIREMENTS-ADJUSTMENT-001-R1.md`。本报告仍是调整草案执行报告，不是批准报告；R1 结果待重新正式复审。
...
- 定向调整需求 `DSUB-REQ-097/098/099/103`；验收调整累计涉及 `DSUB-AC-048/107/108/109/110/114/117` 共 7 条（原任务调整 6 条，R1 定向修正补充 `DSUB-AC-048` 并发保护残留）；并把 §14/§4.11 标题改为「无并发保护边界」。
...
### 5.1 业务行对照（累计 DSUB-AC-048/107/108/109/110/114/117，共 7 条）

| 编号 | 调整前 | 调整后 |
|---|---|---|
| DSUB-AC-048 | 删除仍按既定物理删除、二次确认和并发保护规则执行 | 删除仍按既定物理删除和二次确认规则执行，不做并发保护 |
...
相对原始基准 `026417e7...`，其余 119 条 `DSUB-AC` 业务行逐行保持不变；相对 R1 基准 `27a27e3...`，本次 R1 只新增修改 `DSUB-AC-048`。
...
### 5.3 DSUB-AC-048 并发保护残留的 R1 修正

`DSUB-AC-048`（订阅引用的源库或目标库已停用或不存在且为正常单源库记录）原验收描述结尾为“删除仍按既定物理删除、二次确认和并发保护规则执行”，与当前 `DSUB-REQ-103`、`DSUB-AC-114` 的“删除不做并发保护”语义直接冲突。ChatGPT 对提交 `27a27e3...` 正式复审确认该残留与当前需求直接冲突；R1 已将 `DSUB-AC-048` 纳入授权范围并定向修正为“删除仍按既定物理删除和二次确认规则执行，不做并发保护”。该修正只消除验收基线内部矛盾，不新增业务语义；验收基线现在统一为删除不做并发保护。
...
1. 三类数据源 ID 查询规则在服务层 Java 字面量过滤中的落地：不含英文逗号的普通 ID 对存储 CSV 去除 token 首尾空白后按完整 token 字面精确匹配；仅含英文句点且不含英文逗号的 ID 仍按完整 token 字面精确匹配（句点不是 `DATA_FROM_SOURCE_ID`/`DATA_TO_SOURCE_ID` 的分隔符）；含英文逗号的 ID 按“历史兼容可能匹配”处理，返回可能匹配记录集合并通过 `queryWarnings` 展示歧义警告；列表 API 响应结构调整为 `items + queryWarnings`；
...
| 6 | ACCEPTANCE 累计 7 条业务行变化 | `git diff 026417e -- ACCEPTANCE.md \| grep -E '^[+-]\| DSUB-AC-'` 见 DSUB-AC-048/107/108/109/110/114/117 | PASS |
| 7 | 其余 119 条 DSUB-AC 业务行不变（相对 `026417e`） | 对照原始基准逐行核对；相对 R1 基准 `27a27e3` 仅 `DSUB-AC-048` 一条变化（见 §5.3） | PASS |
