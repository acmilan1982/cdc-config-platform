...
| 文档状态 | `APPROVED`（正式验收前 UI 交互基线对齐草案及 R1 状态文字修订已经获得 ChatGPT 对结果提交 `26094c6b6d8f9b8d5971ef38648851611799adee` 的正式复审 `APPROVED`，批准收口完成；当前版本把 `DSUB-AC-058/061/069/072/073/082` 对应的最终交互事实（弹窗尺寸与视口约束、描述单行输入、空间优先级与顶部布局、紧凑目标库卡片、白色主体四态、Shift 连选）作为正式批准验收标准基线；上一正式批准验收标准版本为“取消并发保护”验收标准调整版本，已获得 ChatGPT 对 R1 结果提交 `43a909773aec63fe8c4de2957074f113910f4686` 的正式复审 `APPROVED`，该批准历史保留；更早“含逗号数据源 ID 查询兼容”批准版本（批准依据提交 `5d5b5f4606da14f160e9db43068f114d35501db8`）与“英文句点 `.` 保留分隔符”批准版本（批准依据提交 `bb8716c26d5181edf84ba1f07d4e60e8f1c1918a`）同样作为历史事实保留，见 §6 变更记录；验收编号为 126 条连续唯一；批准当时各用例状态为 `NOT_RUN`——该状态为验收标准批准及功能实现、验收执行前各历史阶段的当时状态，仅作历史保留，不代表当前；其后正式验收 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001` 与 R1 补验已执行并完成证据修订，126 条用例全部为 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`）；当时（R1-R1 复审通过前）验收执行状态为 `EXECUTED_PENDING_REVIEW`、实现状态为 `IMPLEMENTED_FORMAL_ACCEPTANCE_EXECUTED_PENDING_REVIEW`，均为该阶段的当时状态（详见”实现状态”行，不作为当前）；随后 ChatGPT 已对 R1-R1 结果提交 `ae66d90e4415ce51be54f8be2523bb44b55b78a2` 的正式复审 `APPROVED`，任务 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-CLOSEOUT-001` 完成最终接受收口，当前正式验收状态 `ACCEPTED`、验收执行状态 `PASS`、实现状态 `IMPLEMENTED_ACCEPTED`（详见”实现状态”/”正式验收状态”行）；本次批准的是验收标准基线，不代表批准当时验收已经执行或通过（历史事实）） |
| 实现状态 | `IMPLEMENTED_ACCEPTED`（数据订阅 Feature 已实现并完成正式验收与最终接受收口。过程：正式验收 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001` 已执行，126 条验收用例全部 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`），证据归档于 `evidence/DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001/`，报告见 `reports/DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001.md`；R1（`DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001-R1`）真实浏览器补验与证据定向修订已执行：126 条全部保持 `PASS`，R1 证据目录 `evidence/DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001-R1/`（54 个浏览器场景 + 干净 worktree 前端测试 + 数据库恢复证据），`coverage-matrix-r1.md` 取代原 `coverage-matrix.md` 的证据标注并纠正不准确的 `BR` 声明；R1-R1（`DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001-R1-R1`）完成状态与证据元数据定向修订；正式验收结果已经 ChatGPT 对结果提交 `ae66d90e4415ce51be54f8be2523bb44b55b78a2` 的正式复审 `APPROVED`，任务 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-CLOSEOUT-001` 完成最终接受收口。当前实现状态收口为 `IMPLEMENTED_ACCEPTED`、正式验收状态 `ACCEPTED`、验收执行状态 `PASS`；`IMPLEMENTED_FORMAL_ACCEPTANCE_EXECUTED_PENDING_REVIEW`/`EXECUTED_PENDING_REVIEW` 等为收口前各阶段的当时状态，仅作历史保留，本 Feature 当前不再有待复审入口） |
| 正式验收状态 | `ACCEPTED`（数据订阅正式验收已执行并经 ChatGPT 正式复审批准：ChatGPT 对结果提交 `ae66d90e4415ce51be54f8be2523bb44b55b78a2` 的正式复审结论 `APPROVED`，最终接受收口由任务 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-CLOSEOUT-001` 完成；126 条正式验收用例全部 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`），验收执行状态 `PASS`，本 Feature 不再有待验收复审入口） |
...
状态含义必须清楚：批准验收标准、执行验收、正式验收通过、实现正式接受是不同状态。上一正式批准版本（提交 `d7560445be1504e6ed9957fa7b31be1fd393ea19`）已获得项目负责人正式批准；其后与“英文句点 `.` 保留分隔符”直接相关的既有验收项定向扩展版本已获得 ChatGPT 正式复审 `APPROVED`（批准依据提交 `bb8716c26d5181edf84ba1f07d4e60e8f1c1918a`），该批准历史保留不变；再后含英文逗号数据源 ID 查询兼容验收语义的定向调整版本已获得 ChatGPT 正式复审 `APPROVED`（批准依据提交 `5d5b5f4606da14f160e9db43068f114d35501db8`）；项目负责人随后明确“数据订阅页面的新增、编辑、删除完全不处理并发操作，不考虑其他页面用户或人工直接修改数据库造成的并发冲突”，本文件据此定向调整 `DSUB-AC-107/108/109/110/114/117` 与 §4.11，形成“取消并发保护”验收标准调整草案；该草案（含 R1 对 `DSUB-AC-048` 的定向修正）已获得 ChatGPT 对结果提交 `43a909773aec63fe8c4de2957074f113910f4686` 的正式复审 `APPROVED`，当前验收标准版本已正式批准。验收标准基线获得批准不等于验收用例已执行。验收标准获批当时功能尚未实现（当时实现状态为 `NOT_STARTED`，属历史事实，不代表当前状态）；其后后端实现及真实数据库集成验证、前端 R3 代码与视觉、R3-R1 报告元数据收口均已获 ChatGPT 正式批准，功能已经实现并完成实现复审，当时（正式验收执行前）实现状态为 `IMPLEMENTED_REVIEW_APPROVED_PENDING_FORMAL_ACCEPTANCE`（代码已实现并复审通过，待正式验收，不是 `NOT_STARTED`，也不是 `IMPLEMENTED_ACCEPTED`，均为正式验收执行前阶段的当时状态，不代表当前）；当时（正式验收执行前）全部 126 条用例状态仍为 `NOT_RUN`，须待未来实际执行并取得与步骤匹配的客观证据后才允许更新为 `PASS / FAIL / BLOCKED`——该 `NOT_RUN` 为当时历史状态，不代表当前。后续正式验收前 UI 交互基线对齐草案（`DATA-SUBSCRIPTION-PRE-ACCEPTANCE-UI-BASELINE-ALIGNMENT-001`）把已确认、已实现、已复审通过的最终交互事实（弹窗尺寸、目标库卡片白色主体四态、Shift 连选）同步为待正式复审的文档草案；当时 126 条用例仍全部为 `NOT_RUN`（草案形成时的历史状态），本草案不代表正式验收通过；批准验收标准、实现复审通过、正式验收执行、Feature 正式接受仍是不同状态，该草案阶段当时本文件不得作为功能已实现或验收已通过的证据（为当时草案状态的约束，不代表当前）。随后，ChatGPT 对批准收口结果提交 `dc97ffdee677ce8068d2652c9758359349174c82` 的正式复核结论为 `CHANGES_REQUIRED`，唯一问题是四份文档当前状态元数据未完全收口；本 R1（`DATA-SUBSCRIPTION-PRE-ACCEPTANCE-UI-BASELINE-ALIGNMENT-APPROVAL-001-R1`）已定向修正本文件实现状态说明中把当前批准版本称为“草案”的过期文字（“当前功能已经实现并完成实现复审”的说明保持不回退），业务语义零变化，文档状态保持 `APPROVED`；当时 126 条用例仍全部为 `NOT_RUN`（该 R1 属批准收口阶段、正式验收执行前的元数据修订，`NOT_RUN` 为当时历史状态，不代表当前）。其后正式验收 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001` 及 R1 补验均已执行，当前 126 条用例全部为 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`），当时验收执行状态为 `EXECUTED_PENDING_REVIEW`、实现状态为 `IMPLEMENTED_FORMAL_ACCEPTANCE_EXECUTED_PENDING_REVIEW`（均为 R1-R1 复审通过前的当时状态）；其后 ChatGPT 对 R1-R1 结果提交 `ae66d90e4415ce51be54f8be2523bb44b55b78a2` 的正式复审结论为 `APPROVED`，任务 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-CLOSEOUT-001` 完成最终接受收口；当前 126 条用例全部为 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`），当前验收执行状态为 `PASS`、正式验收状态为 `ACCEPTED`、实现状态为 `IMPLEMENTED_ACCEPTED`，本 Feature 不再有待验收复审入口。
...
| 2026-09-02 | R1-R1 状态与证据元数据定向修订：ChatGPT 对 R1 结果提交 `f76239b...` 正式复审结论 `CHANGES_REQUIRED`——R1 的真实浏览器补验（54 场景）、干净 worktree 前端测试（定向 124 / 全量 359）、`npm run build`、数据库恢复（12=12、双向 MINUS 0、CLOB 一致 0、R1 残留 0）以及 126 条证据映射本身全部通过复审，仅三处状态/证据元数据矛盾需定向修正；本 R1-R1 修正：① ACCEPTANCE 顶部“文档状态”元数据行与 §1 长状态说明中把当前状态写作 `NOT_RUN` 的过期表达明确限定为历史阶段，当前唯一一致事实为 126 条全部 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`）、验收执行状态 `EXECUTED_PENDING_REVIEW`、实现状态 `IMPLEMENTED_FORMAL_ACCEPTANCE_EXECUTED_PENDING_REVIEW`；② 按既有明细 JSON 客观值（`browser/s4b-shift240.json reservedSelected=[]`、`browser/s4b-anchor240.json reservedFinal=[]`）把 S4-7a/S4-7b 及 `browser-scenario-index.md` 中“保留字符被选中”的相反/残缺汇总文字定向修正为“保留字符未被选中/已跳过、规则通过”；③ S5-9 场景 `ac` 数组与 `browser-scenario-index.md` 验收 ID 追加 `DSUB-AC-126`，与覆盖矩阵一致；文档状态保持 `APPROVED`，126 条表格行逐字零变化，浏览器场景数量保持 54、PASS 结论不变；未运行测试/构建/浏览器/服务或数据库操作；本 R1-R1 为纯文档与既有证据元数据定向修订，不代表正式验收通过或 `IMPLEMENTED_ACCEPTED` | DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001-R1-R1（数据订阅正式验收 R1 状态与证据元数据定向修订；本记录仅更新状态/证据元数据与执行汇总，验收标准业务语义零变化） |
| 2026-09-02 | 正式验收接受收口：ChatGPT 对 R1-R1 结果提交 `ae66d90e4415ce51be54f8be2523bb44b55b78a2` 的正式复审结论 `APPROVED`，确认 126 条正式验收用例全部 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`）、126 条验收业务行逐字零变化、R1 的 54 浏览器场景/干净 worktree 前端测试与构建/真实 HTTP/源 Oracle 元数据/数据库恢复证据继续有效、R1-R1 三项元数据修订正确、需求/设计/API/UI/数据库及前后端业务与测试代码无未经批准变化；本任务据此把 Feature 状态收口：文档状态保持 `APPROVED`，实现状态更新为 `IMPLEMENTED_ACCEPTED`，正式验收状态 `ACCEPTED`，验收执行状态 `PASS`，126 条全部 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`），本 Feature 不再有待验收复审入口；本任务为纯文档收口，未修改业务语义、验收业务行、代码、测试或证据，未运行测试/构建/浏览器/服务，未访问数据库、未执行 DDL/DML，未操作 ZooKeeper/Kafka/sync-client；大屏调整状态为 `READY_FOR_SEPARATE_TASK`（本次未执行）；下一独立入口为大屏 `DATA_SOURCE_TABLE` 英文逗号分隔解析修正任务的需求核验与任务规划 | DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-CLOSEOUT-001（数据订阅正式验收接受收口；本记录仅收口状态元数据与执行汇总，验收标准业务语义零变化） |
