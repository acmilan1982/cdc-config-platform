...
| 文档状态 | `APPROVED`（正式验收前 UI 交互基线对齐草案及 R1 状态文字修订已经获得 ChatGPT 对结果提交 `26094c6b6d8f9b8d5971ef38648851611799adee` 的正式复审 `APPROVED`，批准收口完成；当前版本把 `DSUB-AC-058/061/069/072/073/082` 对应的最终交互事实（弹窗尺寸与视口约束、描述单行输入、空间优先级与顶部布局、紧凑目标库卡片、白色主体四态、Shift 连选）作为正式批准验收标准基线；上一正式批准验收标准版本为“取消并发保护”验收标准调整版本，已获得 ChatGPT 对 R1 结果提交 `43a909773aec63fe8c4de2957074f113910f4686` 的正式复审 `APPROVED`，该批准历史保留；更早“含逗号数据源 ID 查询兼容”批准版本（批准依据提交 `5d5b5f4606da14f160e9db43068f114d35501db8`）与“英文句点 `.` 保留分隔符”批准版本（批准依据提交 `bb8716c26d5181edf84ba1f07d4e60e8f1c1918a`）同样作为历史事实保留，见 §6 变更记录；验收编号为 126 条连续唯一；批准当时各用例状态为 `NOT_RUN`——该状态为验收标准批准及功能实现、验收执行前各历史阶段的当时状态，仅作历史保留，不代表当前；其后正式验收 `DATA-SUBSCRIPTION-FORMAL-ACCEPTANCE-001` 与 R1 补验已执行并完成证据修订，当前 126 条用例全部为 `PASS`（0 `FAIL`/0 `BLOCKED`/0 `NOT_RUN`），当前验收执行状态为 `EXECUTED_PENDING_REVIEW`、实现状态为 `IMPLEMENTED_FORMAL_ACCEPTANCE_EXECUTED_PENDING_REVIEW`（详见“实现状态”行），等待 ChatGPT 对 R1-R1 结果提交正式复审，当前不是 `IMPLEMENTED_ACCEPTED`；本次批准的是验收标准基线，不代表批准当时验收已经执行或通过） |
