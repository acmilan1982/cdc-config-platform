...
| SC-UI-04 | 按 `CONFIG_KEY` 升序稳定排列。 |
| SC-UI-05 | 列表只展示：配置项说明 `CONFIG_DESC`、配置Key `CONFIG_KEY`、配置值 `CONFIG_VALUE`。 |
...
### 7.3 列布局规则（内容特征驱动，非机械等宽）

| 编号 | 需求 |
|---|---|
| SC-UI-10 | 列布局必须适配真实内容长度：配置Key和配置值通常较短，配置项说明通常较长，因此配置项说明应作为主内容列，占据列表最大可用宽度；配置Key和配置值使用相对紧凑但足以操作的列宽，不得三列机械等宽。 |
| SC-UI-11 | 配置项说明允许自然换行、多行完整展示，不应因固定窄列造成大段内容省略。 |
| SC-UI-12 | 配置Key或只读配置值偶尔超出列宽时可以使用省略显示，但悬停必须展示完整原文；这不属于脱敏。 |
...
| SC-DISPLAY-02 | 展示顺序为 `CONFIG_KEY` 升序；同名 key 的稳定排序以已批准数据库基线为准（当前无重复 key，按主键等稳定次序处理即可，不引入分页/筛选）。 |
| SC-DISPLAY-03 | 每条记录展示三列：配置项说明 `CONFIG_DESC`、配置Key `CONFIG_KEY`、配置值 `CONFIG_VALUE`。 |
| SC-DISPLAY-04 | `CONFIG_VALUE` 为空、空白或异常时，按只读原样展示；不得因显示异常数据触发编辑能力。 |
...
| SC-CFG-GEN-02 | 保存值必须满足数据库物理长度 `VARCHAR2(64)`；不得依赖数据库截断或异常作为正常校验方式。 |
...
| SC-CONFIRM-02 | 确认框只展示实际发生变化的配置，逐项包含：配置项说明、配置Key、原值、新值。 |
