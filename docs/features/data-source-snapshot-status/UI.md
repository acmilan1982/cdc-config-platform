...
| 本轮（查询控件交互调整草案）下一入口 | `CHATGPT_BASELINE_R1_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_APPROVAL`（本轮草案基线 R1 的下一入口只能是 ChatGPT 从远程 Git 复审本草案 R1，再由项目负责人明确决定是否批准；批准前不得开始实现） |
...
- 变更记录：2026-09-10，新增 §20 并更新 §1 元数据（新增本轮草案相关状态行）；本 R1 只定向纠正当前状态残留与下一入口（不修改 §20 界面业务规则）。下一入口：`CHATGPT_BASELINE_R1_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_APPROVAL`。
