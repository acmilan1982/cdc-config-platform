...
- 代码零差异：本轮不产生 `frontend/`/`backend/`/测试/SQL/配置/依赖/锁文件差异（`frontend_code_diff=ZERO`、`backend_code_diff=ZERO`）。
