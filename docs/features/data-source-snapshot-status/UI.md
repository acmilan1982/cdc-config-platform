...
| 本轮（查询下拉固定宽度基线）下一入口 | `CHATGPT_POPPER_WIDTH_BASELINE_R2_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_DOCUMENT_APPROVAL`（本轮界面规则已批准收口（`popper_width_document_status=APPROVED`）；批准收口任务 `...-BASELINE-APPROVAL-001`、R1 修正任务 `...-BASELINE-APPROVAL-001-R1` 与独立实现任务 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-IMPLEMENTATION-001` 均已完成，实现已在 `5173` 正式前端按批准内容基准 `f74dd725682b745f1b8ac6a1b9358eff10aaddc2` 落地 `DSS-REQ-087` 固定宽度规则（`popper_width_formal_5173_implementation_status=IMPLEMENTED_ADJUSTMENT_PENDING_REVIEW`，界面落点见 §21），其实现提交经 ChatGPT 从远程 Git 复审结论为 `CHANGES_REQUIRED`（`popper_width_formal_code_review_status=CHANGES_REQUIRED`）；本 R2 任务提出支持视口边界修正草案（正式支持下限 `1280px`、`<1280px` 仅防御性观察、不计入正式验收，文档状态 `DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`，未批准），当前下一入口为 ChatGPT 从 Git 复审本 R2 文档、随后由项目负责人做文档批准；**不得**在本任务执行正式验收、107 条验收保持 `NOT_RUN`） |
| 本轮（查询下拉固定宽度基线）支持视口边界修正（R2）状态 | `DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`（`popper_width_r2_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`：正式支持视口下限修正为 `viewport width >= 1280px`，`<1280px` 仅保留防御性收缩、其窄视口观察不计入正式验收；修正本文件 §21 与 `DSS-REQ-087`/`DSS-AC-104~107`，删除“`<496/<416/<256px` 必须正式通过”的要求；该修正基于 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-IMPLEMENTATION-001` 实现证据提出，属项目负责人支持边界修正草案，**尚未**经 ChatGPT 复审与项目负责人批准，**不得**写成 `APPROVED`/`IMPLEMENTED_ACCEPTED`/`FORMALLY_ACCEPTED`/`PASS`/`COMPLETED`；原 `480/400/240px` 数值、外层公式与触发控件尺寸均不变） |
...
- 编号与追踪：`DSS-REQ-087`（累计 87 条，已收口为 `APPROVED`）、`DSS-AC-104~107`（累计 107 条全部 `NOT_RUN`、`acceptance_not_run_count=107`，已收口为 `APPROVED`）；矩阵 87/87 与 107/107 双向无悬空；本轮已批准，但**批准不等于已实现或已验收**，禁止写成 `IMPLEMENTED`/`ACCEPTED`/`COMPLETED`，也不得把任何验收写成 `PASS`。
