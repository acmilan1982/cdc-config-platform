...
> 本节为 2026-09-15 任务 `DATA-SOURCE-SNAPSHOT-STATUS-QUERY-BUTTON-AND-TABLE-LAYOUT-STABILITY-BASELINE-001` 新增的独立界面规则草案小节，对应 `REQUIREMENTS.md` §21.10 新增需求 `DSS-REQ-090`/`DSS-REQ-091` 与 `ACCEPTANCE.md` §4.24 新增验收 `DSS-AC-114~118`（5 条**全部** `NOT_RUN`）。**本节为纯文档草案**：`query_button_and_table_layout_stability_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`，**未批准**（**不得**写成 `APPROVED`）、**未实现**（**不得**写成 `IMPLEMENTED`）、**未验收**（**不得**写成 `PASS`/`ACCEPTED`）、Feature 最终接受收口因本轮新增调整暂缓。方向本身已由项目负责人确认（`query_button_and_table_layout_stability_solution_direction_status=APPROVED_BY_PROJECT_OWNER`），但确认方向不等于文档批准、更不等于实现或验收授权。
