...
| 本轮（查询下拉固定宽度基线草案）内容状态 | `DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`（`popper_width_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`：本节新增 `DSS-REQ-087` 与 `ACCEPTANCE.md` §4.22 `DSS-AC-104~107` 均为**草案内容**，须待 ChatGPT 从 Git 复审后再由项目负责人批准；**不得**写成 `APPROVED`/`IMPLEMENTED`/`PASS`/`ACCEPTED`） |
