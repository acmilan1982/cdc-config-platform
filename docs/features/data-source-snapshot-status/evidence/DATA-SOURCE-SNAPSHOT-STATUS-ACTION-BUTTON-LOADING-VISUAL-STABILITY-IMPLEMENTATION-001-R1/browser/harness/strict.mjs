// R1 strict whole-rect matrix (DATA-SOURCE-SNAPSHOT-STATUS-ACTION-BUTTON-LOADING-VISUAL-STABILITY-IMPLEMENTATION-001-R1).
// Runs the real Chromium against the Vite dev server of THIS result worktree, with the single GET endpoint
// intercepted at the browser layer. GET-only: no backend, no DB, no ZooKeeper, no writes.
//
// It deliberately REPLACES the R0 substitute criterion ("right anchor edge stable + same timestamp string")
// with the original DSS-AC-113 judgement: the whole .dss-refresh-group rect (x/y/width/height) must be
// identical across every state, including '--', several HH:mm:ss values with different glyph widths, several
// countdown values, and the post-success time update. No tolerance is used to mask displacement.
...
async function main() {
  ...
  for (const [w, h] of VIEWPORTS) {
    ...
    await fulfill(held, okEnvelope(R2))
    await waitFor(
      `!document.querySelector('.dss-refresh-btn .dss-btn-spinner').classList.contains('is-visible')`,
      6000,
      'manual loading off',
    ...
    // ---- 19: QUERY_SUCCESS
    await fulfill(qheld, okEnvelope(R2))
    await waitFor(
      `!document.querySelector('.dss-query-btn .dss-btn-spinner').classList.contains('is-visible')`,
      6000,
      'query loading off',
    ...
    process.stdout.write(`viewport ${w}x${h}: group delta=${JSON.stringify(d.refresh_group_rect_delta)}\n`)
  }
  ...
  await fulfill(rmHeld, okEnvelope(R2))
  await sleep(300)
