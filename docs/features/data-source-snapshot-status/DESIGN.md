...
## 27. 查询下拉弹层（popper）固定宽度基线草案设计记录（`DSS-REQ-087`）

> 本节为 2026-09-11 任务 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-BASELINE-001` 新增的独立设计记录，对应 `REQUIREMENTS.md` §21.8 新增草案需求 `DSS-REQ-087` 与 `ACCEPTANCE.md` §4.22 新增草案验收 `DSS-AC-104~107`。本设计落点为 `D§27、U§21`。本节内容为**待复审草案**（`popper_width_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`），**不得**写成 `APPROVED`；`5173` 正式前端尚未应用本规则（`popper_width_formal_5173_implementation_status=PENDING_FORMAL_IMPLEMENTATION_ON_5173`）。
...
- 本任务为**纯文档草案建立**：不产生 `frontend/`/`backend/`/SQL/配置差异、不把 prototype 代码拷入 `5173`、不执行正式验收（`formal_acceptance_status=NOT_RUN`、`acceptance_not_run_count=107`）、不代替项目负责人批准草案。
