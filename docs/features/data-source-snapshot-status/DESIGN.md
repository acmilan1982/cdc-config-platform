...
| 查询按钮与表格布局稳定性文档分层状态（本轮） | 本轮新增设计章节 §38，对应需求 `DSS-REQ-090`/`DSS-REQ-091`（见 `REQUIREMENTS.md` §21.10）与验收 `DSS-AC-114~118`（见 `ACCEPTANCE.md` §4.24，全部 `NOT_RUN`）。分层状态：`accepted_scope_acceptance_status=PASS_113`、`accepted_scope_pass_count=113`、`query_button_and_table_layout_stability_solution_direction_status=APPROVED_BY_PROJECT_OWNER`、`query_button_and_table_layout_stability_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`、`query_button_and_table_layout_stability_requirements_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`、`query_button_and_table_layout_stability_design_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`、`query_button_and_table_layout_stability_ui_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`、`query_button_and_table_layout_stability_implementation_status=PENDING_FORMAL_IMPLEMENTATION_ON_5173`、`query_button_and_table_layout_stability_code_review_status=NOT_RUN`、`query_button_and_table_layout_stability_human_visual_interaction_review_status=NOT_RUN`、`query_button_and_table_layout_stability_acceptance_status=NOT_RUN`、`query_button_and_table_layout_stability_acceptance_not_run_count=5`、`requirements_count=91`、`acceptance_count=118`、`formal_acceptance_pass_count=113`、`pending_user_review=YES`、`pending_user_confirmation_count=0`。**本轮不得写成 `APPROVED`/`IMPLEMENTED`/`PASS`/`ACCEPTED`/`COMPLETED` 或最终收口**；既有 113 条 `PASS` 是此前范围的真实执行结果，不得改回 `NOT_RUN`，也不得延伸解释为本轮 5 条新增验收已经通过。Feature 最终接受收口因本轮新增调整暂缓。 |
| 查询按钮与表格布局稳定性 R1 文档口径纠正状态（本轮 `DATA-SOURCE-SNAPSHOT-STATUS-QUERY-BUTTON-AND-TABLE-LAYOUT-STABILITY-BASELINE-001-R1`，2026-09-15） | 按 ChatGPT 对 R0 提交 `be101a19558ae1ef1436bb3d61746dc21338e49a` 的复审结论执行纯文档极小定向 R1 纠正：① 统一“重置”按钮四属性锁定口径为 `width = min-width = max-width = flex-basis = 62px`（`UI.md` §32.2，与 §38.3 一致）；② 消除 `DSS-AC-114` 跨按钮坐标歧义（改为每个按钮分别与自身稳定基准比较；“查询”与“重置”只比较宽度集合均为 `[62]`，不比较绝对 `x` 坐标）。本文件 §38 业务规则正文逐字节不变。分层状态：`chatgpt_r0_review_status=CHANGES_REQUIRED_TWO_DOCUMENT_WORDING_CONFLICTS_ONLY`、`r1_document_wording_correction_status=COMPLETED_PENDING_CHATGPT_REVIEW`、`accepted_scope_acceptance_status=PASS_113`、`query_button_and_table_layout_stability_solution_direction_status=APPROVED_BY_PROJECT_OWNER`、`query_button_and_table_layout_stability_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`、`query_button_and_table_layout_stability_implementation_status=PENDING_FORMAL_IMPLEMENTATION_ON_5173`、`query_button_and_table_layout_stability_code_review_status=NOT_RUN`、`query_button_and_table_layout_stability_acceptance_status=NOT_RUN`、`query_button_and_table_layout_stability_acceptance_not_run_count=5`、`pending_user_review=YES`、`pending_user_confirmation_count=0`。 |
| 查询按钮与表格布局稳定性草案下一入口 | `CHATGPT_QUERY_BUTTON_AND_TABLE_LAYOUT_STABILITY_BASELINE_R1_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_DOCUMENT_APPROVAL`（**当前直接值（2026-09-15 R1 纠正任务提交并推送后）**：等待 ChatGPT 从远程 Git 复审 R1 纠正结果（R0 复审结论 `chatgpt_r0_review_status=CHANGES_REQUIRED_TWO_DOCUMENT_WORDING_CONFLICTS_ONLY`，两项文字口径冲突已由 `DATA-SOURCE-SNAPSHOT-STATUS-QUERY-BUTTON-AND-TABLE-LAYOUT-STABILITY-BASELINE-001-R1` 原位纠正），再由项目负责人决定是否批准本草案文档。历史（2026-09-15 R0 草案提交并推送后）该入口曾为 `CHATGPT_QUERY_BUTTON_AND_TABLE_LAYOUT_STABILITY_BASELINE_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_DOCUMENT_APPROVAL`（**历史（2026-09-15 R0 草案提交并推送后曾为本轮当前直接值，已由 `DATA-SOURCE-SNAPSHOT-STATUS-QUERY-BUTTON-AND-TABLE-LAYOUT-STABILITY-BASELINE-001-R1` 处理，不构成本轮当前直接值）**）：等待 ChatGPT 从远程 Git 复审本轮草案，之后由项目负责人决定是否批准文档；历史（2026-09-14 实现复审收口任务提交后至 2026-09-15 R1 纠正任务时点）：曾为 `DATA-SOURCE-SNAPSHOT-STATUS-ACTION-BUTTON-LOADING-VISUAL-STABILITY-FORMAL-ACCEPTANCE-001` 与 `CHATGPT_ACTION_BUTTON_LOADING_VISUAL_STABILITY_FORMAL_ACCEPTANCE_R1_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_FINAL_ACCEPTANCE_DECISION`，均已被本任务取代，不构成本轮当前直接值；再前截至 2026-09-14 操作按钮 Loading 视觉稳定性调整提出前为 `NONE_FEATURE_ACCEPTED`） |
...
> 本节为 2026-09-15 任务 `DATA-SOURCE-SNAPSHOT-STATUS-QUERY-BUTTON-AND-TABLE-LAYOUT-STABILITY-BASELINE-001` 新增的独立设计章节，对应 `REQUIREMENTS.md` §21.10 新增需求 `DSS-REQ-090`/`DSS-REQ-091` 与 `ACCEPTANCE.md` §4.24 新增验收 `DSS-AC-114~118`。本节只记录**已由项目负责人确认的调整方向与冻结的设计口径**，属**纯文档草案**（`query_button_and_table_layout_stability_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`）：不批准草案、不在 `5173` 实现、不执行本轮新增验收、不做最终接受收口、不启停服务、不访问数据库/ZooKeeper/Kafka。既有 `DSS-AC-001~113` 共 113 条保持 `PASS`，本轮 5 条新增验收全部 `NOT_RUN`；本节**不得**被解释为本轮已实现或已通过。
