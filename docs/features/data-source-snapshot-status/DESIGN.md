...
| 本轮（查询下拉固定宽度基线）正式实现状态 | `IMPLEMENTED_ADJUSTMENT_PENDING_REVIEW`（`popper_width_formal_5173_implementation_status=IMPLEMENTED_ADJUSTMENT_PENDING_REVIEW`、`popper_width_formal_code_review_status=CHANGES_REQUIRED`（ChatGPT 从远程 Git 对该实现提交的代码复审结论为 `CHANGES_REQUIRED`）；已由独立实现任务 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-IMPLEMENTATION-001` 在 `5173` 正式前端按批准内容基准 `f74dd725682b745f1b8ac6a1b9358eff10aaddc2` 落地 `DSS-REQ-087` 固定宽度规则——只改 Feature 私有 `DataSourceSnapshotQueryBar.vue` 与其 `.spec.ts`，`frontend_code_diff` 仅限该 Feature 私有文件、`backend_code_diff=ZERO`，开发验证状态 `DEV_SELF_TEST_DONE`；`formal_acceptance_status=NOT_RUN`、`human_formal_5173_review_status=NOT_RUN_FOR_THIS_ADJUSTMENT`、`pending_user_review=NO`、`pending_user_confirmation_count=0`；**不得**写成 `IMPLEMENTED`/`IMPLEMENTED_ACCEPTED`/`FORMALLY_ACCEPTED`/`PASS`/`COMPLETED`） |
| 本轮（查询下拉固定宽度基线）追踪计数 | 需求 → 设计落点 **87/87**（§14.2）、验收 → 设计落点 **107/107**（§14.3）；新增 `DSS-AC-104~107` 已收口为 `APPROVED` 且全部 `NOT_RUN` |
| 本轮（查询下拉固定宽度基线）下一入口 | `CHATGPT_POPPER_WIDTH_BASELINE_R2_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_DOCUMENT_APPROVAL`（本轮设计已批准收口（`popper_width_document_status=APPROVED`）；批准收口任务 `...-BASELINE-APPROVAL-001`、R1 修正任务 `...-BASELINE-APPROVAL-001-R1` 与独立实现任务 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-IMPLEMENTATION-001` 均已完成，实现已在 `5173` 正式前端按批准内容基准 `f74dd725682b745f1b8ac6a1b9358eff10aaddc2` 落地 `DSS-REQ-087` 固定宽度规则（`popper_width_formal_5173_implementation_status=IMPLEMENTED_ADJUSTMENT_PENDING_REVIEW`，设计落点见 §27），其实现提交经 ChatGPT 从远程 Git 复审结论为 `CHANGES_REQUIRED`（`popper_width_formal_code_review_status=CHANGES_REQUIRED`）；本 R2 任务提出支持视口边界修正草案（正式支持下限 `1280px`、`<1280px` 仅防御性观察、不计入正式验收，文档状态 `DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`，未批准），当前下一入口为 ChatGPT 从 Git 复审本 R2 文档、随后由项目负责人做文档批准；**不得**在本任务执行正式验收、107 条验收保持 `NOT_RUN`） |
| 本轮（查询下拉固定宽度基线）支持视口边界修正（R2）状态 | `DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`（`popper_width_r2_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`：正式支持视口下限修正为 `viewport width >= 1280px`，`<1280px` 仅保留防御性收缩、其窄视口观察不计入正式验收；修正 §27.4/§27.7 与 `DSS-REQ-087`/`DSS-AC-104~107`，删除“`<496/<416/<256px` 必须正式通过”的要求；该修正基于 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-IMPLEMENTATION-001` 实现证据提出，属项目负责人支持边界修正草案，**尚未**经 ChatGPT 复审与项目负责人批准，**不得**写成 `APPROVED`/`IMPLEMENTED_ACCEPTED`/`FORMALLY_ACCEPTED`/`PASS`/`COMPLETED`；原 `480/400/240px` 数值、外层公式与触发控件尺寸均不变） |
...
> 本节为 2026-09-11 任务 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-BASELINE-001` 新增的独立设计记录，对应 `REQUIREMENTS.md` §21.8 新增需求 `DSS-REQ-087` 与 `ACCEPTANCE.md` §4.22 新增验收 `DSS-AC-104~107`，已由 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-BASELINE-APPROVAL-001` 收口为 `APPROVED`（`popper_width_document_status=APPROVED`：ChatGPT 已从远程 Git 对批准内容基准提交 `f74dd725682b745f1b8ac6a1b9358eff10aaddc2` 完成最终复审 `APPROVED`、项目负责人于 2026-09-11 明确回复“批准”）。本设计落点为 `D§27、U§21`。本规则已由 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-IMPLEMENTATION-001` 在 `5173` 正式前端落地（`popper_width_formal_5173_implementation_status=IMPLEMENTED_ADJUSTMENT_PENDING_REVIEW`），其代码复审结论 `CHANGES_REQUIRED`（`popper_width_formal_code_review_status=CHANGES_REQUIRED`）；R2 任务 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-BASELINE-001-R2` 据此提出支持视口边界修正（正式支持下限 `viewport width >= 1280px`、`<1280px` 仅防御性观察、不计入正式验收），其文档状态 `popper_width_r2_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`（**未批准**）。
...
- 分层状态：`prototype_width_decision_status=APPROVED_BY_PROJECT_OWNER`、`popper_width_document_status=APPROVED`、`popper_width_r2_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`（R2 支持视口边界修正草案，**未批准**）、`popper_width_formal_5173_implementation_status=IMPLEMENTED_ADJUSTMENT_PENDING_REVIEW`、`popper_width_formal_code_review_status=CHANGES_REQUIRED`、`formal_acceptance_status=NOT_RUN`、`human_formal_5173_review_status=NOT_PASSED`、`pending_user_review=YES`、`pending_user_confirmation_count=0`。
- 下一入口：`CHATGPT_POPPER_WIDTH_BASELINE_R2_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_DOCUMENT_APPROVAL`（本轮设计已批准收口（`popper_width_document_status=APPROVED`）；独立实现任务 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-IMPLEMENTATION-001` 已按批准内容基准 `f74dd725682b745f1b8ac6a1b9358eff10aaddc2` 在 `5173` 正式前端落地 `DSS-REQ-087`，其实现提交经 ChatGPT 从远程 Git 复审结论为 `CHANGES_REQUIRED`；R2 任务已据实现证据提出支持视口边界修正草案（正式支持下限 `1280px`、`<1280px` 防御性观察、不计入正式验收，文档状态 `DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`，未批准），当前下一入口为 ChatGPT 从 Git 复审本 R2 文档、随后由项目负责人做文档批准；**不得**在本任务执行正式验收、107 条验收保持 `NOT_RUN`）。
