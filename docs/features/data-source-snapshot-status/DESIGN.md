...
- 验收状态：`DSS-AC-001~107` 共 107 条**全部 `NOT_RUN`**、`acceptance_not_run_count=107`；本轮已批准（`popper_width_document_status=APPROVED`），但**批准不等于已实现或已验收**——禁止把本轮批准写成 `IMPLEMENTED`/`ACCEPTED`/`COMPLETED`，也不得把任何验收写成 `PASS`。
