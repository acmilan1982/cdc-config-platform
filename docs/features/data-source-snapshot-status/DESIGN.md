...
- 下一入口：`CHATGPT_BASELINE_REVIEW_FROM_GIT_THEN_PROJECT_OWNER_APPROVAL`（ChatGPT 从 Git 复审本草案基线，再由项目负责人决定是否批准；批准前不得开始实现）。
