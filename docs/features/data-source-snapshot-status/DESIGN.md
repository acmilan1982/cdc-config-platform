...
> 本节为 2026-09-15 任务 `DATA-SOURCE-SNAPSHOT-STATUS-QUERY-BUTTON-AND-TABLE-LAYOUT-STABILITY-BASELINE-001` 新增的独立设计章节，对应 `REQUIREMENTS.md` §21.10 新增需求 `DSS-REQ-090`/`DSS-REQ-091` 与 `ACCEPTANCE.md` §4.24 新增验收 `DSS-AC-114~118`。本节记录**已由项目负责人确认的调整方向与冻结的设计口径**，并已由 `DATA-SOURCE-SNAPSHOT-STATUS-QUERY-BUTTON-AND-TABLE-LAYOUT-STABILITY-BASELINE-APPROVAL-001` 批准收口为 `query_button_and_table_layout_stability_document_status=APPROVED`（批准日期 2026-09-15，批准内容基准提交 `46d8aeb3fc3ec332788933635c47936592b70b1e`）：仍不在 `5173` 实现、不执行本轮新增验收、不做最终接受收口、不启停服务、不访问数据库/ZooKeeper/Kafka（**批准前历史状态（2026-09-15 草案建立至本次批准收口前）**：当时本节为**纯文档草案**（`query_button_and_table_layout_stability_document_status=DRAFT_PENDING_CHATGPT_REVIEW_AND_PROJECT_OWNER_APPROVAL`）、未批准、未实现、未验收，**已由批准收口处理完毕，不构成当前直接值**）。既有 `DSS-AC-001~113` 共 113 条保持 `PASS`，本轮 5 条新增验收全部 `NOT_RUN`；**批准文档不等于实现完成，也不等于本轮 5 条新增验收 `DSS-AC-114~118` 已执行**；本节**不得**被解释为本轮已实现或已通过。
...
- **自检**：`DSS-REQ-090`/`DSS-REQ-091` 与 `DSS-AC-114~118` 编号连续、无重号、无复用；本轮 5 条新增验收全部 `NOT_RUN`；既有 `DSS-REQ-001~089` 与 `DSS-AC-001~113` 业务行相对基准 `94a84239ae9b617e783f5acf20274e838e11a105` 逐字节不变；`frontend/**`、`backend/**`、测试代码、SQL、配置、依赖与锁文件零差异。
