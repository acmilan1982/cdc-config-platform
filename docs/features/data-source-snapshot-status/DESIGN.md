...
| 本轮（查询下拉固定宽度基线）追踪计数 | 需求 → 设计落点 **87/87**（§14.2）、验收 → 设计落点 **107/107**（§14.3）；新增 `DSS-AC-104~107` 已收口为 `APPROVED` 且全部 `NOT_RUN` |
...
以下为本轮查询控件交互调整草案新增（对应 REQUIREMENTS §21.6、ACCEPTANCE §4.21；对应基线已批准，实现已进入 `5173`，其独立代码复审已由 `DATA-SOURCE-SNAPSHOT-STATUS-QUERY-CONTROL-INTERACTION-IMPLEMENTATION-REVIEW-CLOSEOUT-001`（2026-09-12）从 `PENDING_CHATGPT_REVIEW` 收口为 `APPROVED`（ChatGPT 从远程 Git 复审 R1 修正实现提交 `edba7c891884d0f000a7c9edc196b2bffb95e0b0`），项目负责人人工视觉/交互检查 `APPROVED_BY_PROJECT_OWNER`；这 8 条验收全部 `NOT_RUN`）：
...
- 本任务为**纯文档批准收口**（`DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-BASELINE-APPROVAL-001`）：不产生 `frontend/`/`backend/`/SQL/配置差异、不把 prototype 代码拷入 `5173`、不执行正式验收（`formal_acceptance_status=NOT_RUN`、`acceptance_not_run_count=107`）；基准草案任务为 `DATA-SOURCE-SNAPSHOT-STATUS-SELECT-POPPER-FIXED-WIDTH-BASELINE-001`。
