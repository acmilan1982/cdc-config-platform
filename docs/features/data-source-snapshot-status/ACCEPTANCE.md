...
| 文档状态 | `APPROVED`（当前版为 2026-09-08 第二轮验收前 UI 调整草案 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-002` 及其 R1 极小定向修订 `...-002-R1` 收口后的当前第二轮 UI 调整版本。驱动：项目负责人对第一轮 UI 调整实现 R1（提交 `5933ec2...`）对应预览页人工检查结论 `CHANGES_REQUIRED`，故在已批准基线之上定向修订展示规则相关既有验收用例并新增 `DSS-AC-081~086`（修订说明见 §4.6/§4.8/§4.18/§4.19）；R1 极小定向修订仅澄清相关既有需求/设计与实现职责归属表述，不改本轮验收范围与用例集合。第一轮 UI 调整批准版本（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-001`，批准内容基准提交 `5757237...`，2026-09-07）保留为历史。本轮只围绕四类已确认调整范围（表格铺满结果卡片、探针端列展示简化、源库列展示简化、探针端查询下拉长度与文本截断）定向修订既有验收用例并新增 `DSS-AC-081~086`（全部 `NOT_RUN`）；本轮不改接口、SQL、表结构、数据库访问与产品只读边界（API.md/DATABASE.md 整文件零差异）。该版本已经 ChatGPT 对 R1 结果提交 `5da9b17c1a720f89482eeda1436ad633145fe9fa` 独立正式复审 `APPROVED` 与项目负责人明确“批准”，`acceptance_status`/`baseline_status` 当前第二轮调整版本收口为 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准日期 2026-09-08）。批准的是本轮验收标准，不代表本轮调整已实现、正式验收已执行或通过，不等于 `IMPLEMENTED_ACCEPTED`；全部 `DSS-AC-001~086` 仍 `NOT_RUN`，见本表“本轮（第二轮 UI 调整版本）批准链”与 §1 重要声明） |
| baseline_status | 本轮（第二轮）UI 调整版本已批准为 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准内容基准提交 `5da9b17...`，2026-09-08）；第一轮 UI 调整批准版本 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-001`，批准内容基准提交 `5757237...`，2026-09-07）与既有批准版 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-REQUIREMENTS-BASELINE-APPROVAL-001`）保留为历史 |
| acceptance_status | `APPROVED`（当前第二轮 UI 调整版本已批准：正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准内容基准提交 `5da9b17...`，2026-09-08；`DSS-AC-001~086` 共 86 条全部 `NOT_RUN`；第一轮批准版本与既有批准版本保留为历史） |
...
| 实现状态 | `IMPLEMENTED_ADJUSTMENT_PENDING`（implementation_status=IMPLEMENTED_ADJUSTMENT_PENDING：第二轮 UI 调整已批准、尚未实现，将另立第二轮 UI 调整实现任务（严格基于批准内容基准提交 `5da9b17...`）落地；第一轮 UI 调整已由实现任务 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-IMPLEMENTATION-001`（2026-09-07）按批准内容基准提交 `5757237...` 落地、实现 R1 提交 `5933ec2...` 经 ChatGPT 独立复审 `APPROVED`、实现状态 `IMPLEMENTED_ADJUSTMENT_PENDING_REVIEW`，作为历史；不代表代码复审通过、不代表正式验收或人工验收已执行或通过，不等于 `IMPLEMENTED_ACCEPTED`） |
| 设计状态 | `DESIGN.md`/`UI.md` 当前第二轮调整版本为 `APPROVED`（已随本轮批准收口，见对应文档）；`API.md`/`DATABASE.md` 保持已批准（`APPROVED`）且本轮**整文件零差异**（本轮不改接口、SQL、表结构、数据库访问与产品只读边界） |
...
| 调整草案关系 | 历史基线：已批准需求/验收基线（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-REQUIREMENTS-BASELINE-APPROVAL-001`，批准内容基准提交 `4234af73db2190098f3dcd219319a4281fdabafd`）、已批准设计基线（`DATA-SOURCE-SNAPSHOT-STATUS-DESIGN-BASELINE-APPROVAL-001`）与第一轮 UI 调整批准版本（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-001`，批准内容基准提交 `5757237...`，2026-09-07；已由实现任务 `...-UI-ADJUSTMENT-IMPLEMENTATION-001` 落地、实现 R1 提交 `5933ec2...` 经 ChatGPT 独立复审 `APPROVED`）全部保留为历史。当前（第二轮）UI 调整版本（第二轮验收前 UI 调整草案 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-002` 及 R1 极小定向修订 `...-002-R1`）经 ChatGPT 对 R1 结果提交 `5da9b17c1a720f89482eeda1436ad633145fe9fa` 独立正式复审 `APPROVED` 且项目负责人明确回复“批准”，已由 `DRAFT_ADJUSTMENT_PENDING_USER_REVIEW` 收口为 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准日期 2026-09-08）。本轮定向修订受影响的既有验收用例（`DSS-AC-020/022/026/027/038/039/040/041/042/073/074/075/076/077/080`，见 §4.6/§4.8/§4.18）并连续新增 `DSS-AC-081~086`（见 §4.19），不改接口、SQL、表结构、数据库访问与产品只读边界（API.md/DATABASE.md 整文件零差异）。下一入口为另立第二轮 UI 调整实现任务（严格基于批准内容基准提交 `5da9b17...` 实现） |
| 文档版本 | 批准收口版（2026-09-08）：第二轮验收前 UI 调整草案 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-002` 及 R1 极小定向修订（`...-002-R1`）经 ChatGPT 对 R1 结果提交 `5da9b17c1a720f89482eeda1436ad633145fe9fa` 独立正式复审 `APPROVED`、项目负责人随后明确“批准”，本文件当前第二轮 UI 调整版本由 `DRAFT_ADJUSTMENT_PENDING_USER_REVIEW` 收口为 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准内容基准提交 `5da9b17...`，2026-09-08）；业务零变化——`DSS-AC-001~086` 仍 86 条全部 `NOT_RUN`、需求 `DSS-REQ-001~075` 仍 75 条、§5 需求—验收追踪矩阵零差异、DESIGN/UI 业务内容与追踪矩阵零差异、`API.md`/`DATABASE.md`/第二轮初版报告 `...-002.md`/R1 报告 `...-002-R1.md` 整文件零差异。实现状态保持 `IMPLEMENTED_ADJUSTMENT_PENDING`、正式验收执行 `NOT_RUN`、人工页面验收 `NOT_RUN`、`human_visual_review_status=CHANGES_REQUIRED`（第一轮页面人工检查历史）、`pending_user_review=NO`、`pending_user_confirmation_count=0`。历史版本：需求与验收批准收口版（2026-09-05，正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-REQUIREMENTS-BASELINE-APPROVAL-001`，批准内容基准 `4234af73db2190098f3dcd219319a4281fdabafd`）与第一轮 UI 调整批准版本保留为历史；批准的是各轮验收标准基线，不代表本轮调整已实现、验收已执行或通过，不等于 `IMPLEMENTED_ACCEPTED`。计数：`DSS-AC-001~086` 共 86 条全部 `NOT_RUN`（详见 §6/§7） |
| 创建日期 | 2026-09-05（初版）；R1、R2、R3 修订及批准收口同日；2026-09-07 建立第一轮验收前 UI 调整草案并批准/实现；2026-09-08 建立第二轮验收前 UI 调整草案 |
| 依据需求 | `docs/features/data-source-snapshot-status/REQUIREMENTS.md`（既有批准基线 `DSS-REQ-001~065`、第一轮 UI 调整版本新增 `DSS-REQ-066~071` 与第二轮 UI 调整版本新增 `DSS-REQ-072~075`，共 `DSS-REQ-001~075` 75 条；当前第二轮 UI 调整版本文档状态 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准内容基准提交 `5da9b17...`，2026-09-08），既有批准版本保留为历史） |
...
- 本版（2026-09-08）为第二轮验收前 UI 调整草案（`DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-002`）及其 R1 极小定向修订（`...-002-R1`）收口后的当前第二轮 UI 调整版本：依据项目负责人对第一轮实现 R1 对应预览页人工检查结论（`human_visual_review_status=CHANGES_REQUIRED`，第一轮页面人工检查历史）定向修订受影响的既有验收用例（`DSS-AC-020/022/026/027/038/039/040/041/042/073/074/075/076/077/080`，见 §4.6/§4.8/§4.18）并连续新增 `DSS-AC-081~086`（见 §4.19），修订后为 `DSS-AC-001~086` 共 86 条全部 `NOT_RUN`（本轮不改接口、SQL、表结构、数据库访问与产品只读边界，`API.md`/`DATABASE.md` 整文件零差异）。该版本已经 ChatGPT 对 R1 结果提交 `5da9b17c1a720f89482eeda1436ad633145fe9fa` 独立正式复审 `APPROVED` 并经项目负责人明确“批准”，本文件 `acceptance_status` 收口为 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准内容基准提交 `5da9b17...`，2026-09-08）。验收标准获批不代表本轮调整已实现、已执行正式验收、正式验收通过或实现正式接受，不等于 `IMPLEMENTED_ACCEPTED`。
...
- 验收标准基线已批准（需求与验收基线批准收口任务 `DATA-SOURCE-SNAPSHOT-STATUS-REQUIREMENTS-BASELINE-APPROVAL-001`，2026-09-05），设计基线已批准（`DATA-SOURCE-SNAPSHOT-STATUS-DESIGN-BASELINE-APPROVAL-001`，2026-09-06），功能已由实现任务 `DATA-SOURCE-SNAPSHOT-STATUS-IMPLEMENTATION-001`/`-R1` 完成（既有实现存在）；第一轮 UI 调整版本已批准收口（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-001`，批准内容基准提交 `5757237...`，2026-09-07）并由实现任务 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-IMPLEMENTATION-001`（2026-09-07）按批准内容基准提交 `5757237...` 落地、实现 R1 提交 `5933ec2...` 经 ChatGPT 独立复审 `APPROVED`（上述第一轮事实保留为历史）。当前（第二轮）UI 调整版本（第二轮验收前 UI 调整草案 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-002` 及 R1 极小定向修订 `...-002-R1`）已批准收口：本文件 `acceptance_status` 当前第二轮调整版本为 `APPROVED`（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准内容基准提交 `5da9b17...`，2026-09-08）、实现状态 `IMPLEMENTED_ADJUSTMENT_PENDING`（第二轮调整已批准、尚未实现）、`formal_acceptance_execution_status=NOT_RUN`、`human_visual_acceptance_status=NOT_RUN`、`human_visual_review_status=CHANGES_REQUIRED`（第一轮页面人工检查历史）、`pending_user_review=NO`、`pending_user_confirmation_count=0`。正式执行验收须由后续独立正式验收任务在实现完成后执行，验收结果只能由正式验收任务写入；各轮验收标准获批不等于本轮调整已实现、代码复审通过或已执行正式验收。
- 依赖数据库只读比对或受控测试数据构造的用例，执行时必须遵守项目数据库只读/审批规则；测试数据 DML 仅在后续任务提示词显式纳入 `DSS-REQ-065` 授权时执行。
- 本文件不授权任何数据库写操作或测试数据写入；验收标准获批不等于执行验收、验收通过或实现正式接受；第一轮 UI 调整版本已批准并实现（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-001`，批准内容基准提交 `5757237...`，2026-09-07；历史）；当前（第二轮）UI 调整版本已由 ChatGPT 对 R1 结果提交 `5da9b17c1a720f89482eeda1436ad633145fe9fa` 独立正式复审 `APPROVED` 并经项目负责人明确批准，构成新批准基线（正式批准版本 `DATA-SOURCE-SNAPSHOT-STATUS-UI-ADJUSTMENT-BASELINE-APPROVAL-002`，批准内容基准提交 `5da9b17...`，2026-09-08），但批准的是本轮验收标准，不代表第二轮调整已实现、代码复审通过、正式验收已执行或通过，不等于 `IMPLEMENTED_ACCEPTED`。
