...
| data-subscription | 数据订阅 | 占位 | REQUIREMENTS、ACCEPTANCE、DESIGN、API、UI、DATABASE | APPROVED（需求与验收基线已获项目负责人正式批准；`APPROVED` 仅表示文档基线获批，不代表功能已实现或验收已通过；DESIGN/API/UI/DATABASE 为设计基线草案，状态 `DRAFT_PENDING_USER_REVIEW`，尚未正式复审批准） | docs/features/data-subscription/（Feature 文档目录标识 `data-subscription`；前端实现目录仍为 `views/data-subscribe/`，路由 `/config/subscribe`，与文档目录标识允许不同，均保持既有值不变）；设计草案 `DESIGN.md`/`API.md`/`UI.md`/`DATABASE.md` 及设计任务报告 `reports/DATA-SUBSCRIPTION-DESIGN-BASELINE-001.md`（依据前序批准任务结果提交 `2f18634...` 的已批准需求/验收基线） | 设计草案待正式复审批准；功能尚未实现（前端仍占位）；126 条验收尚未执行（`NOT_RUN`） | 设计复审 |
