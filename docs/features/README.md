...
| data-subscription | 数据订阅 | 占位 | REQUIREMENTS、ACCEPTANCE | DRAFT（`REQUIREMENTS.md`/`ACCEPTANCE.md` 基线草案 `DRAFT_PENDING_USER_REVIEW`，未批准、未实现） | docs/features/data-subscription/（Feature 文档目录标识 `data-subscription`；前端实现目录仍为 `views/data-subscribe/`，与文档目录标识允许不同）；frontend `views/data-subscribe/`（占位页） | 占位页未对接；缺 DESIGN/API/UI/DATABASE；需求与验收基线待用户复审批准 | 设计阶段 |
