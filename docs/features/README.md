...
| data-subscription | 数据订阅 | 占位 | REQUIREMENTS、ACCEPTANCE | APPROVED（需求与验收基线已获项目负责人正式批准；`APPROVED` 仅表示文档基线获批，不代表功能已实现或验收已通过） | docs/features/data-subscription/（Feature 文档目录标识 `data-subscription`；前端实现目录仍为 `views/data-subscribe/`，路由 `/config/subscribe`，与文档目录标识允许不同，均保持既有值不变）；批准收口报告 `reports/DATA-SUBSCRIPTION-REQUIREMENTS-BASELINE-APPROVAL-001.md`（依据 R1 结果提交 `b9fb1e9...` 的 ChatGPT 正式复审 `APPROVED`） | 缺 DESIGN/API/UI/DATABASE 设计基线（未建立）；功能尚未实现（前端仍占位）；126 条验收尚未执行（`NOT_RUN`） | 设计阶段 |
