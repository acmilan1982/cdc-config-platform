...
| server-config | 中心端配置 | 旧批准版本已实现；本次两项已批准调整尚未实现（`IMPLEMENTED_ADJUSTMENT_PENDING`） | REQUIREMENTS、ACCEPTANCE、DESIGN、API、UI、DATABASE、reports | APPROVED | docs/features/server-config/；设计批准报告 SERVER-CONFIG-DESIGN-BASELINE-APPROVAL-001 及完整设计链（DESIGN/API/UI/DATABASE，批准提交 `77a8c63...`）；验收前调整批准报告 SERVER-CONFIG-PRE-ACCEPTANCE-ADJUSTMENT-BASELINE-APPROVAL-001（批准提交 `b1c5349...`） | 两项已批准调整（`CONFIG_DESC` 人工换行安全显示、`ORDER BY ID_SERVER_CONFIG ASC`）代码尚未实现；正式验收 66 条全部 `NOT_RUN` | 两项已批准调整的小范围实现任务 |
