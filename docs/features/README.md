...
| data-subscription | 数据订阅 | 占位（`NOT_STARTED`） | REQUIREMENTS、ACCEPTANCE、DESIGN、API、UI、DATABASE | 需求/验收：上一正式批准版本提交 `d7560445...`（历史批准事实保留）；当前为“英文句点 `.` 保留分隔符需求/验收调整草案”，状态 `DRAFT_PENDING_USER_REVIEW`，待正式复审（文档基线状态不代表功能已实现或验收已通过）；DESIGN/API/UI/DATABASE 为设计基线草案，状态 `DRAFT_PENDING_USER_REVIEW`，设计复审结论 `CHANGES_REQUIRED`，等待需求调整复审后再执行设计 R1 | docs/features/data-subscription/（Feature 文档目录标识 `data-subscription`；前端实现目录仍为 `views/data-subscribe/`，路由 `/config/subscribe`，与文档目录标识允许不同，均保持既有值不变）；设计草案 `DESIGN.md`/`API.md`/`UI.md`/`DATABASE.md`、设计任务报告 `reports/DATA-SUBSCRIPTION-DESIGN-BASELINE-001.md` 及点号调整报告 `reports/DATA-SUBSCRIPTION-DOT-DELIMITER-REQUIREMENTS-ADJUSTMENT-001.md`（设计草案依据前序批准任务结果提交 `2f18634...` 的已批准需求/验收基线） | 点号需求调整待正式复审；设计 R1（修正设计复审 `CHANGES_REQUIRED` 发现项）待执行；功能尚未实现（前端仍占位）；126 条验收尚未执行（`NOT_RUN`） | 点号需求调整复审 |
