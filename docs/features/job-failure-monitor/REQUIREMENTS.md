...
> **状态说明**：本节已由 Commit `715b476a26d8ad12abc22cdd29e23c60fe9df7a1` 实现；`nextRetryAt` 绝对时间倒计时、60～1 秒动态倒计时、连接中文案"正在重新连接 ZooKeeper…"、请求防重、后台节流校正、手动刷新、生命周期清理、呼吸动画与 reduced-motion 均已完成技术核验；用户已查看真实页面并明确确认效果可以。本次收口不代表 §6.12 ZooKeeper 集群连接目标展示的新需求已经实现。
...
| GAP-OVERVIEW-001 | 概览页调整 | §6.3/§6.6/§6.7/§6.11/§6.12：查询区与六列表格、多数据源展开、客户端卡片视觉一致性、ZooKeeper 运行状态融合、ZK 错误态集群连接目标展示 | 功能主体（含多数据源展开与异常数据源展示）已实现并通过技术核验与人工页面验收；客户端卡片视觉一致性（§6.6）已由 Commit `4993400cebc145dd1fa69de1d1de8733e4568a2e` 实现并人工验收；ZooKeeper 运行状态融合（§6.7）主体已由 Commit `c3b06bf1dec950e3c4b9a8a6f5e7434616f66317` 实现并完成技术核验；§6.11 倒计时与图标动效已由 Commit `715b476a26d8ad12abc22cdd29e23c60fe9df7a1` 实现并人工验收 | 概览页整体收口仍需等待 §6.12 集群连接目标展示实现与人工验收，以及最终 README/基线收口；GAP 在 §6.12 实现与最终验收通过前保持开放 |
...
| GAP-OVERVIEW-ZK-STATUS-001 | ZooKeeper 运行状态融合 | §6.7：以 ZooKeeper `alive` 临时节点作为运行状态依据，与数据库故障事实组合展示，客户端头部显示在线/离线，Job 状态新增"离线"，ZK 失败隐藏旧数据并 60 秒自动重试 | 主体已由 Commit `c3b06bf1dec950e3c4b9a8a6f5e7434616f66317` 实现并完成技术核验：客户端在线/离线、Job 离线、ZK 失败隐藏卡片和 60 秒自动重试均已实现；§6.11 倒计时与图标动效已由 Commit `715b476a26d8ad12abc22cdd29e23c60fe9df7a1` 实现并人工验收；剩余 §6.12 集群连接目标展示 | 需求已批准，主体与倒计时均已实现；剩余 §6.12 集群连接目标展示及最终人工验收/基线收口，GAP 保持开放 |
...
| GAP-OVERVIEW-ZK-CONNECTION-TARGET-001 | ZooKeeper 集群连接目标展示 | §6.12：ZK 错误态显示运行时实际集群连接串和根路径，并支持复制和失败降级 | 当前错误态仅展示倒计时与图标动效（Commit `715b476a26d8ad12abc22cdd29e23c60fe9df7a1`），尚未展示连接目标；现有 `GET /api/monitor/zookeeper/health` 接口与前端 `fetchZkHealth()`/`ZooKeeperHealthResponse` 可复用 | 需求已批准（APPROVED/PENDING），前端实现和人工页面验收待完成，保持 OPEN |
