...
> **需求状态**：`baseline_status: APPROVED`（已批准），`implementation_status: IMPLEMENTED_ACCEPTED`（已实现并验收）。
> **状态说明**：状态融合主体已由 Commit `c3b06bf1dec950e3c4b9a8a6f5e7434616f66317` 实现并完成技术核验；客户端在线/离线、客户端离线短路 Job alive、Job 离线、ZK 失败整体错误态与隐藏旧卡片、复用只读 ZK 客户端等均已完成技术核验；后续倒计时、告警动效、连接目标诊断及 HTTP 复制兼容分别由 Commit `715b476a26d8ad12abc22cdd29e23c60fe9df7a1`、`8268e1b266b64187cc41a6aa1612e37c2f692ef3`、`533bbbc37bf2c9e1aa0939d20d6b6458c035a212` 完成（详见 §6.11、§6.12）；用户已完成最终页面人工评审和真实 HTTP 复制验证；§6.7 及其后续调整已形成完整、可用的当前实现，本节正式收口为 `IMPLEMENTED_ACCEPTED`。
> 在实现本节之前，故障监控概览页的客户端和 Job 状态仅来自数据库故障记录：数据库中没有未闭环故障，只能说明"没有正在处理的故障记录"，不能证明实际 `sync-client` 进程或 Job 正在运行，可能出现"进程实际已关闭、页面仍显示绿色正常"的误判。本节将"以 ZooKeeper `alive` 临时节点作为实际运行状态依据，并与数据库故障事实组合展示"正式写入基线，作为后续实现任务的唯一正式依据。
...
**验收状态**：`JFM-ACCEPT-064` ～ `JFM-ACCEPT-078` 均已通过。证据来自实现 Commit `c3b06bf1dec950e3c4b9a8a6f5e7434616f66317` 的技术核验及后续真实页面人工评审：路径拼接与单 `DATA_SOURCE_ID` jobCode 正确；客户端离线短路、客户端在线下的 Job 在线/离线、数据库状态回退均正确；ZK 读取异常不误判离线并进入统一错误态；错误态隐藏全部卡片和旧数据；客户端头部在线/离线、Job 状态筛选"离线"和客户端排序符合 §6.7；Job 离线仅覆盖状态列，数据库列、数据源显示与 Tooltip 不回归；复用只读 ZK 客户端且全程无 DB/ZK 写操作。真实页面曾核验 `hosp-002`、`hosp-0061` 离线、`hosp-012` 在线，离线卡片、离线 Job、在线 Job、筛选和排序表现符合需求。
...
> **需求状态**：`baseline_status: APPROVED`（已批准），`implementation_status: IMPLEMENTED_ACCEPTED`（已实现并完成人工验收）。
> **状态说明**：本节已由 Commit `715b476a26d8ad12abc22cdd29e23c60fe9df7a1` 实现；`nextRetryAt` 绝对时间倒计时、60～1 秒动态倒计时、连接中文案"正在重新连接 ZooKeeper…"、请求防重、后台节流校正、手动刷新、生命周期清理、呼吸动画与 reduced-motion 均已完成技术核验；用户已查看真实页面并明确确认效果可以。§6.12 ZooKeeper 集群连接目标展示已随后由 Commit `8268e1b266b64187cc41a6aa1612e37c2f692ef3` 实现，并由 Commit `533bbbc37bf2c9e1aa0939d20d6b6458c035a212` 完成真实 HTTP 复制兼容修正，详见 §6.12。
> 本节是对 §6.7 ZK 失败页面体验的增量修订，不重新定义 ZooKeeper 在线状态、路径、接口和后端读取规则。前置事实：§6.7 ZooKeeper 运行状态融合主体已由 Commit `c3b06bf1dec950e3c4b9a8a6f5e7434616f66317` 实现并完成技术核验；在 §6.11 实现前，ZK 无法连接时页面不显示任何客户端卡片和旧监控数据、显示错误图标和固定文案"ZooKeeper 连接失败，将在 60 秒重试"、60 秒后自动重试。现有错误状态过于静态，无法直观看出页面仍在工作，本节将"60 秒重试"改为实时倒计时，并为文案上方的感叹号增加适度闪烁提示。
...
| 编号 | 验收标准 |
|---|---|
| JFM-ACCEPT-121 | ZK 失败后立即显示 60 秒，并按真实时间递减至 1，文案格式正确 【✅ 已通过：首次进入实测 60→58 秒递减，文案格式为"ZooKeeper 连接失败，将在 {N} 秒后重试"（Commit `715b476` 技术核验 Phase A）】 |
| JFM-ACCEPT-122 | 到点只触发一次请求，请求期间显示"正在重新连接 ZooKeeper…"，不稳定停留在 0 秒 【✅ 已通过：60 秒自然到期仅触发 1 次请求，请求期实测显示"正在重新连接 ZooKeeper…"，无稳定"0 秒"状态（Phase B）】 |
| JFM-ACCEPT-123 | 重试失败后从 60 秒重新计时；重试成功后错误态、倒计时和旧定时器全部清除并恢复卡片 【✅ 已通过：重试失败后重新显示"将在 60 秒后重试"并递减；手动刷新成功后错误态清除、恢复 2 张客户端卡片（Phase B/C/E）】 |
| JFM-ACCEPT-124 | 浏览器后台停顿或定时器节流后倒计时能够按 `nextRetryAt` 校正，不出现明显漂移或集中重复请求 【✅ 已通过：Debugger 暂停 7 秒后恢复，倒计时按绝对时间校正为"正在重新连接 ZooKeeper…"并重新从 60 计时，期间仅 1 次请求（Phase F）】 |
| JFM-ACCEPT-125 | 多次错误、路由离开/返回和组件卸载均不产生重复重试器、倒计时器或残留请求 【✅ 已通过：多轮错误每轮仅 1 次重试请求；路由离开期间 0 次请求、返回后重新进入错误态仅 1 次请求（Phase B/H）】 |
| JFM-ACCEPT-126 | 手动刷新立即请求；失败后重置为新的 60 秒倒计时，成功后恢复页面 【✅ 已通过：手动刷新立即发出 1 次请求；连点刷新仅 1 次请求；失败后从 60 重新计时，按钮 loading 正确结束；成功后恢复页面（Phase E）】 |
| JFM-ACCEPT-127 | 仅感叹号柔和呼吸，文字稳定；`prefers-reduced-motion` 下动画关闭且信息仍完整 【✅ 已通过：仅 `.zk-error-icon` 呼吸动画（1.4s、透明度 1↔0.35、轻微缩放），文字/容器无动画；reduced-motion 下动画关闭、图标与文案仍可见（Phase G）】 |
| JFM-ACCEPT-128 | ZK 失败期间无客户端卡片和旧数据，60 分钟页面自动刷新与 60 秒故障重试互不混淆 【✅ 已通过：错误态下 0 张卡片、无旧数据；页面"1 分钟"自动刷新每周期仅 1 次请求，与 60 秒故障重试互不混淆（Phase A/D）】 |
| JFM-ACCEPT-129 | 前端构建、浏览器控制台和既有在线/离线、筛选排序、手动刷新功能均无回归，DB/ZK 全程只读 【✅ 已通过：`npm run build` 通过；成功态回归 2 张卡片、在线/离线、筛选/重置/折叠均正常，无页面横向滚动，控制台 0 错误；DB/ZK 全程只读（Phase I）】 |
...
### 6.12 ZooKeeper 集群连接目标展示（本轮已确认 · 目标需求）

> **需求状态**：`baseline_status: APPROVED`（已批准），`implementation_status: IMPLEMENTED_ACCEPTED`（已实现并验收）。
> **状态说明**：本节由初始实现 Commit `8268e1b266b64187cc41a6aa1612e37c2f692ef3` 实现，并由 Commit `533bbbc37bf2c9e1aa0939d20d6b6458c035a212` 完成真实 HTTP 非安全上下文复制兼容修正；复用 `GET /api/monitor/zookeeper/health` 获取完整 connectString/rootPath、单错误生命周期单次请求与防重缓存、Summary 错误态优先、加载失败/空值降级、长文本安全换行、复制按钮及成功/失败反馈、诊断区中性克制动效边界与响应式均已完成技术核验和真实 HTTP 浏览器验证；用户已在真实局域网 HTTP 部署地址尝试复制并确认"可以正常复制了"，不再出现原"复制失败"缺陷；逐字符内容正确性以技术核验中对完整原始值的捕获证据为依据。
> 本节只定义 ZooKeeper 错误态下的诊断信息展示，不改变在线/离线判定、Summary API、60 秒重试机制和 CDC 节点状态页面。当 ZK 连接失败时，页面在动态倒计时下方展示应用后端实际生效的完整 ZooKeeper 集群连接串和根路径，便于运维人员直接判断集群地址、端口或根路径是否配置错误，并区分"应用无法连接配置的 ZK 集群"与单台机器故障。页面语义必须表述为"当前应用进程无法通过配置的集群连接串获得可用的 ZooKeeper 连接"，不得表述为"三台 ZK 均宕机"或对某一台服务器下结论。

#### 6.12.1 范围与非范围

| 编号 | 规则 | 说明 |
|---|---|---|
| JFM-ADJ-238 | ZK 连接失败错误态在动态倒计时下方增加诊断信息区，展示实际生效的完整集群连接串和根路径 | — |
| JFM-ADJ-239 | 用户可见标签固定为"集群地址"和"根路径"；不得使用"当前连接节点"，因为失败状态下不存在可确认的当前连接节点 | — |
| JFM-ADJ-240 | "集群地址"必须完整展示后端运行时有效 `connectString`，单机显示单地址，集群显示逗号分隔的全部地址，不得只选其中一台 | — |
| JFM-ADJ-241 | "根路径"必须展示后端运行时有效 `rootPath`，例如 `/bsoft-cdc` | — |
| JFM-ADJ-242 | 配置数据必须来自现有 `GET /api/monitor/zookeeper/health` 的结构化字段，不得前端硬编码，不得从错误消息中解析 | — |
| JFM-ADJ-243 | 复用现有 `fetchZkHealth()`、`ZooKeeperHealthResponse` 和既有后端健康接口；不得为本需求新增 API 或修改全局错误响应 | — |
| JFM-ADJ-244 | 健康接口的 `connected` 只作为返回信息，不得反向覆盖 Summary API 的 ZK 错误态；故障监控是否显示错误仍以 Summary 结果为准 | — |
| JFM-ADJ-245 | 只展示 `connectString` 与 `rootPath`；不得展示健康接口的内部 `errorMessage`、认证信息、ACL、Digest、密码或其他敏感配置 | — |
| JFM-ADJ-246 | 首次从正常/初始状态进入 ZK 错误态时获取一次诊断配置并缓存；倒计时每秒更新和每轮 60 秒重试不得反复请求健康接口 | — |
| JFM-ADJ-247 | 在同一次组件生命周期内，已有有效诊断配置时可以复用；页面重新进入后按新生命周期重新获取，以反映后端重启后的有效配置 | — |
| JFM-ADJ-248 | 健康接口请求必须有单请求防重；其失败不能中断倒计时、自动重试、手动刷新或错误态主体展示，也不得产生独立无限重试 | — |
| JFM-ADJ-249 | 健康信息加载期间显示克制的"正在获取连接配置…"或骨架状态；获取失败显示"连接配置信息获取失败"，不得伪造地址 | — |
| JFM-ADJ-250 | `connectString` 或 `rootPath` 为 null、空串或纯空白时，对应字段显示"未配置"，不得显示空白或字符串 `null/undefined` | — |
| JFM-ADJ-251 | 集群地址和根路径分别提供复制按钮，复制完整原始值；无有效值时复制按钮禁用或隐藏，并提供成功/失败反馈 | — |
| JFM-ADJ-252 | 诊断区使用中性灰、较小字号和等宽值文本，位于红色主错误文案下方；不得与主错误抢视觉焦点，不得参与感叹号动画 | — |
| JFM-ADJ-253 | 长集群连接串必须在常用桌面与窄宽下安全换行或滚动，不截断实际值，不引发页面横向滚动；复制仍为完整值 | — |
| JFM-ADJ-254 | 本轮保持只读：不修改 ZK 配置，不探测或断言单台节点宕机，不写 DB/ZK，不改变 60 秒倒计时、隐藏旧卡片及其他监控行为 | — |
...
```text
              [呼吸感叹号]
ZooKeeper 连接失败，将在 47 秒后重试

集群地址  10.19.16.111:2181, 10.19.16.112:2181, 10.19.16.113:2181  [复制]
根路径    /bsoft-cdc                                                   [复制]
```
...
- 主错误文案继续使用现有红色；诊断信息使用中性灰色，不整块使用红色；
- 标签与值有清晰对齐；值建议使用等宽字体；
- 诊断区可以使用极浅灰背景、细边框和小圆角，但不得做成新的大卡片；
- 集群连接串保持后端原始完整值，不擅自排序、去重或改写；
- 页面不展示"3 台均宕机"等推断性结论。
...
```text
GET /api/monitor/zookeeper/health
```
...
- 后端来源为运行时 `ZooKeeperConfig.getConnectString()` 与 `getRootPath()`；
- 现有前端类型 `ZooKeeperHealthResponse` 当前仍声明为非空 `string`，实现阶段可以只在消费端做运行时空值保护；本轮需求不要求修改类型文件；
- 不得为了本需求改变后端字段语义。
...
1. Summary 首次返回 ZK 失败；
2. 页面立即进入倒计时错误态并隐藏旧数据；
3. 异步请求一次 ZK health 获取诊断配置；
4. health 请求不阻塞倒计时启动；
5. 取得配置后补充展示；
6. 后续 60 秒 Summary 重试不重复请求 health；
7. 离开并重新进入页面后允许重新获取一次。

#### 6.12.5 失败降级与空值规则

如果 health 接口请求失败：

- 主错误态继续正常展示；60 秒倒计时继续；自动/手动 Summary 重试继续；
- 诊断区显示"连接配置信息获取失败"；
- 不使用代码默认值、环境猜测值或上一次其他页面的数据兜底；
- 不针对 health 请求建立循环重试。

如果 health 返回 `connected=true`，但本次 Summary 返回 ZK 错误：

- 仍保持当前故障监控错误态和倒计时；
- 只读取 `connectString`/`rootPath`；
- 不依据 `health.connected` 恢复卡片；
- 后续 Summary 成功才恢复页面。

`connectString` 或 `rootPath` 为 null、空串或纯空白时，对应字段显示"未配置"。

#### 6.12.6 复制行为

- 两个字段分别复制；
- 复制内容是完整原始字符串，不包含"集群地址：""根路径："标签；不复制截断文本；
- 成功提示建议为"集群地址已复制"或"根路径已复制"；复制失败显示明确失败提示；
- 优先复用项目既有复制实现或浏览器 Clipboard API；
- 不因复制失败影响倒计时和重试。
...
| 编号 | 验收标准 |
|---|---|
| JFM-ACCEPT-130 | Summary 进入 ZK 错误态后，动态倒计时下方展示 health 接口返回的完整 connectString 与 rootPath |
| JFM-ACCEPT-131 | 单机连接串和三节点逗号连接串均完整显示；标签为"集群地址""根路径"，不显示"当前连接节点"或节点宕机推断 |
| JFM-ACCEPT-132 | 浏览器网络证据确认诊断值来自 `/api/monitor/zookeeper/health`，无前端硬编码、无错误文案解析、无新增后端 API |
| JFM-ACCEPT-133 | 一次错误生命周期只请求一次 health；倒计时更新及多轮 Summary 重试不重复请求；离开并重新进入后可重新请求一次 |
| JFM-ACCEPT-134 | health 加载、失败、空 connectString、空 rootPath 均有正确降级，且不影响 60 秒倒计时、自动重试和手动刷新 |
| JFM-ACCEPT-135 | health.connected 与 Summary 结果不一致时仍以 Summary 控制错误态，后续 Summary 成功才恢复卡片 |
| JFM-ACCEPT-136 | 两个复制按钮分别复制完整原始值，成功/失败反馈正确，无值时不可复制 |
| JFM-ACCEPT-137 | 诊断区视觉克制、文字不参与呼吸动画；长集群地址可完整查看，窄宽和桌面宽度均无页面横向滚动 |
| JFM-ACCEPT-138 | 既有倒计时、连接中文案、呼吸/reduced-motion、无旧卡片、在线离线、筛选排序等均不回归，前端构建通过，DB/ZK 只读 |

**验收状态**：`JFM-ACCEPT-130` ～ `JFM-ACCEPT-138` 均已通过。
- 技术核验（实现 Commit `8268e1b266b64187cc41a6aa1612e37c2f692ef3`）：完整 connectString/rootPath 展示与"集群地址""根路径"标签；来源确认为 `/api/monitor/zookeeper/health`（网络证据，无前端硬编码、无错误文案解析、无新增后端 API）；单错误生命周期单次 health 请求且倒计时更新与多轮 Summary 重试不重复请求；health 加载失败、空 connectString、空 rootPath 均有正确降级且不影响倒计时、自动重试与手动刷新；`health.connected` 与 Summary 不一致时仍以 Summary 控制错误态；两个复制按钮复制完整原始值、成功/失败反馈正确、无值时不可复制；诊断区视觉克制、文字不参与呼吸动画；长集群地址可完整查看且窄宽/桌面宽度均无横向滚动；前端构建通过、浏览器控制台无本任务引入错误。
- 人工验收（真实 HTTP 复制兼容修正 Commit `533bbbc37bf2c9e1aa0939d20d6b6458c035a212`）：用户已在真实局域网 HTTP 部署地址尝试复制两个按钮并确认可以正常复制，不再出现原"复制失败"。该人工反馈证明实际部署环境复制功能已恢复可用；逐字符内容正确性以技术核验中对完整原始值的捕获证据为依据，未把用户反馈改写为用户人工逐字符比对。
...
| 编号 | 规则 | 说明 |
|---|---|---|
| JFM-ADJ-284 | 概览行"查看历史"应为可导航入口，携带精确 `clientId` 与单个 `dataSourceId`，不得携带逗号拼接原字段 | 默认导航目标标签页由 JFM-ADJ-313～317 修订为新标签页；真实链接、精确 ID 与直接访问规则继续有效 |
...
| JFM-ADJ-291 | 历史过程按首次失败时间降序排列；时间相同使用精确 `faultRootIdText` 或等价稳定字段作次级排序，保证刷新与翻页顺序稳定 | — 【已被本轮（JOB-FAILURE-HISTORY-NO-PAGINATION-REQ-001）修订：首次失败时间降序与精确次级排序继续有效，由 JFM-ADJ-337 承接完整列表排序；"翻页顺序稳定"随分页取消，改为"刷新与切换范围后顺序稳定"】 |
| JFM-ADJ-292 | 历史列表采用后端分页，默认每页20条，并提供项目现有风格允许的页大小选择（建议20/50/100）；切换时间范围回到第1页，空结果显示明确空状态 | — 【已被本轮替代：独立历史列表改为前后端完全不分页，取消后端分页、默认20条与20/50/100选择，所选范围内全部故障过程一次性返回展示（JFM-ADJ-332～336、JFM-ACCEPT-167～171）】 |
...
| JFM-ADJ-295 | 同一组件内 clientId、dataSourceId、时间范围或页码变化时必须重新加载与当前 URL 一致的数据，不得显示上一个参数的残留结果 | — 【已被本轮修订：页码变化部分随分页取消不再适用；clientId、dataSourceId、时间范围变化时重新加载与当前 URL 一致数据的规则继续有效（依据 JFM-ADJ-335、JFM-ADJ-339）】 |
| JFM-ADJ-296 | 列表请求需要请求序号、取消或等价一致性保护；较早请求晚返回时不得覆盖新筛选、新页码或新路由结果 | — 【已被本轮修订：请求序号/取消/一致性保护继续有效，切换时间范围时仍须防止旧范围请求晚返回覆盖新结果；"新页码"维度随分页取消不再适用（依据 JFM-ADJ-339）】 |
...
| JFM-ADJ-302 | 后端提供单个当前配置数据源的分页历史过程查询能力：接收 clientId、单个 dataSourceId、固定范围枚举、page/pageSize，返回总数和 JFM-ADJ-289 所需字段；优先复用已有过程摘要构建逻辑，避免状态口径分叉 | — 【已被本轮替代：分页 List 查询契约由完整 List 契约替代（JFM-ADJ-333/334），取消 page/pageSize 与总数返回；过程摘要构建、字段复用与状态口径复用继续有效（依据 JFM-ADJ-339）】 |
...
| JFM-HIST-002 | 历史列表不显示传统分页组件 | 已被 §6.13 替代并经本轮（JOB-FAILURE-HISTORY-NO-PAGINATION-REQ-001）修正：独立历史列表改为前后端完全不分页，不显示任何分页组件（JFM-ADJ-335、JFM-ACCEPT-167）；JFM-ADJ-292 的分页替代关系已被 JFM-ADJ-332～336 替代 |
| JFM-HIST-003 | 所选时间范围内全部记录必须可见，不得因 pageSize 限制静默截断 | 已被 §6.13 替代并经本轮修正：独立历史列表一次性返回并展示所选自然日范围内全部故障过程（JFM-ADJ-332、JFM-ACCEPT-169），不依赖分页与总数翻页，不再固定 pageSize=1000 |
...
| JFM-API-003 | API-3 `/history/{clientId}/{dataSourceId}` 接受任意 startTime/endTime | 已被 §6.13 修订并经本轮修正：新页面使用固定 `range` 枚举的 API-7（JFM-ADJ-288、JFM-ADJ-302；API-7 分页契约已由本轮 JFM-ADJ-333/334 修订为完整 List）；API-3 仅保留至旧详情页"历史故障过程"区块移除 |
| JFM-PERF-003 | 故障历史按时间范围查询，不设 pageSize 硬编码上限 | 已被 §6.13 修订并经本轮修正：独立列表改为前后端完全不分页，取消默认20条与最大100条等任何条数上限（JFM-ADJ-336）；"避免静默截断"由所选范围内全部过程完整返回承接 |
| JFM-ADJ-291（分页相关部分） | 历史列表时间相同次级排序保证"刷新与翻页顺序稳定" | 已被本轮修订：排序继续有效，由 JFM-ADJ-337 承接完整列表排序；"翻页"表述随分页取消不再适用 |
| JFM-ADJ-292 | 历史列表采用后端分页，默认每页20条并提供20/50/100选择，切换范围回到第1页 | 已被本轮替代：改为前后端完全不分页，一次性返回并展示所选范围内全部故障过程（JFM-ADJ-332～336） |
| JFM-ADJ-295（页码变化部分） | 页码变化时重新加载数据 | 已被本轮修订：页码维度随分页取消不再适用；clientId/dataSourceId/range 变化重载继续有效（JFM-ADJ-335） |
| JFM-ADJ-296（页码维度） | 旧请求不得覆盖新页码结果 | 已被本轮修订：请求一致性继续有效，不再包含页码维度（JFM-ADJ-339） |
| JFM-ADJ-302（分页部分） | 分页 List 查询：clientId/dataSourceId/range/page/pageSize，返回总数 | 已被本轮替代：List 契约改为完整返回（JFM-ADJ-333/334），取消 page/pageSize 与总数；过程摘要与状态复用继续有效 |
| JFM-ACCEPT-147（分页部分） | 历史列表支持后端分页、切换范围回第1页 | 已被本轮替代：分页验收由 JFM-ACCEPT-167～171 承接；7列、范围、倒序、空状态继续有效 |
...
| JFM-ACCEPT-147 | 历史列表支持今日/近7天/近30天、后端分页、7列顺序、首次失败时间倒序、空状态和切换范围回第1页 | API + 浏览器 【已被本轮替代：后端分页与切换范围回第1页由 JFM-ACCEPT-167～171 替代，列表改为完整返回与完整展示；今日/近7天/近30天、7列顺序、首次失败时间倒序与空状态继续有效】 |
...
| JFM-ACCEPT-155 | 首次进入和组件重新挂载时，所有客户端卡片均展开；0故障、停用或无效数据源所在卡片同样展开 | 浏览器实页 + DOM |
| JFM-ACCEPT-156 | 手动折叠/展开正常；自动刷新和顶部手动刷新保留仍存在 clientId 的人工状态，新出现客户端默认展开、消失客户端状态被清理 | 前端定向测试或浏览器交互 |
| JFM-ACCEPT-157 | 点击查询或重置后，当前结果全部展开；筛选、排序和刷新后展开状态按 clientId 正确对应，不因数组位置变化串位 | 浏览器交互 + 代码审查 |
| JFM-ACCEPT-158 | 本轮为纯前端最小修正，历史统计 API、自然日口径、排序、分页、数据源显示、详情链接和 DB-only 行为无回归，前端构建与控制台核验通过 | diff + build + 浏览器回归 【已被本轮修订：原"分页"回归项随分页取消，由 JFM-ACCEPT-171（取消分页后各口径不回归）承接；其余继续有效】 |
| JFM-ACCEPT-159 | 概览每行"查看历史"为真实 `href`，普通点击默认新标签页，原概览页筛选、展开状态和滚动位置保持 | DOM + 浏览器人工操作 |
| JFM-ACCEPT-160 | 新标签页 URL 使用完整 clientId/dataSourceId，可复制、刷新和直接访问；无 `window.open()`；历史列表到过程详情仍在当前标签页且后退正常 | URL/DOM/代码审查 + 浏览器 |
| JFM-ACCEPT-161 | 未筛选时顶部显示类似"同步平台共 3 个客户端，7 个数据源"；选择客户端并查询后文案仍保持平台总量不变 | 真实页面筛选前后对比 |
| JFM-ACCEPT-162 | 客户端和数据源总量符合当前配置全集、逗号拆分和不去重口径；零故障、未激活、ORG 空、无效数据源计入，配置外数据不计入 | API/代码审查 + 可控数据测试 |
| JFM-ACCEPT-163 | 数据源历史列表顶部显示"客户端 <ID>｜数据源 <ORG/规则化名称>"，五种显示组合正确，悬停显示完整"数据源 ID：<值>" | 浏览器 + Tooltip + 单测/Mock |
...
#### 6.13.13 客户端卡片默认展开与状态保持（本轮人工页面评审补充）

**背景（人工页面评审）**：Commit `744980f45060a30d507c75fddf06aa164a042167` 已实现 §6.13 主体。人工打开 `/monitor/job-failure/history` 发现所有客户端卡片首次加载后默认处于折叠状态，页面只显示客户端 ID 与箭头，隐藏了页面最核心的今日/近7天/近30天统计。用户确认：该页面重点是查询和比较历史故障统计，不应根据数据源当前正常、异常、在线或离线决定展开状态；客户端最多约10个、每个客户端约5个数据源，默认全部展开不会造成不可接受的信息量；无论当前数据源有无故障、三个统计窗口是否全部为0，所有客户端卡片首次进入时都应展开；保留用户手动折叠能力。

| 编号 | 规则 | 说明 |
|---|---|---|
| JFM-ADJ-306 | 首次进入或组件重新挂载"故障历史"概览时，当前显示的所有客户端卡片必须默认展开，用户无需逐一点击后才能看到历史统计 | 补充 JFM-ADJ-265/266 |
| JFM-ADJ-307 | 默认展开与故障次数、最近处理结果、数据源启用状态及任何实时状态无关；即使今日/近7天/近30天均为0，也必须默认展开 | 补充 JFM-ADJ-267 |
| JFM-ADJ-308 | 保留每张客户端卡片的手动展开/折叠箭头；用户可主动收起不需要查看的客户端，本轮不取消折叠能力 | 补充 JFM-ADJ-266 |
| JFM-ADJ-309 | 用户执行显式"查询"或"重置"后，当前查询结果中的所有客户端卡片重新默认展开，使新的查询结果立即可见 | 补充 JFM-ADJ-268 |
| JFM-ADJ-310 | 顶部手动"刷新"和页面自动刷新不得强制重置用户已经手动选择的展开/折叠状态；仍存在的同一 clientId 应保持刷新前状态 | — |
| JFM-ADJ-311 | 刷新后新出现、此前没有展开状态记录的客户端必须默认展开；已经从结果中消失的 clientId 状态应清理，不得无限保留陈旧状态 | — |
| JFM-ADJ-312 | 展开状态必须以稳定 clientId 为键维护，不得依赖数组下标、排序位置、故障次数或 ZK 状态；筛选、排序和刷新后不得把一个客户端的折叠状态错误应用到另一个客户端 | 补充 JFM-ADJ-283 |

**交互状态矩阵**：

| 场景 | 目标行为 |
|---|---|
| 首次进入故障历史概览 | 所有客户端卡片展开 |
| 从其他路由返回且组件重新挂载 | 所有客户端卡片展开 |
| 今日/7天/30天次数均为0 | 仍默认展开 |
| 数据源停用或记录不存在 | 仍默认展开，数据源显示规则不变 |
| 用户手动折叠某客户端 | 该客户端立即折叠 |
| 自动刷新 | 保留仍存在 clientId 的人工展开/折叠状态 |
| 点击顶部"刷新" | 保留仍存在 clientId 的人工展开/折叠状态 |
| 点击"查询" | 当前查询结果全部展开 |
| 点击"重置" | 重置后的全部结果展开 |
| 刷新后出现新客户端 | 新客户端展开 |
| 客户端从结果中消失 | 清理该 clientId 的陈旧状态 |

**既有规则修订关系**（保留原文，仅补充默认展开与状态保持语义，不改变原规则）：

- `JFM-ADJ-265`：全宽卡片视觉继续有效，并由 JFM-ADJ-306～312 补充默认展开与状态保持；
- `JFM-ADJ-266`：头部客户端 ID 与箭头继续有效，箭头仍支持人工折叠；
- `JFM-ADJ-267`：无在线/离线状态继续有效，展开逻辑不得依赖实时状态；
- `JFM-ADJ-268`：查询/重置继续有效，并由 JFM-ADJ-309 明确执行后的展开行为；
- `JFM-ADJ-283`：稳定排序继续有效，并由 JFM-ADJ-312 明确折叠状态按 clientId 绑定；
- 其他 §6.13 统计、数据源、路由和详情规则保持不变。【已被本轮（JOB-FAILURE-HISTORY-NO-PAGINATION-REQ-001）修订：其中"分页"规则由 JFM-ADJ-332～340 替代，见 §6.13.15；其余统计、数据源、路由和详情规则继续保持不变】

**语义边界**："默认全部展开"不是"永久强制展开"。用户手动折叠仍然有效；系统只在首次展示、组件重新挂载、新查询/重置结果或新 clientId 出现时应用默认展开。不得创建与上述规则冲突的"异常优先展开""正常默认折叠""0故障自动折叠"等规则。
...
| 编号 | 规则 | 说明 |
|---|---|---|
| JFM-ADJ-313 | 故障历史概览中每个数据源行的"查看历史"必须使用真实可访问链接，默认在新的浏览器标签页打开对应数据源历史列表 | 修订 JFM-ADJ-284 的默认目标标签页 |
| JFM-ADJ-314 | 链接使用前端路由 `/monitor/job-failure/history/list?clientId=<clientId>&dataSourceId=<dataSourceId>`，参数继续使用完整原始字符串，不得把名称代替 ID，不得经 JavaScript number 转换 | 补充 JFM-ADJ-285/286 |
| JFM-ADJ-315 | 链接应采用声明式真实 `href`/RouterLink 与 `target="_blank"`，同时设置 `rel="noopener noreferrer"`；不得以 `window.open()` 或普通按钮伪装链接 | 补充 JFM-ADJ-284 |
| JFM-ADJ-316 | 打开新标签页后，原故障历史概览页必须保持当前客户端筛选、卡片展开/折叠状态和滚动位置，不得因打开历史列表而重置或重新加载 | 补充 JFM-ADJ-310/312 |
| JFM-ADJ-317 | 本轮仅修改"故障历史概览 → 单数据源历史列表"的导航；历史列表中的"查看详情"继续在当前标签页进入单次故障过程详情，并保留现有浏览器后退、直接访问和精确 `faultRootIdText` 行为 | 补充 JFM-ADJ-293 |
| JFM-ADJ-318 | 故障历史概览标题旁的数量文案统一为"同步平台共 X 个客户端，Y 个数据源"，例如"同步平台共 3 个客户端，7 个数据源" | 替代原"共 N 个数据源"实现文案 |
| JFM-ADJ-319 | X/Y 表示同步平台当前配置范围的整体规模，不是当前客户端筛选后的结果数量；选择客户端、点击查询或查看子集时，顶部 X/Y 不得变化 | 补充 JFM-ADJ-283 |
| JFM-ADJ-320 | 客户端数 X 取当前故障历史配置全集中实际纳入页面范围的客户端数量，与 §6.13 当前配置范围、`CDC_CLIENT_MULTIPLE.FG_ACTIVE='1'` 规则保持一致 | 补充 JFM-ADJ-259 |
| JFM-ADJ-321 | 数据源数 Y 按每个纳入范围客户端的 `CDC_CLIENT_MULTIPLE.DATA_SOURCE_ID` 执行英文逗号拆分、Trim、忽略空项后逐项计数；不按 `DATA_SOURCE_ORG`、名称或 ID 去重 | 补充 JFM-ADJ-260 |
| JFM-ADJ-322 | 当前配置中的零故障、`CDC_DATA_SOURCE.FG_ACTIVE=0`、`DATA_SOURCE_ORG` 为空或 `CDC_DATA_SOURCE` 记录不存在的数据源均计入 Y；已经从当前客户端配置删除或客户端本身不在纳入范围的数据源不计入 | 补充 JFM-ADJ-261/262 |
| JFM-ADJ-323 | 平台配置发生变化并重新加载完整配置范围后 X/Y 应更新；客户端筛选仅改变卡片展示结果，不得覆盖或污染平台总量；不得用筛选结果反推平台总量 | — |
| JFM-ADJ-324 | 数据源历史列表顶部上下文统一显示为"客户端 <CLIENT_ID>｜数据源 <可读名称>"，必须同时保留"客户端"和"数据源"两个字段标签 | 修订原 dataSourceId 无标签主文本展示 |
| JFM-ADJ-325 | 数据源主文本使用 `CDC_DATA_SOURCE.DATA_SOURCE_ORG`，不得继续直接把 `DATA_SOURCE_ID` 作为无标签主文本展示 | 修订 JFM-ADJ-286 的识别区主文本 |
| JFM-ADJ-326 | 数据源可读名称必须复用故障监控/故障历史概览的五种显示组合：存在且激活且 ORG 非空显示原值；存在且激活且 ORG 空显示"未定义名称"；存在且未激活且 ORG 非空显示 ORG + 红字"(数据源未激活)"；存在且未激活且 ORG 空显示"未定义名称" + 红字后缀；记录不存在显示整体红字"无效数据源" | 复用 §6.5 五种显示组合，扩展到历史列表顶部 |
| JFM-ADJ-327 | 鼠标悬停数据源可读名称区域时必须显示 Tooltip"数据源 ID：<完整 DATA_SOURCE_ID>"；Tooltip 和后续请求继续使用完整原始 ID，不得复制缩略值 | 补充 JFM-ADJ-286 |
| JFM-ADJ-328 | `dataSourceOrg`、`dataSourceExists`、`dataSourceActive` 必须来自后端可信的当前配置/数据源查询结果；不得仅把 ORG 作为路由 query 或前端路由 state 传递后直接信任，确保复制 URL、刷新和直接访问时仍能得到正确名称 | — |
| JFM-ADJ-329 | 优先复用现有 History Summary/API 中已经返回的 `dataSourceOrg`、`dataSourceExists`、`dataSourceActive`，按 `clientId + dataSourceId` 精确匹配；如现有可信接口足以满足，不得新增重复接口或前端逐行补查。列表无历史记录时仍必须能够显示数据源上下文 | 无 N+1，详见 §14 |
| JFM-ADJ-330 | 数据源历史列表的7列表格必须占满内容容器可用宽度，所有列共同铺满整行，"操作"列右边不得保留当前截图中的大块无意义空白占位区 | 修订原固定列宽总和不足的布局 |
| JFM-ADJ-331 | 列宽应使用合理的自适应宽度/`min-width` 组合，让时间、处理结果等主信息列吸收剩余空间；"操作"列保持紧凑并位于表格最右侧。常用桌面宽度不得产生页面级横向滚动，窄宽按现有响应式或表格内部安全滚动处理，不得截断核心内容 | 补充 JFM-ADJ-289 |

**统计口径**：

- X 与 Y 均针对当前纳入故障历史配置范围的未筛选全集（`CDC_CLIENT_MULTIPLE.FG_ACTIVE='1'`）；
- Y 按每个纳入客户端 `DATA_SOURCE_ID` 英文逗号拆分、Trim、忽略空项后逐项计数，不去重；
- 零故障、`FG_ACTIVE=0`、`DATA_SOURCE_ORG` 为空、`CDC_DATA_SOURCE` 记录不存在均计入 Y；
- 已从当前客户端配置删除、客户端不在纳入范围的数据源不计入；
- 客户端筛选、查询或查看子集不得改变顶部 X/Y；平台配置变化并重新加载完整配置范围后 X/Y 才更新；
- 不得以筛选子集反推平台总量，不得把数字前端硬编码。

**既有规则修订关系**（保留原文，仅修订/补充上述体验语义，不改变原规则）：

- `JFM-ADJ-284`：真实链接与精确 `clientId`/`dataSourceId` 继续有效，默认目标标签页由 JFM-ADJ-313～317 修订为新标签页；
- `JFM-ADJ-285`：独立历史列表路由继续有效，导航目标由概览新标签页打开；
- `JFM-ADJ-286`：`clientId`/`dataSourceId` 按字符串处理与正确 URL 编码继续有效；识别区主文本由 JFM-ADJ-324～327 修订为可读名称；
- `JFM-ADJ-289`：7列顺序与字段继续有效，仅由 JFM-ADJ-330～331 修订宽度布局；
- `JFM-ADJ-293`：历史列表到过程详情真实链接与 `faultRootIdText` 继续有效，仍为当前标签页线性钻取；
- `JFM-ADJ-306～312`：默认展开与状态保持规则继续有效，不得因本轮导航或总量调整回退；
- `JFM-ADJ-259～262`：当前配置全集、启用客户端、逗号拆分与0故障展示规则继续有效，并为 JFM-ADJ-318～323 提供口径依据；
- §6.10 指定故障过程独立路由继续有效；历史列表到过程详情仍为当前标签页；
- 五种显示组合与 ID Tooltip 规则继续有效，并由 JFM-ADJ-326～327 扩展到数据源历史列表顶部；
- DB-only 边界继续有效，本轮不得引入 ZooKeeper。

**禁止项**：

- "查看历史"仍强制当前标签页；
- 用 `window.open()` 代替真实链接；
- 顶部总量随筛选变化；
- 只统计有故障或激活的数据源；
- 直接信任 URL 中传入的 ORG；
- 继续把 `DATA_SOURCE_ID` 当作无标签主显示值；
- 通过新增空白列填满表格。

#### 6.13.15 历史列表前后端完全不分页（本轮需求增量修订）

**背景（用户最终确认）**：当前数据源故障历史列表（Commit `0bcefdcf75dabae0f1f9d293b070c9b5839dfe91`）仍是前后端分页实现（后端接收 page/pageSize、返回 PageResult；前端展示分页控件并维护页码状态）。用户明确要求改为前后端完全不分页：选择"今日/近7天/近30天"中的任一范围后，后端一次性返回该范围内该客户端、该数据源的全部故障过程，前端在同一张表格中一次性展示全部结果；不设置默认20条、最大100条或其他条数上限；不使用 ROWNUM、FETCH FIRST、LIMIT、subList、懒加载、无限滚动、分批加载或虚拟分页变相截断；数据较多时通过页面正常纵向滚动浏览。该规则只针对独立"数据源故障历史列表"；"故障历史"客户端卡片概览本来就不分页；故障过程详情页继续不恢复已移除的"历史故障过程"区块；当前故障监控、ZK 状态融合及其他页面不受影响。

| 编号 | 最终目标规则 | 说明 |
|---|---|---|
| JFM-ADJ-332 | 单个数据源故障历史列表采用前后端完全不分页模式；选择今日、近7天或近30天后，后端一次性返回该范围内全部故障过程，前端在同一张表格中一次性展示全部结果 | 替代 JFM-ADJ-292 的分页模式 |
| JFM-ADJ-333 | History List API 参数只包含 clientId、单个 dataSourceId 和固定 range；不得接收 page、pageSize、cursor、offset、limit 或其他分页/分批参数 | 替代 JFM-ADJ-302 的分页参数 |
| JFM-ADJ-334 | History List API 返回完整历史过程数组；不得返回 PageResult、IPage、total/pages/current/size 等分页对象或元数据 | 替代 JFM-ADJ-302 的 PageResult 响应 |
| JFM-ADJ-335 | 前端不得展示分页器、每页条数、跳页、上一页/下一页等控件，也不得维护任何分页状态；切换范围时直接重新加载该范围完整列表 | 修订 JFM-ADJ-295/296 的页码维度 |
| JFM-ADJ-336 | 不得设置默认20条、最大100条或任何静默条数上限；不得使用 ROWNUM、FETCH FIRST、LIMIT、subList、懒加载、无限滚动或分批请求截断/拆分结果 | 承接 JFM-HIST-003、JFM-PERF-003 的"不静默截断"意图 |
| JFM-ADJ-337 | 完整列表继续按首次失败时间降序；时间相同时按精确 faultRootIdText 或等价稳定字符串排序，刷新和切换范围后顺序稳定 | 承接 JFM-ADJ-291 的排序要求 |
| JFM-ADJ-338 | 结果为空时显示明确空状态；结果较多时使用页面正常纵向滚动，所有行仍一次渲染在同一表格中，不引入虚拟分页语义 | 修订 JFM-ADJ-292 的空状态要求 |
...
- `JFM-ADJ-291`：仅"翻页顺序稳定"表述被 JFM-ADJ-337 修订，排序继续有效；
- `JFM-ADJ-292`：后端分页、默认20条、20/50/100 与切换范围回第1页被 JFM-ADJ-332～336、338 替代；
- `JFM-ADJ-295`：页码变化部分被替代，clientId/dataSourceId/range 变化重载继续有效（JFM-ADJ-335）；
- `JFM-ADJ-296`：请求一致性继续有效，不再包含页码维度（JFM-ADJ-339）；
- `JFM-ADJ-302`：分页 List API 被 JFM-ADJ-333/334 替代，过程字段与状态复用继续有效；
- `JFM-ACCEPT-147`：分页验收被 JFM-ACCEPT-167～171 替代，7列、范围、倒序、空状态继续有效；
...
| JFM-HIST-002 | 不显示传统分页组件（无页码、无每页条数选择器） | — 【已被 §6.13 替代并经本轮（JOB-FAILURE-HISTORY-NO-PAGINATION-REQ-001）重新承接：独立历史列表改为前后端完全不分页，不显示任何分页组件；JFM-ADJ-292 的"后端分页/默认20条/20/50/100"替代关系由 JFM-ADJ-332～336 取代（JFM-ADJ-335、JFM-ACCEPT-167）】 |
| JFM-HIST-003 | 所选时间范围内的全部符合条件记录必须可见 | 不得因 pageSize 限制静默截断 【已被 §6.13 替代并经本轮重新承接：独立历史列表一次性返回并展示所选自然日范围内全部故障过程，不依赖分页与总数翻页，不再固定 pageSize=1000（JFM-ADJ-332、JFM-ACCEPT-169）】 |
...
> **数据源历史列表上下文与平台总量契约（本轮已确认 · 目标需求，baseline_status: APPROVED / implementation_status: IMPLEMENTED_ADJUSTMENT_PENDING）**：`JFM-API-006` History Summary 已返回 `clientId`、`dataSourceId`、`dataSourceOrg`、`dataSourceExists`、`dataSourceActive`，可由数据源历史列表页按 `clientId + dataSourceId` 精确匹配复用，用于顶部可读名称，且列表无历史记录时仍可读取该上下文（JFM-ADJ-328/329）。URL 仍只传稳定技术标识 `clientId` 与 `dataSourceId`，不把 `dataSourceOrg` 放入 URL。平台整体 X/Y（JFM-ADJ-318～323）的数据来源必须覆盖未筛选当前配置全集，不得从筛选子集计算。数据源上下文单页最多使用一次可复用的批量/摘要请求，不得逐行 N+1 补查。本轮为需求文档任务，不决定新增 API：若现有 Summary 足以满足直接访问、空列表上下文与稳定平台总量，则优先复用；仅当实现阶段证明现有契约无法同时满足时，才允许在后续实现提示词中提出最小接口扩展并附明确证据。
...
| JFM-PERF-003 | 故障历史按时间范围查询，不设 pageSize 硬编码上限 | 数据量增长后需评估是否需要后端调整查询策略 【已被 §6.13 修订并经本轮（JOB-FAILURE-HISTORY-NO-PAGINATION-REQ-001）重新承接：独立历史列表改为前后端完全不分页，无默认20条/最大100条或任何静默条数上限，所选范围内全部过程完整返回（JFM-ADJ-336、JFM-ACCEPT-170）；原"分页 + 总数返回"的承接由完整返回取代，不得以性能为由私自截断】 |
...
| JFM-ACCEPT-028 | 历史页面不显示传统分页组件（无页码、无每页条数选择器） | 浏览器实页验证 【已被 §6.13 替代并经本轮（JOB-FAILURE-HISTORY-NO-PAGINATION-REQ-001）重新承接：独立历史列表改为前后端完全不分页，不显示分页器、页码、每页条数、跳页、上一页/下一页等任何分页控件，也不维护分页状态，范围切换一次展示完整结果（JFM-ADJ-335、JFM-ACCEPT-167）】 |
| JFM-ACCEPT-029 | 所选时间范围内全部符合条件的记录必须可见 | 浏览器实页验证 + 接口返回核对 【已被 §6.13 替代并经本轮重新承接：独立历史列表一次性返回并展示所选自然日范围内全部故障过程，不依赖分页与总数翻页（JFM-ADJ-332、JFM-ACCEPT-169）】 |
| JFM-ACCEPT-030 | 固定 `pageSize=1000` 不得造成静默截断 | 代码审查 + 需确认数据量超过 1000 时的行为 【已被 §6.13 替代并经本轮重新承接：取消默认20条/最大100条与任何条数上限，所选范围内全部过程完整返回，不得以 ROWNUM、FETCH FIRST、LIMIT、subList、懒加载、分批或虚拟分页任何形式静默截断（JFM-ADJ-336、JFM-ACCEPT-170）】 |
...
| GAP-OVERVIEW-ZK-STATUS-001 | ZooKeeper 运行状态融合 | §6.7：以 ZooKeeper `alive` 临时节点作为运行状态依据，与数据库故障事实组合展示，客户端头部显示在线/离线，Job 状态新增"离线"，ZK 失败隐藏旧数据并 60 秒自动重试 | 完整链路已完成：状态融合主体由 Commit `c3b06bf1dec950e3c4b9a8a6f5e7434616f66317` 实现并完成技术核验；60 秒真实倒计时与告警图标动效由 Commit `715b476a26d8ad12abc22cdd29e23c60fe9df7a1` 实现并人工验收；集群地址/根路径诊断由 Commit `8268e1b266b64187cc41a6aa1612e37c2f692ef3` 实现、真实 HTTP 复制兼容由 Commit `533bbbc37bf2c9e1aa0939d20d6b6458c035a212` 修正并获用户确认；README 当前实现事实已同步 | 已解决/已关闭，仅保留用于历史追溯，已不存在该 GAP 描述的待实现差距 |
| GAP-OVERVIEW-ZK-RETRY-COUNTDOWN-001 | ZooKeeper 连接失败重试倒计时与告警图标动效 | §6.11：ZK 失败文案显示精确倒计时，重试时显示连接中状态，感叹号柔和呼吸并支持减少动态效果 | 已由 Commit `715b476a26d8ad12abc22cdd29e23c60fe9df7a1` 解决：`nextRetryAt` 绝对时间动态倒计时（60～1 秒）、连接中文案"正在重新连接 ZooKeeper…"、请求防重、后台节流校正、手动刷新、生命周期清理、呼吸动画与 reduced-motion 均已完成技术核验，并经用户查看真实页面确认"效果可以" | 已解决/已关闭，仅保留用于历史追溯，已不存在待实现差距 |
| GAP-OVERVIEW-ZK-CONNECTION-TARGET-001 | ZooKeeper 集群连接目标展示 | §6.12：ZK 错误态显示运行时实际集群连接串和根路径，并支持复制和失败降级 | 已由 Commit `8268e1b266b64187cc41a6aa1612e37c2f692ef3` 实现诊断区与 `GET /api/monitor/zookeeper/health` 复用（单生命周期单次请求、防重缓存、Summary 优先、降级、长文本、复制反馈）；由 Commit `533bbbc37bf2c9e1aa0939d20d6b6458c035a212` 解决真实 HTTP 非安全上下文复制失败；完整地址/根路径、复制与响应式均完成技术核验，用户已在真实 HTTP 页面确认可以正常复制 | 已解决/已关闭，仅保留用于历史追溯，已不存在该 GAP 描述的待实现差距 |
