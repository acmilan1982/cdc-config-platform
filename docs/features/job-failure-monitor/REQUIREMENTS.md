...
> **需求状态**：`baseline_status: APPROVED`（已批准），`implementation_status: IMPLEMENTED_ADJUSTMENT_PENDING`（已实现，待 §6.9 二次调整与人工验收）。
> 本节修订故障监控首页客户端卡片头部展示，将故障监控范围内用户可见术语统一为"数据源"，并明确故障过程详情页的数据源显示规则、处理结果命名、根事件 ID 展示与概览区域布局，作为后续实现与人工页面验收的唯一正式依据。
> 原 §6.8 已由 Commit `153750cef67afc112360e0d6f39340cde4677457` 实现并完成技术核验，但人工页面评审提出 §6.9 的二次调整，尚未完成最终人工验收和基线收口。本节不得标记为 `IMPLEMENTED_ACCEPTED`。
...
验收方式：需求文本自检、代码实现后的后端单元测试、前端构建、真实接口核对、浏览器页面检查、Tooltip/复制/路由网络请求核对以及人工页面验收。本节为文档任务，不伪造已完成的代码验收证据。
