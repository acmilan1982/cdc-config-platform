...
### 6.8 故障监控首页头部精简与故障详情页展示调整（本轮已确认 · 已实现待二次调整）

> **需求状态**：`baseline_status: APPROVED`（已批准），`implementation_status: IMPLEMENTED_ADJUSTMENT_PENDING`（已实现，待 §6.9 浅色语义分区与最终人工验收）。
> 本节修订故障监控首页客户端卡片头部展示，将故障监控范围内用户可见术语统一为"数据源"，并明确故障过程详情页的数据源显示规则、处理结果命名、根事件 ID 展示与概览区域布局，作为后续实现与人工页面验收的唯一正式依据。
> 原 §6.8 已由 Commit `153750cef67afc112360e0d6f39340cde4677457` 实现并完成技术核验；§6.9 主体已由 Commit `61bc3f197121c78ad72711c13f56fcec37b47328` 实现并完成技术核验，但人工页面评审进一步提出概览浅色语义分区调整，浅色语义分区及最终人工验收待完成，尚未完成基线收口。本节不得标记为 `IMPLEMENTED_ACCEPTED`。
...
### 6.9 故障详情页运维可读性、ID 精度与概览布局二次调整（本轮已确认 · 目标需求）

> **需求状态**：`baseline_status: APPROVED`（已批准），`implementation_status: IMPLEMENTED_VISUAL_ADJUSTMENT_PENDING`（主体已实现，浅色语义分区待开发与人工验收）。
...
> §6.9 主体（标题、事件表、精确 ID、2×3 概览与"处理概况"）已由 Commit `61bc3f197121c78ad72711c13f56fcec37b47328` 实现并完成技术核验；人工页面评审确认结构与内容正确，但提出概览浅色语义分区调整。本轮新增的浅色视觉规则（§6.9.5）尚未实现和人工验收，因此本节不得标记为 `IMPLEMENTED_ACCEPTED`。
...
| GAP-OVERVIEW-DETAIL-PRESENTATION-001 | 首页头部精简与详情页展示调整 | §6.8：首页客户端卡片头部精简（删除 Job 总数/正常/异常）、故障监控范围内用户可见术语"业务库"统一为"数据源"、首页与详情页数据源显示规则一致、详情页概览字段"当前处理状态"改为"本次故障处理结果"、根事件 ID 缩略显示与复制、概览区域改为 2 行 × 4 列带边框属性网格 | 需求已批准（APPROVED/IMPLEMENTED_ADJUSTMENT_PENDING）。§6.8 第一版已由 Commit `153750cef67afc112360e0d6f39340cde4677457` 实现并完成技术核验；§6.9 主体（标题、事件表语义、全链路精确 ID 与 2×3 概览）已由 Commit `61bc3f197121c78ad72711c13f56fcec37b47328` 实现并完成技术核验；概览浅色语义分区尚未实现并人工验收 | 首页头部精简、术语统一、详情数据源规则、处理结果命名、根事件 ID 展示和 2×4 概览网格已实现；§6.9 主体（标题、事件表、精确 ID、2×3 概览）已实现并技术核验，浅色语义分区及最终人工验收待完成；GAP 保持 OPEN |
| GAP-DETAIL-OPERABILITY-001 | 详情页运维可读性与 ID 精度二次调整 | §6.9：详情页用户可见标题（Job 重启轨迹/故障发生明细/未计入本次故障的事件）、事件表格精简与 `EVENT_RESULT` 展示、全链路 Snowflake ID 精确字符串契约、概览 2 行 × 3 列及"处理概况" | 需求已批准（APPROVED/IMPLEMENTED_VISUAL_ADJUSTMENT_PENDING）。Commit `61bc3f197121c78ad72711c13f56fcec37b47328` 已实现 §6.9 主体（标题、事件表、精确 ID、2×3 概览与"处理概况"）并完成技术核验；概览浅色语义分区尚未实现并人工验收 | 详情页标题、事件表语义、精确 ID 契约与 2×3 概览已实现并技术核验；概览浅色语义分区及最终人工验收待完成。该 GAP 与 GAP-OVERVIEW-DETAIL-PRESENTATION-001 关联：后者追踪 §6.8 第一版实现后的收口，本 GAP 追踪 §6.9 二次调整及浅色语义分区；两者均保持 OPEN |
