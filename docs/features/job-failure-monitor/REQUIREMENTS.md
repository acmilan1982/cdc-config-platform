...
### 6.8 故障监控首页头部精简与故障详情页展示调整（本轮已确认 · 目标需求）
...
| 编号 | 规则 | 说明 |
|---|---|---|
| JFM-ADJ-124 | 整体为 `2 行 × 4 列` 的视觉网格 | 修订 JFM-DETAIL-001 的"4 列网格布局" |
| JFM-ADJ-125 | 每个单元格有清晰边框；相邻单元格使用共享边界，避免双重粗线 | — |
| JFM-ADJ-126 | 每个单元格内部仍采用"字段名称在上、字段值在下"的层级 | — |
| JFM-ADJ-127 | 第二行"本次故障处理结果"横跨第 3、4 列，不留下无意义空白格 | — |
| JFM-ADJ-128 | 处理结果使用内容自适应宽度的状态标签，不再使用横向铺满的大块绿色状态条 | — |
| JFM-ADJ-129 | 网格应与现有详情卡片的边框、圆角、背景和字号协调 | — |
| JFM-ADJ-130 | 常用桌面宽度下不得产生由该网格引起的横向滚动 | — |
| JFM-ADJ-131 | 较窄宽度下允许响应式降为两列或一列，但字段顺序和语义必须保持，内容不得重叠或截断 | — |
| JFM-ADJ-132 | 本轮不调整"物理 Job 链""主链事件""处理过程""历史故障过程"的业务内容和数据逻辑 | — |
...
| JFM-ACCEPT-088 | 概览为两行四列带边框网格，第二行结果跨两列，状态标签不横向铺满 |
| JFM-ACCEPT-089 | 常用桌面宽度无横向滚动，较窄宽度布局可用 |
| JFM-ACCEPT-090 | 物理 Job 链、主链事件、处理过程、历史故障过程及详情跳转不回归 |
...
| 编号 | 规则 | 说明 |
|---|---|---|
| JFM-DETAIL-001 | 故障概览网格展示：客户端、业务库、故障根事件、首次失败时间、最近处理时间、重启次数、当前处理状态 | 4 列网格布局 【已被 §6.8 修订：概览字段"业务库"统一为"数据源"、"当前处理状态"改为"本次故障处理结果"、网格改为 2 行 × 4 列带边框属性网格（依据 §6.8.2 JFM-ADJ-103、§6.8.4 JFM-ADJ-114、§6.8.6 JFM-ADJ-124）】 |
...
| GAP-OVERVIEW-DETAIL-PRESENTATION-001 | 首页头部精简与详情页展示调整 | §6.8：首页客户端卡片头部精简（删除 Job 总数/正常/异常）、故障监控范围内用户可见术语"业务库"统一为"数据源"、首页与详情页数据源显示规则一致、详情页概览字段"当前处理状态"改为"本次故障处理结果"、根事件 ID 缩略显示与复制、概览区域改为 2 行 × 4 列带边框属性网格 | 需求已批准（APPROVED/PENDING），尚未实现 | 首页头部精简、术语统一、详情数据源规则、处理结果命名、根事件 ID 展示和概览网格尚待代码实现与人工页面验收 |
