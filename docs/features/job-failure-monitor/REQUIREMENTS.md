...
### 6.10 历史故障过程独立路由导航（本轮已确认 · 目标需求）

> **需求状态**：`baseline_status: APPROVED`（已批准），`implementation_status: PENDING`（待实现）。
...
> 本节只写需求，不实现代码，不更新 README，不提前关闭新需求 GAP。本节为文档任务，不得伪造代码已实现。
...
验收方式：代码审查、前端类型检查与构建、真实页面普通点击、Ctrl/中键或 href 核对、地址栏 URL、刷新、后退、前进、页面滚动位置、网络请求精确路径、非法 ID、控制台和人工页面验收。本次为文档任务，不得伪造代码已实现。
...
> **与 §6.10 的关系（本轮已确认 · 目标需求）**：`JFM-API-004`（`GET /api/job-failure/process/{faultRootId}`）后端 API 路径、Controller 与查询语义不变；新增的前端 `/monitor/job-failure/process/:faultRootId` 路由复用该 API。前端页面路由与后端 API 路由是两种不同资源，不得把二者写成同一个 URL（详见 §6.10）。
...
| GAP-DETAIL-HISTORY-NAVIGATION-001 | 历史故障过程独立路由导航 | §6.10：历史故障过程通过独立前端路由 `/monitor/job-failure/process/:faultRootId` 访问，支持刷新、浏览器前后导航、浏览器原生新标签页操作及自动回到详情顶部 | 需求已批准（APPROVED/PENDING）。Commit `c0b811e8f3b6f48446c204544b7f6b900e11f177` 仍为组件内原地替换详情，浏览器 URL 不变，刷新和后退均无法恢复 | 需求已批准，代码待实现和人工验收，GAP 保持 OPEN |
