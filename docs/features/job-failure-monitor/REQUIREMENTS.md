...
> **需求状态**：`baseline_status: APPROVED`（已批准），`implementation_status: IMPLEMENTED_ADJUSTMENT_PENDING`（主体已实现，默认展开交互调整待完成）。
...
> **状态说明**：本节是已批准的目标需求。Commit `744980f45060a30d507c75fddf06aa164a042167` 已实现 §6.13 主体：独立"故障历史"菜单、自然日统计概览（今日/近7天/近30天）、当前配置数据源全集、根过程去重计数、独立分页历史列表、精确 `faultRootIdText` 详情链接、详情页"历史故障过程"区块移除，以及 DB-only 与 ZooKeeper 解耦；后端定向测试、后端打包与前端构建已完成技术核验。人工页面评审发现故障历史概览客户端卡片首次加载默认折叠，今日/近7天/近30天核心统计被隐藏，默认全部展开与刷新状态保持规则（JFM-ADJ-306～312、JFM-ACCEPT-155～158）待纯前端最小修正后再次人工验收，因此本节不得标记为 `IMPLEMENTED_ACCEPTED`，`GAP-HISTORY-001` 保持 OPEN。README 继续描述当前实现事实，不得提前按本节改写。
...
JFM-ACCEPT-139～154 为 §6.13 主体验收标准：主体技术核验已完成（Commit `744980f45060a30d507c75fddf06aa164a042167`），但因默认展开交互调整尚未完成，本节不得整体标记为已通过；JFM-ACCEPT-155～158 待本轮纯前端最小修正后执行技术核验与人工页面验收。
...
| GAP-HISTORY-001 | 历史查询/展示/时间范围（独立"故障历史"页面） | JFM-HIST-002、JFM-HIST-003：无传统分页组件，所选时间范围内全部记录可见；该展示在目标需求中由 §6.13 独立"故障历史"页面替代：独立菜单、自然日统计概览（今日/近7天/近30天）、独立分页历史列表、当前配置校验及详情页"历史故障过程"区块移除（JFM-ADJ-255～312、JFM-ACCEPT-139～158） | §6.13 主体已由 Commit `744980f45060a30d507c75fddf06aa164a042167` 实现：独立"故障历史"菜单、自然日统计概览、当前配置数据源全集、根过程去重计数、独立分页历史列表、精确 `faultRootIdText` 详情链接、详情页"历史故障过程"区块移除及 DB-only，后端定向测试/打包与前端构建已完成技术核验；人工页面评审发现客户端卡片首次加载默认折叠，主要历史统计内容被隐藏 | 超过 1000 条的旧截断问题已由独立分页列表承接；当前待办为客户端卡片默认全部展开与刷新状态保持（JFM-ADJ-306～312），完成 JFM-ACCEPT-155～158 技术核验和人工页面验收后再收口。`baseline_status: APPROVED / implementation_status: IMPLEMENTED_ADJUSTMENT_PENDING`，本 GAP 保持 OPEN，不提前关闭 |
