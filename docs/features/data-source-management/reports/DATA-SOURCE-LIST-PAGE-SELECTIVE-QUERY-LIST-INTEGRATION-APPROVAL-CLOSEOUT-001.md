...
## ⚠️ R1 勘误声明（2026-09-19，醒目前置；本条取代本报告 §7 与 §8 中相关结论）

> **本报告 §7 → “`DATABASE.md §8` 保留 `DRAFT_PENDING_USER_REVIEW` 属已知残留项，应按独立维护任务处理”的判断已被远程独立复审判定为错误，现已作废。**
>
> - **触发**：ChatGPT 从远程 Git 对上一批准收口提交 `178b7a0cb5ae2b467847ba940615d1b80a323da2` 独立复审返回 `review_status=CHANGES_REQUIRED`、`blocking_finding_count=1`；唯一阻塞问题即 `DATABASE.md §8` 状态未随本轮调整基线一并收口为 `APPROVED`。
> - **原因**：`DATABASE.md §8`（“本轮列表首页调整的数据库变化声明”）是**本轮调整基线专门新增的当前声明**，不是与本轮无关的历史过期元数据。项目负责人批准整轮调整基线后，该声明必须同步收口为 `APPROVED`。本报告 §7 把它当作“历史维护事项/已知残留项”处理，属**判断错误**。
> - **归因**：该遗漏源于上一份批准收口提示词的**范围排除错误**（错误地把 `DATABASE.md` 排除在允许修改范围之外），**不是上一 Agent 未按提示词执行**。
> - **取代关系**：本报告 §7 “因此 `DATABASE.md §8` 保留 `DRAFT_PENDING_USER_REVIEW` 字样，属已知残留项……不得在本批准任务中顺手修改”的结论，以及 §8 机械核验表中“剩余 `DRAFT_PENDING_USER_REVIEW` 仅出现在历史变更记录或 `DATABASE.md §8` 残留 | 通过（符合“历史事实不机械替换”规则）”一行中关于 `DATABASE.md §8` 的部分，**均已被本条勘误取代**。
> - **当前权威状态**：以 R1 修订后的 **`DATABASE.md §8 APPROVED`** 为准；修订任务为 `DATA-SOURCE-LIST-PAGE-SELECTIVE-QUERY-LIST-INTEGRATION-APPROVAL-CLOSEOUT-001-R1`，详见同目录 R1 报告。
> - **保留范围**：本报告 §1~§6、§9~§12 中除上述被取代结论外的其他执行事实、Git 结果、正文冻结核验与安全声明，仍作为**历史记录**原样保留，未删除、未重写。
> - **性质不变**：本条勘误只纠正状态范围遗漏，**不修改任何数据库技术结论**，本轮仍为“无数据库变化声明”，仍为 `NOT_STARTED` / 验收全部 `NOT_RUN` / 无 DDL。
...
> **⚠️ 本节结论已被远程复审判定为错误（2026-09-19，见文首 R1 勘误声明）**：本节“`DATABASE.md §8` 属已知残留项、应按独立维护任务处理、不得在本批准任务中顺手修改”的判断**不再成立**，已由 `...-APPROVAL-CLOSEOUT-001-R1` 取代。本节以下原文仅作为**历史记录**保留。
