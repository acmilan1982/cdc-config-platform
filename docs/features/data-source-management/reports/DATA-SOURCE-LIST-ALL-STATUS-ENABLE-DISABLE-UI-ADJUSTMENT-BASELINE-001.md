...
> ## ⚠️ R1 勘误声明（2026-09-19，任务 `...-001-R1`）
>
> **本报告为初版草案报告，其部分结论已被 R1 定向修订取代，阅读时必须以 R1 结论为准。**
>
> - ChatGPT 对远程提交 `4ccd66106e832a0fd10dc42617507899cbb26463` 的独立复审结论为 **`CHANGES_REQUIRED`**，`blocking_finding_count=3`，`implementation_may_start=NO`。
> - 本报告中下列结论**已被 R1 取代**：① `traceability_status=COMPLETE`（§8、§9）——因初版错误引用了一个不存在的需求编号（记为 `DS-REQ-18x`，编号超出本轮最大 `DS-REQ-177`），该结论不成立，R1 已修正为引用 `DS-REQ-047`/`DS-REQ-107`/`DS-REQ-169`；② §6 中“单条 `UPDATE` 短事务”“`disable` 可由 `UPDATE ... WHERE DATA_SOURCE_ID=?` 自身完成归一化”——已由 R1 取代为“两接口均先读取、每次非幂等状态变更最多一条**带原状态条件**的 `UPDATE`、`NULL` 正确匹配、**禁止**无状态条件 `UPDATE`”；③ §6 前端消息处理中“`40400`/`40250` 后刷新列表”——已由 R1 取代为“**任何启停失败均不刷新列表**，只有成功才刷新”；④ §3.1/§6 中“异常/冲突结果按实现期冻结的错误码”——已由 R1 取代为明确的“条件 `UPDATE` 冲突 → `50002` 回滚”。
> - 上述问题**源于初版草案文档内部不一致**，**不是**已实现功能的问题（本轮未实现任何功能），亦**不是**测试或验收结论。
> - R1 完成后本轮草案仍为 `DRAFT_PENDING_USER_REVIEW`、`implementation_status=NOT_STARTED`、`implementation_authorization_status=NOT_GRANTED_IN_THIS_TASK`、`new_adjustment_acceptance_status=ALL_NOT_RUN`；未获批准、未授权实现、未执行验收。
> - 修订详情见 [DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-BASELINE-001-R1.md](./DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-BASELINE-001-R1.md)。
>
...
- 并发：不加锁、不引入乐观版本、不新增并发控制，最终**收敛**；文档**不承诺**“至多一次成功”，也**不承诺**请求顺序与最终值一一对应。**【已被 R1 取代/补强】** R1 进一步冻结为：两接口均**先读取**原始 `FG_ACTIVE`；每次非幂等状态变更最多一条**带原状态条件**的 `UPDATE`（`NULL` 正确匹配）；**禁止**无状态条件 `UPDATE`；发生条件 `UPDATE` 冲突（影响行数 ≠ 1）的请求返回 `50002` 并回滚（不再表述为“实现期冻结”）。
...
- 单条主表记录的**单条 `UPDATE`**、短事务；`enable` 写入前**先读取**当前 `FG_ACTIVE` 以实施状态机；`disable` 可由 `UPDATE ... SET FG_ACTIVE='0' WHERE DATA_SOURCE_ID=?` 自身完成归一化。**【已被 R1 取代】** 改为：两接口均**先读取**；**每次非幂等状态变更最多执行一条 `UPDATE`，允许 `UPDATE` 前先读取**；`UPDATE` **必须带原状态条件**（`DATA_SOURCE_ID` + 事务首次读取的原始 `FG_ACTIVE`，含 `NULL`），**禁止**无状态条件 `UPDATE ... WHERE DATA_SOURCE_ID=?`。
...
- 并发：不加锁、不引入乐观版本、不新增并发控制；最终**收敛**；不承诺“至多一次成功”。**【已被 R1 取代/补强】** R1 冻结：发生条件 `UPDATE` 冲突（影响行数 ≠ 1，含读取后被其他请求改变导致 `0` 行）的请求返回 `50002` 并回滚；不引入重试/悲观锁/乐观版本/新错误码；重复目标状态仅在事务首次读取已处于目标状态时幂等成功。
...
| `UPDATE` 影响行数 ≠ 1（含读取后被改变导致 `0` 行） | `50002`（`STATUS_FAILED`，恢复启用既有码），事务**回滚**；发生条件 `UPDATE` 冲突的并发请求同样返回 `50002` 【R1 补强】 |
| 其他保存类失败 | `50000`（`SAVE_FAILED`，复用） |

**前端消息处理（冻结）**：`40400` → 提示目标不存在并按已应用条件刷新；`40250` → 提示状态异常、只能先停用归一化并刷新；`50002`/`50000` → 提示失败请重试，**保留**当前列表与已应用条件、恢复 busy；成功提示只陈述“状态已更新”。**成功刷新按当前已应用条件**（不读未点击“查询”的草稿条件）；因列表展示全部状态，停用成功后该行**仍保留**并出现“停用”标识，启用成功后标识消失。**【已被 R1 取代】** 删除 `40400`/`40250` 后刷新列表：**任何启停失败（含网络失败、`40400`、`40250`、`50002`、`50000` 或其他业务错误）均不自动重新查询**，一律保留当前列表、结果总数与已应用条件并恢复 busy；**只有成功**才按当前已应用条件重新查询（`DS-REQ-173`/`DS-AC-177`）。
...
> **【已被 R1 取代】** 以下 `traceability_status=COMPLETE` 在初版不成立：`API.md` §11 曾错误引用一个不存在的需求编号（记为 `DS-REQ-18x`）。R1 已将该引用改为 `DS-REQ-047`/`DS-REQ-107`/`DS-REQ-169`，并在 §5 追踪矩阵为 `DS-REQ-047`/`DS-REQ-107` 增补 `DS-AC-181`；R1 后状态为 `traceability_status=COMPLETE_AFTER_R1`。
...
| 2026-09-19 | **批准收口**：记录批准链（`4ccd661...` → `c4e1048...` → `aa906c0...`）与项目负责人批准“批准本轮调整基线”；修正原“ChatGPT 远程 R2 复审 `CHANGES_REQUIRED`”历史链措辞为“对 R1 提交 `c4e1048...` 复审 `CHANGES_REQUIRED`，触发 R2 修订”，并区分 R2 提交复审为 `REVIEW_PASS`；本轮状态由 `DRAFT_PENDING_USER_REVIEW` 收口为 `APPROVED`，仍未授权实现、验收全部 `NOT_RUN` | DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-APPROVAL-CLOSEOUT-001（纯文档任务） |
