...
## 11. 启用/停用接口与列表状态字段（`APPROVED`，`IMPLEMENTED_ACCEPTED`，正式验收已执行且最终验收已通过 `PASS=42/FAIL=0/BLOCKED=0/NOT_RUN=0`，`final_acceptance_status=ACCEPTED`）

> 本轮调整基线分层状态：`adjustment_document_status=APPROVED`、`adjustment_baseline_status=APPROVED`、`adjustment_api_status=APPROVED`、`implementation_status=IMPLEMENTED_ACCEPTED`、`final_acceptance_status=ACCEPTED`、`implementation_authorization_status=GRANTED_IN_THIS_TASK`、`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`、`new_adjustment_acceptance_status=PASS_42_OF_42`（2026-09-20 由 `DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-FORMAL-ACCEPTANCE-001` 在真实环境执行，并经项目负责人 2026-09-20 最终验收接受；该 `ACCEPTED` **仅**适用于本节当前接口与字段契约）。
>
...
| 接口 | 事务内**首次读取**到的 `FG_ACTIVE` | 行为 | 结果 |
|---|---|---|---|
| `enable` | `'0'` | 条件 `UPDATE` 写 `'1'` | 成功 |
| `enable` | `'1'` | 不执行 DML | 幂等成功（重复目标状态） |
| `enable` | `NULL` / 非 `0`/`1` | 拒绝，不执行 DML | `40250` |
| `disable` | `'1'` | 条件 `UPDATE` 写 `'0'` | 成功 |
| `disable` | `'0'` | 不执行 DML | 幂等成功（重复目标状态） |
| `disable` | `NULL` / 非 `0`/`1` | 条件 `UPDATE` 写 `'0'` | 成功（显式归一化） |

- **读取（冻结）**：`enable` 与 `disable` 均在事务内**先按 `DATA_SOURCE_ID` 读取主表当前记录及其原始 `FG_ACTIVE`**，**不**按 `FG_ACTIVE='1'` 过滤；记录不存在 → `40400`，不执行任何 DML。只读取本表该记录，**不访问**源库、ZooKeeper/Kafka 或进程；**不加锁**、**不新增**版本字段。
- **事务与写入边界（冻结）**：**每次非幂等状态变更最多执行一条 `UPDATE`；允许在 `UPDATE` 前执行状态读取**——即接口全程**不保证**只有一条 SQL。非幂等路径的 `UPDATE` **必须带原状态条件**，同时匹配 `DATA_SOURCE_ID` 与**本事务开始时读取到的原始 `FG_ACTIVE`**（`NULL` 须正确匹配）；等价伪 SQL 见 `DATABASE.md` §9.3。**不得**退化为无状态条件的 `UPDATE ... WHERE DATA_SOURCE_ID=?`。
...
- **影响行数与并发（冻结）**：非幂等路径的条件 `UPDATE` 影响行数为 `1` → 成功；**不为 `1`（含读取后被其他请求改变导致 `0` 行）→ `50002`，当前事务回滚**。**不**引入重试、再次读取后改判成功、悲观锁、乐观版本字段或新错误码。重复目标状态**只有**在事务首次读取时已处于目标状态，才按幂等成功处理。并发相反请求的最终数据库状态允许为其中一个请求的目标值；发生条件 `UPDATE` 冲突的请求返回 `50002`（`DS-REQ-171`）。
...
| 场景 | HTTP | code | 说明 |
|---|---|---|---|
| 目标主表记录不存在（事务内首次读取未命中） | 200（既有统一结构） | `40400` | 复用 §5.2 `40400`；事务内无任何写入 |
| 非法状态（仅 `enable`，当前值为 `NULL`/非 `0`/`1`） | 200 | `40250` | **本轮唯一新增业务码**；不写库 |
| 重复目标状态（`enable` 遇 `'1'`、`disable` 遇 `'0'`） | 200 | 成功码 | 幂等成功，不写库、不报错 |
| `UPDATE` 影响行数 ≠ 1（含读取后状态被其他请求改变导致 `0` 行） | 200 | `50002` | `STATUS_FAILED`（由“废弃”恢复为生效，见 §5.2 追加说明）；**当前事务回滚**，不留中间状态 |
| 其他保存类失败 | 200 | `50000` | 沿用既有 `SAVE_FAILED` 语义 |

**前端消息处理（冻结）**：`40400` → 提示目标数据源不存在，**不刷新列表**；`40250` → 提示状态异常、需先停用归一化，**不刷新列表**；`50002`/`50000` → 提示操作失败、请重试，**不刷新列表**。**任何启停失败（含网络失败、`40400`、`40250`、`50002`、`50000` 或其他业务错误）均不自动重新查询**，一律**保留**当前列表、结果总数与已应用查询条件并**恢复 busy**（`DS-REQ-173`/`DS-AC-177`）；**只有成功**才按**当前已应用查询条件**重新查询（`DS-REQ-172`）。错误消息不得泄露堆栈、密码或其他敏感信息（`DS-REQ-047`/`DS-REQ-107`；并见 `DS-REQ-169` 不访问外部系统与成功消息边界）。
...
| DS-REQ-047、DS-REQ-107、DS-REQ-169 | §11.2、§11.4（不返回密码等敏感字段、不泄露敏感信息；启停不访问外部系统、成功消息不越界） |
...
### 12.2 R1 定向修订（2026-09-19，任务 `DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-BASELINE-001-R1`）

- 依据 ChatGPT 对远程提交 `4ccd6610...` 的 R1 复审结论 `CHANGES_REQUIRED`（3 类阻塞问题），只做对应最小修订；**不新增需求编号**，`DS-REQ-139~177` 正文与编号零变化。
...
### 12.4 实现状态回写（2026-09-19，任务 `DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-IMPLEMENTATION-001`）

- §11 章节标题由“（`APPROVED`，未实现）”更新为“（`APPROVED`，`IMPLEMENTED_PENDING_USER_REVIEW`）”；分层状态由 `implementation_status=NOT_STARTED`/`implementation_authorization_status=NOT_GRANTED_IN_THIS_TASK` 更新为 `IMPLEMENTED_PENDING_USER_REVIEW`/`GRANTED_IN_THIS_TASK`，并补充实现任务号（`adjustment_document_status`/`adjustment_baseline_status`/`adjustment_api_status` 保持 `APPROVED`，`formal_acceptance_execution_status=NOT_RUN`、`new_adjustment_acceptance_status=ALL_NOT_RUN` 保持）；
- 实现按 §11 已批准契约落地：`GET /api/data-sources` 列表返回 `CDC_DATA_SOURCE` 全部状态并在行 VO 中返回原始 `fgActive`（不归一化）；新增 `PUT /api/data-sources/{dataSourceId}/enable` 与 `PUT /api/data-sources/{dataSourceId}/disable`（无请求体，路径段 `encodeURIComponent`）；错误码复用 `40400`、新增 `40250`（仅 `enable` 遇异常状态）、恢复 `50002`（影响行数 ≠ 1 / 并发冲突），HTTP 仍为 200 + 业务 `code`；
- 非幂等 `UPDATE` 带原状态条件（`NULL` 用 `FG_ACTIVE IS NULL`、非空异常值用 `FG_ACTIVE=:observedStatus`），重复目标状态幂等不写库，影响行数 ≠ 1 → `50002` 回滚；失败不刷新列表；
...
### 12.7 最终验收接受与状态回写（2026-09-20，任务 `DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-FINAL-ACCEPTANCE-CLOSEOUT-001`）
...
- **§11.1~§11.7 API 契约逐字冻结**：`PUT /api/data-sources/{id}/enable`、`PUT /api/data-sources/{id}/disable` 的方法、路径、请求体、成功响应、错误码与错误响应、事务与状态机边界，以及 `fgActive` 四态 JSON 契约均**零变化**；`DS-REQ-139~177` 与 `DS-AC-001~182` 编号与正文未改。
...
## 13. 新增/修改时间字段维护、列表默认排序与主弹窗表单视觉调整的接口影响（`APPROVED`，`IMPLEMENTED_ACCEPTED`，正式验收已在本地真实环境执行且 17 条全部 `PASS`，`final_acceptance_status=ACCEPTED`）

> 分层状态：`adjustment_document_status=APPROVED`、`adjustment_baseline_status=APPROVED`、`adjustment_api_status=APPROVED`、`implementation_authorization_status=GRANTED_IN_THIS_TASK`、`implementation_status=IMPLEMENTED_ACCEPTED`、`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`、`new_adjustment_acceptance_status=PASS_17_OF_17`、`final_acceptance_status=ACCEPTED`、`accepted_scope=DS-REQ-178~188 / DS-AC-183~199`、`accepted_business_implementation_commit=55e6273b74182c408e36b75e09ad21819f33d3e2`、`formal_acceptance_evidence_commit=935478a0966d2ec6fe157c3494d813427b1e05ff`、`chatgpt_remote_formal_acceptance_review=REVIEW_PASS`、`blocking_finding_count=0`、`project_owner_final_acceptance_date=2026-09-20`、`project_owner_visual_review_status=PASS`、`password_false_required_defect_status=FIXED_CONFIRMED`。本轮验收 `DS-AC-183~199`（17 条）已于 2026-09-20 正式验收执行：`PASS=17/FAIL=0/BLOCKED=0/NOT_RUN=0`（`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`、`new_adjustment_acceptance_status=PASS_17_OF_17` 为本地真实执行分层值，**逐字保留**）。
>
> `project_owner_visual_review_status=PASS` 仅代表项目负责人 2026-09-20 在真实运行页面完成**页面目测/功能复测并通过**（原话“我试了，新增修改都没有问题了。”）。ChatGPT 已从远程 Git 对正式验收证据提交 `935478a0966d2ec6fe157c3494d813427b1e05ff` 完成正式验收复审，结论 `status=REVIEW_PASS`、`acceptance_result=PASS_17_OF_17`、`blocking_finding_count=0`、`project_owner_final_acceptance_recommendation=ACCEPT`；项目负责人据此于 2026-09-20 作出**最终验收接受**决定，本组接口影响随调整收口为 `IMPLEMENTED_ACCEPTED`/`final_acceptance_status=ACCEPTED`（收口任务 `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FINAL-ACCEPTANCE-CLOSEOUT-001`）。本次 `ACCEPTED` **严格只适用于** `DS-REQ-178~188`/`DS-AC-183~199` 当前调整，**不代表**数据源管理 Feature 整体正式验收状态或系统生产可用；被最终接受的业务实现提交为 `55e6273b...`（完整实现链 `807a5a58... → 55e6273b...`）。
...
### 14.3 正式验收执行与状态回写（2026-09-20，任务 `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FORMAL-ACCEPTANCE-001`）

- **触发**：`DS-AC-183~199`（17 条）在真实后端 + 真实 Oracle 开发库 + 真实浏览器 + 受控验收数据（`RUN_TAG=FACC002`，运行源 `db1cfda7...`）下端到端执行，`PASS=17/FAIL=0/BLOCKED=0/NOT_RUN=0`。
- **§13 标题与状态块更新**：标题由「（`APPROVED`，`IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`）」更新为「（`APPROVED`，`IMPLEMENTED_PENDING_FINAL_ACCEPTANCE`，正式验收已在本地真实环境执行且 17 条全部 `PASS`）」；分层状态 `implementation_status` 由 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE` 更新为 `IMPLEMENTED_PENDING_FINAL_ACCEPTANCE`，`formal_acceptance_execution_status` 由 `NOT_RUN` 更新为 `EXECUTED_PASSED_LOCAL`，`new_adjustment_acceptance_status` 由 `ALL_NOT_RUN` 更新为 `PASS_17_OF_17`，并新增 `final_acceptance_status=NOT_ACCEPTED_PENDING_PROJECT_OWNER`。
- **§13 技术正文逐字冻结**：§13.1 接口数量、§13.2~§13.5 的路径、方法、参数、请求体、响应、错误码、说明文案、排序子句与 §13.6 追踪表**零变化**；`DS-REQ-178~188` 编号与正文未改，`DS-AC-183~199` 编号、关联需求、前置条件、操作步骤、预期结果未改，仅状态列由 `NOT_RUN` 更新为 `PASS`。
...
- **边界**：本轮 `EXECUTED_PASSED_LOCAL` **不构成**最终验收，`DS-AC-183~199` 已全部 `PASS` 但**不得**写为 `ACCEPTED`/`IMPLEMENTED_ACCEPTED`/`final_acceptance_status=ACCEPTED`/生产可用；既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0`、`DS-AC-104`/`DS-AC-108` 两个 `BLOCKED` 逐字保留；上一轮 `DS-AC-116~140`（25 条 `NOT_RUN`）与已最终接受的 `DS-AC-141~182`（42 条 `PASS`、`final_acceptance_status=ACCEPTED`）状态未改变；未修改业务代码/测试/配置/依赖/锁文件/SQL；未访问 ZooKeeper 写、Kafka、业务源库、目标库。报告：`reports/DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FORMAL-ACCEPTANCE-001.md`。

### 14.4 最终验收接受与收口（2026-09-20，任务 `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FINAL-ACCEPTANCE-CLOSEOUT-001`）
...
- **§13 标题与状态块更新**：标题由「（`APPROVED`，`IMPLEMENTED_PENDING_FINAL_ACCEPTANCE`，正式验收已在本地真实环境执行且 17 条全部 `PASS`）」更新为「（`APPROVED`，`IMPLEMENTED_ACCEPTED`，正式验收已在本地真实环境执行且 17 条全部 `PASS`，`final_acceptance_status=ACCEPTED`）」；分层状态 `implementation_status` 由 `IMPLEMENTED_PENDING_FINAL_ACCEPTANCE` 更新为 `IMPLEMENTED_ACCEPTED`、`final_acceptance_status` 由 `NOT_ACCEPTED_PENDING_PROJECT_OWNER` 更新为 `ACCEPTED`，并新增 `accepted_scope`、`accepted_business_implementation_commit=55e6273b...`、`formal_acceptance_evidence_commit=935478a...`、`chatgpt_remote_formal_acceptance_review=REVIEW_PASS`、`blocking_finding_count=0`、`project_owner_final_acceptance_date=2026-09-20`；`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`、`new_adjustment_acceptance_status=PASS_17_OF_17` **逐字保留**。
- **§13 技术正文继续逐字冻结**：§13.1~§13.6 的接口数量、路径、方法、参数、请求体、响应、错误码、说明文案、排序子句与追踪表**零变化**；`DS-REQ-178~188` 编号与正文未改，`DS-AC-183~199` 编号、关联需求、前置条件、操作步骤、预期结果与 17 条 `PASS` 未改。§14.3 历史记录作为历史事实逐字保留。
