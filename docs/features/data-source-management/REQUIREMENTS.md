...
> 项目负责人已确认的产品输入（2026-09-19）：数据源管理列表页**展示全部状态**、允许**逐行启用/停用**，并作**头部、列、行高、字体与新增按钮**的视觉微调。本轮草案只把已确认结论写入权威文档，**不代表**已批准、已实现、已测试、已验收或生产可用。
