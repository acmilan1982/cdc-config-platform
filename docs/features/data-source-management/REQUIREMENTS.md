...
## 23. 列表展示全部状态、启用/停用及视觉微调调整需求（`APPROVED`）

> 状态：`APPROVED`。本轮调整基线分层状态：`adjustment_document_status=APPROVED`、`adjustment_baseline_status=APPROVED`、`adjustment_requirements_status=APPROVED`、`implementation_status=NOT_STARTED`、`implementation_authorization_status=NOT_GRANTED_IN_THIS_TASK`、`formal_acceptance_execution_status=NOT_RUN`、`new_adjustment_acceptance_status=ALL_NOT_RUN`。
...
> 项目负责人已确认的产品输入（2026-09-19）：数据源管理列表页**展示全部状态**、允许**逐行启用/停用**，并作**头部、列、行高、字体与新增按钮**的视觉微调。本轮调整基线已把上述已确认结论写入权威文档；批准只代表需求基线正式成立，**不代表**已实现、已测试、已验收或生产可用。
