...
## 23. 列表展示全部状态、启用/停用及视觉微调调整需求（`APPROVED`，`IMPLEMENTED_ACCEPTED`，正式验收已执行且最终验收已通过 `PASS=42/FAIL=0/BLOCKED=0/NOT_RUN=0`，`final_acceptance_status=ACCEPTED`）

> 状态：`APPROVED`。本轮调整基线分层状态：`adjustment_document_status=APPROVED`、`adjustment_baseline_status=APPROVED`、`adjustment_requirements_status=APPROVED`、`implementation_status=IMPLEMENTED_ACCEPTED`、`final_acceptance_status=ACCEPTED`、`implementation_authorization_status=GRANTED_IN_THIS_TASK`、`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`、`new_adjustment_acceptance_status=PASS_42_OF_42`。
...
> 最终验收收口任务：`DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-FINAL-ACCEPTANCE-CLOSEOUT-001`（项目负责人 2026-09-20 最终验收接受；依据 ChatGPT 对远程证据提交 `30e902f7c2c3de62b7a7ff454fa25a4cef74fdfa` 的正式验收 R1 复审 `REVIEW_PASS`；接受范围 `DS-REQ-139~177`/`DS-AC-141~182`；**仅**适用于本组当前调整，不改变数据源管理 Feature 整体正式验收状态）。
...
> 项目负责人已确认的产品输入（2026-09-19）：数据源管理列表页**展示全部状态**、允许**逐行启用/停用**，并作**头部、列、行高、字体与新增按钮**的视觉微调。本轮调整基线已把上述已确认结论写入权威文档；批准只代表需求基线正式成立，**不代表**已测试、已验收或生产可用：实现已由 `DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-IMPLEMENTATION-001` 完成（实现状态 `IMPLEMENTED_ACCEPTED`、`final_acceptance_status=ACCEPTED`、实现授权 `GRANTED_IN_THIS_TASK`）；新增验收 `DS-AC-141~182`（42 条）已由 `DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-FORMAL-ACCEPTANCE-001` 于 2026-09-20 在真实环境执行，结果 `PASS=42/FAIL=0/BLOCKED=0/NOT_RUN=0`，并经项目负责人 2026-09-20 最终验收接受（`DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-FINAL-ACCEPTANCE-CLOSEOUT-001`）；该最终接受**仅**适用于 `DS-REQ-139~177`/`DS-AC-141~182` 本组当前调整。
...
> **局部替代声明（`DS-REQ-110` 空状态辅助提示语，§23.1）**：`DS-REQ-139`/`DS-REQ-140`（重置零请求、查询仅在用户点击“查询”时发起）**局部替代** `DS-REQ-110` 中“有生效查询条件且结果为零时，…辅助提示为‘请调整查询条件后重试，或点击上方“重置”查看全部数据源’”这一**提示语语义**：该措辞隐含“重置本身即会查询 / 点重置即可看到全部数据源”，与本组“重置零请求”结论直接冲突。本组把该辅助提示语修正为“请调整查询条件后重试，或点击上方‘重置’后再点击‘查询’查看全部数据源”。**仅**替代该提示语中“重置即可查看全部数据源”的措辞与语义；`DS-REQ-110` 关于“空状态依据**最后一次实际执行并生效的查询条件**区分而非依据尚未点击查询的表单内容”“有生效查询条件且结果为零时主提示为‘未找到符合当前查询条件的数据源’”“无生效查询条件且列表为零时主提示为‘暂无数据源’并引导右上角‘新增数据源’创建第一条数据源”等**其余空状态要求继续有效**。本替代属同一空状态需求内部的**已批准新旧条款一致性勘误**（新旧条款对同一提示语给出互斥措辞），**不是**新的业务决策，不新增、不重编号、不删除任何需求条目。
...
## 24. 新增/修改时间字段维护、列表默认排序与主弹窗表单视觉调整需求（`APPROVED`，`IMPLEMENTED_ACCEPTED`，正式验收已在本地真实环境执行且 17 条全部 `PASS`，`final_acceptance_status=ACCEPTED`）

> 本章为本轮调整的权威需求正文。分层状态：`adjustment_document_status=APPROVED`、`adjustment_baseline_status=APPROVED`、`implementation_authorization_status=GRANTED_IN_THIS_TASK`、`implementation_status=IMPLEMENTED_ACCEPTED`、`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`、`new_adjustment_acceptance_status=PASS_17_OF_17`、`final_acceptance_status=ACCEPTED`、`accepted_scope=DS-REQ-178~188 / DS-AC-183~199`、`accepted_business_implementation_commit=55e6273b74182c408e36b75e09ad21819f33d3e2`、`formal_acceptance_evidence_commit=935478a0966d2ec6fe157c3494d813427b1e05ff`、`chatgpt_remote_formal_acceptance_review=REVIEW_PASS`、`blocking_finding_count=0`、`project_owner_final_acceptance_date=2026-09-20`、`project_owner_visual_review_status=PASS`、`password_false_required_defect_status=FIXED_CONFIRMED`。
>
> 本轮验收用例 `DS-AC-183~199`（17 条）已由 `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FORMAL-ACCEPTANCE-001` 于 2026-09-20 在真实后端 + 真实 Oracle 开发库 + 真实浏览器 + 受控验收数据（`RUN_TAG=FACC002`）下端到端执行：`PASS=17/FAIL=0/BLOCKED=0/NOT_RUN=0`（`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`、`new_adjustment_acceptance_status=PASS_17_OF_17`，该分层值**逐字保留**、不得改写成模糊的 `PASS`）。ChatGPT 从远程 Git 对该证据提交 `935478a0966d2ec6fe157c3494d813427b1e05ff` 完成正式验收复审，结论 `status=REVIEW_PASS`、`acceptance_result=PASS_17_OF_17`、`blocking_finding_count=0`、`project_owner_final_acceptance_recommendation=ACCEPT`；项目负责人据此于 2026-09-20 作出**最终验收接受**决定，本组调整收口为 `IMPLEMENTED_ACCEPTED`/`final_acceptance_status=ACCEPTED`（收口任务 `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FINAL-ACCEPTANCE-CLOSEOUT-001`）。被最终接受的业务实现提交为 `55e6273b...`（完整实现链 `807a5a58... → 55e6273b...`；`807a5a58...` 为初始实现提交，存在已修复的密码误报必填缺陷，**不得**单独记作最终被接受实现）。本次 `ACCEPTED` **严格只适用于** `DS-REQ-178~188`/`DS-AC-183~199` 当前调整，**不代表**数据源管理 Feature 整体正式验收状态或系统生产可用。
>
> `project_owner_visual_review_status=PASS` 仅代表项目负责人 2026-09-20 在真实运行页面完成**页面目测/功能复测并通过**（原话“我试了，新增修改都没有问题了。”）；`password_false_required_defect_status=FIXED_CONFIRMED` 表示该密码误报必填缺陷已修复并经复测确认。
>
> `DS-REQ-001~177` 编号、正文与语义逐字冻结；`DS-REQ-178~188` 编号与正文亦**零变化**（仅本章状态回写）。既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0` 与 `DS-AC-104`/`DS-AC-108` 两个 `BLOCKED` 逐字保留；上一轮 `DS-AC-116~140`（25 条）仍全部 `NOT_RUN`；已最终接受的上一调整 `DS-REQ-139~177`/`DS-AC-141~182`（42 条 `PASS`）状态不变；数据源管理 Feature 整体正式验收状态**不得**因本章的 `ACCEPTED` 而改为 `ACCEPTED`。
>
...
| 2026-09-20 | R3 核心文档状态统一（纯文档）：ChatGPT 从远程 Git 复审 R2 提交 `f42aad0...` 结论 `CHANGES_REQUIRED`（唯一阻塞：七份核心文档对最新一轮调整的**当前状态**不统一——`README.md`/`ACCEPTANCE.md`/`DESIGN.md`/`UI.md` 为 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`，而 `REQUIREMENTS.md §24`/`API.md §13`/`DATABASE.md §11` 仍为 `IMPLEMENTED_PENDING_USER_REVIEW`；该残留源于 R2 授权范围未含后三份文档，非 R2 越权遗漏）。本 R3 将 §24 章节标题与分层状态块的 `implementation_status` 由 `IMPLEMENTED_PENDING_USER_REVIEW` 更新为 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`，并补充 `project_owner_visual_review_status=PASS`、`password_false_required_defect_status=FIXED_CONFIRMED` 及“目测通过 ≠ 正式验收执行、17 条仍全部 `NOT_RUN`”的边界说明。`DS-REQ-178~188` 共 11 条编号与需求正文**零变化**，三段“局部替代声明”及其边界未改；`DS-REQ-001~177` 编号、正文、语义逐字冻结；历史记录中描述当时状态的 `IMPLEMENTED_PENDING_USER_REVIEW`（§22/§23 各轮回写行）作为历史事实**逐字保留、未全局替换**；`DS-AC-183~199`（17 条）仍全部 `NOT_RUN`，`formal_acceptance_execution_status=NOT_RUN`、`new_adjustment_acceptance_status=ALL_NOT_RUN` 保持不变；上一轮 `DS-AC-116~140`（25 条）仍全部 `NOT_RUN` 且状态未变；已最终接受的上一调整 `DS-AC-141~182`（42 条 `PASS`、`IMPLEMENTED_ACCEPTED`、`final_acceptance_status=ACCEPTED`）状态未改变；既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0` 与 `DS-AC-104`/`DS-AC-108` 两个 `BLOCKED` 逐字保留；未写为 `PASS_17_OF_17`/`IMPLEMENTED_ACCEPTED`/`ACCEPTED`/生产可用；数据源管理 Feature 整体正式验收状态未改变；未修改业务代码/测试/配置/依赖/锁文件/SQL；未测试、未构建、未访问数据库/ZK/Kafka/业务源库/目标库；未启停任何服务 | `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R3`（七份核心文档当前状态统一；纯文档任务） |
| 2026-09-20 | 最新一轮调整基线**正式验收执行与状态回写**：§24 章节标题由“（`APPROVED`，`IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`）”改为“（`APPROVED`，`IMPLEMENTED_PENDING_FINAL_ACCEPTANCE`，正式验收已在本地真实环境执行且 17 条全部 `PASS`）”；分层状态由 `implementation_status=IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`/`formal_acceptance_execution_status=NOT_RUN`/`new_adjustment_acceptance_status=ALL_NOT_RUN` 更新为 `implementation_status=IMPLEMENTED_PENDING_FINAL_ACCEPTANCE`/`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`/`new_adjustment_acceptance_status=PASS_17_OF_17`，并新增 `final_acceptance_status=NOT_ACCEPTED_PENDING_PROJECT_OWNER`。执行结论：`DS-AC-183~199`（17 条）于 2026-09-20 在真实后端 + 真实 Oracle 开发库 + 真实浏览器 + 受控验收数据（`RUN_TAG=FACC002`，运行源 `db1cfda7...`）下端到端执行，`PASS=17/FAIL=0/BLOCKED=0/NOT_RUN=0`；数据库写操作经项目负责人明确批准（`GRANTED_BY_PROJECT_OWNER_FOR_AGENT_CREATED_DATA_ONLY`），自建数据已按精确主键白名单清理（残留 `0`），既有 36 条主表与 10 条延伸表记录规范化快照四个 SHA-256 **逐字节一致**。本轮**不给出**最终验收（`final_acceptance_status=NOT_ACCEPTED_PENDING_PROJECT_OWNER`）。`DS-REQ-178~188` 共 11 条编号与需求正文**零变化**，三段“局部替代声明”及其边界未改；`DS-REQ-001~177` 编号、正文、语义逐字冻结；仅 `DS-AC-183~199` 状态由 `NOT_RUN` 更新为 `PASS`；上一轮 `DS-AC-116~140`（25 条）仍全部 `NOT_RUN` 且状态未变；已最终接受的上一调整 `DS-AC-141~182`（42 条 `PASS`、`IMPLEMENTED_ACCEPTED`、`final_acceptance_status=ACCEPTED`）状态未改变；既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0` 与 `DS-AC-104`/`DS-AC-108` 两个 `BLOCKED` 逐字保留；未写为 `ACCEPTED`/`IMPLEMENTED_ACCEPTED`/`final_acceptance_status=ACCEPTED`/生产可用；数据源管理 Feature 整体正式验收状态未改变；未修改业务代码/测试/配置/依赖/锁文件/SQL；未执行 DDL/存量清洗；未访问 ZooKeeper 写、Kafka、业务源库、目标库；§21 变更记录表任务前已有的多余空行**未修复**、如实记录 | `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FORMAL-ACCEPTANCE-001`（正式验收执行 + 状态回写；证据与报告见 `reports/DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FORMAL-ACCEPTANCE-001.md`） |
| 2026-09-20 | 最新一轮调整基线**最终验收接受与收口（纯文档）**：ChatGPT 从远程 Git 对正式验收证据提交 `935478a0966d2ec6fe157c3494d813427b1e05ff` 作出正式验收复审结论 `status=REVIEW_PASS`、`acceptance_result=PASS_17_OF_17`、`blocking_finding_count=0`、`nonblocking_finding_count=2`、`project_owner_final_acceptance_recommendation=ACCEPT`；项目负责人据此于 2026-09-20 作出**最终验收接受**决定，接受范围**仅限** `DS-REQ-178~188`/`DS-AC-183~199` 当前调整。§24 章节标题由“（`APPROVED`，`IMPLEMENTED_PENDING_FINAL_ACCEPTANCE`，正式验收已在本地真实环境执行且 17 条全部 `PASS`）”改为“（`APPROVED`，`IMPLEMENTED_ACCEPTED`，正式验收已在本地真实环境执行且 17 条全部 `PASS`，`final_acceptance_status=ACCEPTED`）”；分层状态 `implementation_status` 由 `IMPLEMENTED_PENDING_FINAL_ACCEPTANCE` 更新为 `IMPLEMENTED_ACCEPTED`、`final_acceptance_status` 由 `NOT_ACCEPTED_PENDING_PROJECT_OWNER` 更新为 `ACCEPTED`，并新增 `accepted_scope`、`accepted_business_implementation_commit=55e6273b...`、`formal_acceptance_evidence_commit=935478a...`、`chatgpt_remote_formal_acceptance_review=REVIEW_PASS`、`blocking_finding_count=0`、`project_owner_final_acceptance_date=2026-09-20`；`formal_acceptance_execution_status=EXECUTED_PASSED_LOCAL`、`new_adjustment_acceptance_status=PASS_17_OF_17`、`project_owner_visual_review_status=PASS`、`password_false_required_defect_status=FIXED_CONFIRMED` **逐字保留**。被最终接受的业务实现提交记录为 `55e6273b...`（完整实现链 `807a5a58... → 55e6273b...`）。`DS-REQ-178~188` 共 11 条编号与需求正文**零变化**，三段“局部替代声明”及其边界未改；`DS-REQ-001~177` 编号、正文、语义逐字冻结；`DS-AC-183~199`（17 条）编号、关联需求、前置条件、步骤、预期结果与 17 条 `PASS` 零变化；**该 `ACCEPTED` 严格只适用于本组当前调整，数据源管理 Feature 整体正式验收状态不得改为 `ACCEPTED`**；上一轮 `DS-AC-116~140`（25 条）仍全部 `NOT_RUN` 且状态未变；已最终接受的上一调整 `DS-AC-141~182`（42 条 `PASS`、`IMPLEMENTED_ACCEPTED`）状态未改变；既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0` 与 `DS-AC-104`/`DS-AC-108` 两个 `BLOCKED` 逐字保留；§21 变更记录表任务前已有的多余空行**未修复**；未修改业务代码/测试/配置/依赖/锁文件/SQL；未访问数据库/ZK/Kafka/业务源库/目标库；未测试、未构建、未启停服务 | `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-FINAL-ACCEPTANCE-CLOSEOUT-001`（最终验收接受与状态回写；纯文档任务） |
