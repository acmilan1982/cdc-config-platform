...
## 22. 列表首页选择性接入查询列表页公共组件调整需求（APPROVED）

> 状态：`APPROVED`。本轮调整基线分层状态：`adjustment_baseline_status=APPROVED`、`adjustment_requirements_status=APPROVED`、`implementation_status=NOT_STARTED`、`implementation_authorization_status=NOT_GRANTED_IN_THIS_TASK`、`acceptance_execution_status=ALL_NOT_RUN`。
...
> 批准只代表**需求正式成立**，**不代表**已实现、已测试、已验收或生产可用：本轮实现状态仍为 `NOT_STARTED`，实现授权为 `NOT_GRANTED_IN_THIS_TASK`，新增加的 `DS-AC-116~140`（25 条）仍全部 `NOT_RUN`（`ALL_NOT_RUN`）。
