...
- **范围边界（如实记录）**：本 R2 允许修改范围仅含 `DESIGN.md`/`README.md`/`UI.md`/`ACCEPTANCE.md` 与新增 R2 报告；`REQUIREMENTS.md` §24、`API.md` §13、`DATABASE.md` §11 的 `implementation_status` 仍为 `IMPLEMENTED_PENDING_USER_REVIEW`，**未随本 R2 更新**，需另行授权任务统一，避免残留旧值。

## 3. 文档导航

| 文档 | 职责 | 当前状态 |
|---|---|---|
| [REQUIREMENTS.md](./REQUIREMENTS.md) | 需求基线；§22 为上一轮调整基线需求 `DS-REQ-116~138`；§23 为当前调整基线需求 `DS-REQ-139~177`；§24 为最新一轮调整基线需求 `DS-REQ-178~188` | 既有 `APPROVED`；§22 `APPROVED`；§23 `APPROVED`；§24 `APPROVED`，实现状态 `IMPLEMENTED_PENDING_USER_REVIEW` |
...
| [API.md](./API.md) | API 设计基线；§9 为上一轮角色查询参数；§11 为当前启停接口与列表状态字段；§13 为最新一轮时间字段与排序的接口影响 | 既有 `APPROVED`；§9 `APPROVED`；§11 `APPROVED`；§13 `APPROVED` |
...
| [DATABASE.md](./DATABASE.md) | 数据库设计基线；§8 为上一轮无数据库变化声明；§9 为当前无数据库变化声明；§11 为最新一轮时间字段与排序的数据库变化声明 | 既有 `APPROVED`；§8 `APPROVED`；§9 `APPROVED`；§11 `APPROVED`，实现后无数据库变化 |
