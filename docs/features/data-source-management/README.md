...
>
> **最新一轮调整基线（已批准、已实现、正式验收尚未执行）**：`DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001`，范围「`CDC_DATA_SOURCE` 新增/修改时间字段维护与列表默认排序」+「新增/编辑主弹窗标签对齐、标签文字样式、密码必填标识与创建/保存按钮视觉」，状态 `APPROVED`（项目负责人已在当前会话明确批准），已由本任务实现：`DS-REQ-178~188`（11 条）、`DS-AC-183~199`（17 条，**全部 `NOT_RUN`**），实现状态 `IMPLEMENTED_PENDING_USER_REVIEW`、正式验收执行状态 `NOT_RUN`，见 §2.4。该基线**不影响** §2.1/§2.2/§2.3 的已批准与已接受状态，也**不改变**数据源管理 Feature 整体正式验收状态。
...
### 2.4 最新一轮调整基线（新增/修改时间字段维护、列表默认排序与主弹窗表单视觉调整）（`APPROVED`，`IMPLEMENTED_PENDING_USER_REVIEW`）
...
- 本方案已由项目负责人在当前会话**明确批准**；实现已由本任务完成（`implementation_authorization_status=GRANTED_IN_THIS_TASK`），实现状态为 `IMPLEMENTED_PENDING_USER_REVIEW`。
...
- 下一步为 ChatGPT 从远程 Git 独立复审实现提交，再由项目负责人对新增/编辑主弹窗进行**页面目测**；此后另行决定是否正式执行 `DS-AC-183~199`。
- **项目负责人页面目测（2026-09-20）**：`project_owner_visual_review_status=FAILED_FOUND_DEFECT_THEN_R1_FIXED_PENDING_RETEST`。项目负责人在真实页面目测新增数据源弹窗时发现：密码**已填写**，点击“创建”仍被前端拦截，密码下方显示英文 `password is required`。已由 R1 定向修复（`DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R1`，详见 [UI.md](./UI.md) §13.3、§14.2 与 `reports/DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R1.md`），**修复后尚未重新目测**，等待项目负责人复测。
...
**最新一轮调整基线（§2.4，`APPROVED`，`IMPLEMENTED_PENDING_USER_REVIEW`）对应章节**：[REQUIREMENTS.md](./REQUIREMENTS.md) §24（`DS-REQ-178~188`）、[ACCEPTANCE.md](./ACCEPTANCE.md) §4.18（`DS-AC-183~199`，全部 `NOT_RUN`）、[DESIGN.md](./DESIGN.md) §15、[API.md](./API.md) §13、[UI.md](./UI.md) §13、[DATABASE.md](./DATABASE.md) §11，均已随本轮调整基线批准为 `APPROVED` 并已实现（实现状态 `IMPLEMENTED_PENDING_USER_REVIEW`，正式验收执行状态 `NOT_RUN`）。上表“当前状态”列中既有基线（§2.1）、上一轮调整基线（§2.2）与当前调整基线（§2.3）的状态未因本轮的批准与实现而改变。
