...
>
> **最新一轮调整基线（已批准、已实现、项目负责人页面目测已通过、正式验收尚未执行）**：`DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001`，范围「`CDC_DATA_SOURCE` 新增/修改时间字段维护与列表默认排序」+「新增/编辑主弹窗标签对齐、标签文字样式、密码必填标识与创建/保存按钮视觉」，状态 `APPROVED`（项目负责人已在当前会话明确批准），已由本任务实现：`DS-REQ-178~188`（11 条）、`DS-AC-183~199`（17 条，**全部 `NOT_RUN`**），实现状态 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`、正式验收执行状态 `NOT_RUN`、项目负责人页面目测 `PASS`（2026-09-20），见 §2.4。该基线**不影响** §2.1/§2.2/§2.3 的已批准与已接受状态，也**不改变**数据源管理 Feature 整体正式验收状态。
...
### 2.4 最新一轮调整基线（新增/修改时间字段维护、列表默认排序与主弹窗表单视觉调整）（`APPROVED`，`IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`）

```text
latest_adjustment_task=DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001
adjustment_document_status=APPROVED
adjustment_baseline_status=APPROVED
...
chatgpt_remote_r1_code_review_status=REVIEW_PASS
chatgpt_remote_r1_overall_review_status=CHANGES_REQUIRED_DOCUMENTATION_ONLY
chatgpt_remote_r1_blocking_finding_count=1_RESOLVED_BY_R2
```

- 本轮为 `/config/data-source` 的调整基线，性质为 `APPROVED_FEATURE_ADJUSTMENT_IMPLEMENTATION`；范围是「`CDC_DATA_SOURCE` 新增/修改时间字段维护与列表默认排序」+「新增/编辑主弹窗标签对齐、标签文字样式、密码必填标识与创建/保存按钮视觉」。
- 新增需求 `DS-REQ-178~188`（11 条）、新增验收 `DS-AC-183~199`（17 条，**全部 `NOT_RUN`**）；详见 [REQUIREMENTS.md](./REQUIREMENTS.md) §24 与 [ACCEPTANCE.md](./ACCEPTANCE.md) §4.18。
- 本方案已由项目负责人在当前会话**明确批准**；实现已由本任务完成（`implementation_authorization_status=GRANTED_IN_THIS_TASK`），实现状态为 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`（**不是**已不准确的 `IMPLEMENTED_PENDING_USER_REVIEW`——项目负责人 2026-09-20 已完成页面目测/功能复测并通过）。
- **状态速览（与 §2.2、§2.3 严格区分）**：本节为**最新一轮**调整基线，`DS-AC-183~199`（17 条）全部 `NOT_RUN`、正式验收执行状态 `NOT_RUN`；§2.3（`DS-AC-141~182`，42 条 `PASS`、`IMPLEMENTED_ACCEPTED`）与 §2.2（`DS-AC-116~140`，25 条全部 `NOT_RUN`）状态**均未改变**。三组状态互相独立、不得合并，也不得用“本轮”笼统指代。
- 本轮**零 DDL**、**零存量数据清洗**、**零新增索引**；排序语义替换不要求任何存量回填；**未访问**数据库 / ZooKeeper / Kafka / 业务源库 / 目标库；**未修改**公共查询列表组件、其他路由/Feature、依赖或配置。
- 下一步为另行决定是否正式执行 `DS-AC-183~199`。
- **项目负责人页面目测（2026-09-20，已通过）**：`project_owner_visual_review_status=PASS`、`create_function_visual_review_status=PASS`、`edit_function_visual_review_status=PASS`、`password_false_required_defect_status=FIXED_CONFIRMED`。
  - **过程**：项目负责人先于真实页面目测新增数据源弹窗时发现问题——密码**已填写**，点击“创建”仍被前端拦截，密码下方显示英文 `password is required`（`project_owner_visual_review_status=FAILED_FOUND_DEFECT_THEN_R1_FIXED_PENDING_RETEST`）。该缺陷已由 R1 定向修复（`DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R1`，详见 [UI.md](./UI.md) §13.3、§14.2、[DESIGN.md](./DESIGN.md) §15.5 与 `reports/DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R1.md`），R2 已同步 `DESIGN.md` §15.4/§15.5 与最终代码的一致性。
  - **复测依据**：项目负责人于 2026-09-20 使用 R1 运行页面完成复测后明确回复“我试了，新增修改都没有问题了。”。据此记录新增与编辑功能目测均 `PASS`，密码误报必填缺陷状态 `FIXED_CONFIRMED`。
  - **边界**：`PASS` **仅**代表本轮项目负责人页面目测/功能复测通过，**不等于**正式验收执行；`DS-AC-183~199`（17 条）仍**全部** `NOT_RUN`。
- 本条 `ACCEPTED` 概念**不适用于**本轮：本轮**不得**写为 `PASS`、`IMPLEMENTED_ACCEPTED`、`ACCEPTED` 或生产可用；数据源管理 Feature 整体正式验收状态亦不因本轮改变。
- **R2 文档纠偏（2026-09-20）**：ChatGPT 从远程 Git 复审 R1 提交 `55e6273b...` 结论为 `code_review_status=REVIEW_PASS`、`overall_review_status=CHANGES_REQUIRED`、`blocking_finding_count=1`（唯一阻塞 `DESIGN_DOCUMENT_INCONSISTENCY`：`DESIGN.md` §15.4/§15.5 残留已废止的旧方案）。已由 `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R2` 定向修订 `DESIGN.md` §15.4/§15.5 与本节状态块（纯文档，详见 [DESIGN.md](./DESIGN.md) §16.2 与 `reports/DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R2.md`）。
- **残余状态已由 R3 统一（2026-09-20）**：R2 因授权范围仅含 `DESIGN.md`/`README.md`/`UI.md`/`ACCEPTANCE.md`，`REQUIREMENTS.md` §24、`API.md` §13、`DATABASE.md` §11 的 `implementation_status` 当时仍为 `IMPLEMENTED_PENDING_USER_REVIEW`。该残留已由 `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R3` 定向统一为 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`（详见下方 §5 变更记录与 `reports/DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R3.md`）；七份核心文档对最新一轮调整的**当前状态**现已一致。上一轮（§2.2）的 `IMPLEMENTED_PENDING_USER_REVIEW` 属**另一组基线**的历史与当前状态，**未**被本次统一改动。

## 3. 文档导航

| 文档 | 职责 | 当前状态 |
|---|---|---|
| [REQUIREMENTS.md](./REQUIREMENTS.md) | 需求基线；§22 为上一轮调整基线需求 `DS-REQ-116~138`；§23 为当前调整基线需求 `DS-REQ-139~177`；§24 为最新一轮调整基线需求 `DS-REQ-178~188` | 既有 `APPROVED`；§22 `APPROVED`，实现状态 `IMPLEMENTED_PENDING_USER_REVIEW`；§23 `APPROVED`，`IMPLEMENTED_ACCEPTED`；§24 `APPROVED`，实现状态 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE` |
| [ACCEPTANCE.md](./ACCEPTANCE.md) | 验收基线；§4.16 为上一轮调整基线用例 `DS-AC-116~140`；§4.17 为当前调整基线验收标准 `DS-AC-141~182`；§4.18 为最新一轮调整基线验收标准 `DS-AC-183~199` | 既有 `APPROVED`；§4.16 `APPROVED`，全部 `NOT_RUN`；§4.17 `APPROVED`，42 条已正式验收执行：`PASS=42/FAIL=0/BLOCKED=0/NOT_RUN=0`；§4.18 `APPROVED`，17 条全部 `NOT_RUN` |
| [DESIGN.md](./DESIGN.md) | 设计基线；§11 为上一轮调整基线设计；§13 为当前调整基线设计；§15 为最新一轮调整基线设计 | 既有 `APPROVED`；§11 `APPROVED`；§13 `APPROVED`；§15 `APPROVED` |
| [API.md](./API.md) | API 设计基线；§9 为上一轮角色查询参数；§11 为当前启停接口与列表状态字段；§13 为最新一轮时间字段与排序的接口影响 | 既有 `APPROVED`；§9 `APPROVED`，实现状态 `IMPLEMENTED_PENDING_USER_REVIEW`；§11 `APPROVED`，`IMPLEMENTED_ACCEPTED`；§13 `APPROVED`，实现状态 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE` |
| [UI.md](./UI.md) | UI 设计基线；§10 为上一轮列表首页设计；§11 为当前列表全部状态与启停视觉微调；§13 为最新一轮新增/编辑主弹窗表单视觉调整 | 既有 `APPROVED`；§10 `APPROVED`；§11 `APPROVED`；§13 `APPROVED` |
| [DATABASE.md](./DATABASE.md) | 数据库设计基线；§8 为上一轮无数据库变化声明；§9 为当前无数据库变化声明；§11 为最新一轮时间字段与排序的数据库变化声明 | 既有 `APPROVED`；§8 `APPROVED`，实现状态 `IMPLEMENTED_PENDING_USER_REVIEW`；§9 `APPROVED`，`IMPLEMENTED_ACCEPTED`；§11 `APPROVED`，实现后无数据库变化，实现状态 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE` |
...
**最新一轮调整基线（§2.4，`APPROVED`，`IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`）对应章节**：[REQUIREMENTS.md](./REQUIREMENTS.md) §24（`DS-REQ-178~188`）、[ACCEPTANCE.md](./ACCEPTANCE.md) §4.18（`DS-AC-183~199`，全部 `NOT_RUN`）、[DESIGN.md](./DESIGN.md) §15、[API.md](./API.md) §13、[UI.md](./UI.md) §13、[DATABASE.md](./DATABASE.md) §11，均已随本轮调整基线批准为 `APPROVED` 并已实现（实现状态 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`，正式验收执行状态 `NOT_RUN`，项目负责人 2026-09-20 页面目测 `PASS`）。上表“当前状态”列中既有基线（§2.1）、上一轮调整基线（§2.2）与当前调整基线（§2.3）的状态未因本轮的批准与实现而改变。
...
| 2026-09-20 | 新增/修改时间字段维护、列表默认排序与主弹窗表单视觉调整：新增 §2.4「最新一轮调整基线（新增/修改时间字段维护、列表默认排序与主弹窗表单视觉调整）（`APPROVED`，`IMPLEMENTED_PENDING_USER_REVIEW`）」，登记 `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001` 的六个分层状态键（`adjustment_document_status=APPROVED`、`adjustment_baseline_status=APPROVED`、`implementation_status=IMPLEMENTED_PENDING_USER_REVIEW`、`implementation_authorization_status=GRANTED_IN_THIS_TASK`、`formal_acceptance_execution_status=NOT_RUN`、`new_adjustment_acceptance_status=ALL_NOT_RUN`）、新增需求 `DS-REQ-178~188`（11 条）与新增验收 `DS-AC-183~199`（17 条，全部 `NOT_RUN`）；文档头部新增“最新一轮调整基线（已批准、已实现、正式验收尚未执行）”段并以“最新一轮 / 当前 / 上一轮”明确区分三组状态；§3 文档导航表六行补充本轮对应章节（`REQUIREMENTS §24`/`ACCEPTANCE §4.18`/`DESIGN §15`/`API §13`/`UI §13`/`DATABASE §11`）并新增“最新一轮调整基线对应章节”段；§2.1 既有基线状态、§2.2（`DS-AC-116~140` 25 条全部 `NOT_RUN`、`IMPLEMENTED_PENDING_USER_REVIEW`）与 §2.3（`DS-AC-141~182` 42 条 `PASS`、`IMPLEMENTED_ACCEPTED`、`final_acceptance_status=ACCEPTED`）**均未改变**，既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0` 与 `DS-AC-104`/`DS-AC-108` 两个 `BLOCKED` 逐字保留；本轮**不得**写为 `PASS`/`IMPLEMENTED_ACCEPTED`/`ACCEPTED`/生产可用 | `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001`（已批准调整基线的前后端实现、自动化测试、构建与文档状态回写；未访问数据库/ZK/Kafka/源库/目标库；未执行 DDL/存量清洗；从最终提交启动临时服务供项目负责人目测） |
...
| 2026-09-20 | 最新一轮调整基线 **R1 定向修复（项目负责人真实页面目测发现缺陷）**：项目负责人在真实页面目测新增数据源弹窗时发现——密码**已填写**，点击“创建”仍被前端拦截，密码下方显示英文 `password is required`。根因：§13.3 初版在密码 `el-form-item` 上使用 `:required="!isEdit"`，Element Plus 据此生成针对 `prop="password"` 的隐式必填规则，该校验 `editorForm.password`；而密码输入实际绑定独立状态 `passwordInput`（不在 `editorForm` 内），故即使已输入密码仍被框架英文默认提示拦截。修复：删除该 `required` 属性，改为纯视觉局部类 `editor-password-required-mark` + 局部 CSS 伪元素星号，**未**改绑表单模型、**未**改动 `validatePassword()` 与既有密码安全逻辑。§2.4 状态块新增 `project_owner_visual_review_status=FAILED_FOUND_DEFECT_THEN_R1_FIXED_PENDING_RETEST` 并追加目测与修复说明；[UI.md](./UI.md) §13.3 按最终机制改写、§14.2 追加 R1 修复链；新增报告 `reports/DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R1.md`。`implementation_status=IMPLEMENTED_PENDING_USER_REVIEW`、`formal_acceptance_execution_status=NOT_RUN`、`new_adjustment_acceptance_status=ALL_NOT_RUN` **保持不变**；`DS-REQ-178~188` 保持原批准内容、不新增需求/验收编号；`DS-AC-183~199`（17 条）仍全部 `NOT_RUN`；既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0`、`DS-AC-104`/`DS-AC-108` 两个 `BLOCKED`、`DS-AC-116~140`（25 条 `NOT_RUN`）与 `DS-AC-141~182`（42 条 `PASS`）逐字保留；数据源管理 Feature 整体正式验收状态**未**改为 `ACCEPTED`；未置 `PASS`/`ACCEPTED`/`IMPLEMENTED_ACCEPTED`/生产可用；未修改后端业务代码/数据库/接口契约/依赖/锁文件；未访问数据库/ZK/Kafka/源库/目标库；**修复后尚未重新目测**，等待项目负责人复测 | `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R1`（新增模式密码误报必填缺陷定向修复、回归测试与构建、文档状态回写；从最终提交重启临时前端供项目负责人复测） |
| 2026-09-20 | 最新一轮调整基线 **R2 设计文档纠偏与目测结果回写（纯文档）**：ChatGPT 从远程 Git 复审 R1 提交 `55e6273b...` 结论 `code_review_status=REVIEW_PASS`、`overall_review_status=CHANGES_REQUIRED`、`blocking_finding_count=1`（唯一阻塞 `DESIGN_DOCUMENT_INCONSISTENCY`：`DESIGN.md` §15.4 仍笼统写星号由 `is-required` 渲染、§15.5 仍要求密码表单项使用 `:required="!isEdit"`）。R2 修订：`DESIGN.md` §15.4 改为“普通必填字段仍可由 `is-required` 渲染、密码字段为明确例外”，§15.5 整节替换为 R1 已落地的最终方案（保留 `prop="password"` 并删除 `required`、新增模式挂载 `editor-password-required-mark` 局部类、scoped CSS `::before` 渲染星号、只控制视觉无校验副作用、空密码仍提示 `请输入密码`、编辑模式无星号且未修改可保存、掩码与请求体规则不变）并补充 R1 根因说明，另追加 §16.2 变更记录。**§2.4 状态块**按项目负责人 2026-09-20 复测通过更新：`implementation_status` 由 `IMPLEMENTED_PENDING_USER_REVIEW` 更新为 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`，`project_owner_visual_review_status` 由 `FAILED_FOUND_DEFECT_THEN_R1_FIXED_PENDING_RETEST` 更新为 `PASS`，新增 `create_function_visual_review_status=PASS`、`edit_function_visual_review_status=PASS`、`password_false_required_defect_status=FIXED_CONFIRMED` 及 ChatGPT 复审三项键；复测依据为项目负责人原话“我试了，新增修改都没有问题了。”。**目测通过不等于正式验收**：`DS-AC-183~199`（17 条）仍**全部** `NOT_RUN`、`formal_acceptance_execution_status=NOT_RUN`、`new_adjustment_acceptance_status=ALL_NOT_RUN`；`DS-REQ-178~188` 编号与正文零变化，`DS-AC-001~199` 编号、关联需求、前置条件、操作步骤、预期结果零变化；既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0`、`DS-AC-104`/`DS-AC-108` 两个 `BLOCKED`、上一轮 `DS-AC-116~140`（25 条全部 `NOT_RUN`）与已最终接受的 `DS-AC-141~182`（42 条 `PASS`）逐字保留；数据源管理 Feature 整体正式验收状态**未**改变；未置 `PASS_17_OF_17`/`ACCEPTED`/`IMPLEMENTED_ACCEPTED`/生产可用；未修改任何运行代码/测试/配置/依赖/锁文件/SQL；未访问数据库/ZK/Kafka/源库/目标库；未执行测试或构建；未启停任何服务；初版与 R1 报告作为历史证据逐字保留 | `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R2`（设计文档纠偏与项目负责人目测结果回写；纯文档任务） |
| 2026-09-20 | 最新一轮调整基线 **R3 核心文档状态统一（纯文档）**：ChatGPT 从远程 Git 复审 R2 提交 `f42aad0...` 结论 `CHANGES_REQUIRED`（唯一阻塞：七份核心文档对最新一轮调整的**当前状态**不统一——本入口文档、`ACCEPTANCE.md`、`DESIGN.md`、`UI.md` 为 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`，而 `REQUIREMENTS.md §24`、`API.md §13`、`DATABASE.md §11` 仍为 `IMPLEMENTED_PENDING_USER_REVIEW`；该残留源于 R2 授权范围未含后三份文档，非 R2 越权遗漏）。R3 修订：后三份文档的**当前章节**标题与分层状态块 `implementation_status` 统一为 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE` 并补充 `project_owner_visual_review_status=PASS`、`password_false_required_defect_status=FIXED_CONFIRMED` 与“目测通过 ≠ 正式验收执行”边界，各自追加变更记录（`REQUIREMENTS.md` §21、`API.md` §14.2、`DATABASE.md` §12.2）；本入口 §2.4 原“范围边界（如实记录）”条目改为“残余状态已由 R3 统一”，§3 文档导航表 `REQUIREMENTS.md`/`API.md`/`DATABASE.md` 三行的最新一轮当前状态同步为 `IMPLEMENTED_PENDING_FORMAL_ACCEPTANCE`。`DS-REQ-178~188` 编号与正文零变化，`DS-AC-183~199` 编号、关联需求、前置条件、操作步骤、预期结果零变化且仍**全部 `NOT_RUN`**；`formal_acceptance_execution_status=NOT_RUN`、`new_adjustment_acceptance_status=ALL_NOT_RUN` 保持不变；上一轮 §2.2（`DS-AC-116~140` 25 条 `NOT_RUN`、`IMPLEMENTED_PENDING_USER_REVIEW`）与当前调整 §2.3（`DS-AC-141~182` 42 条 `PASS`、`IMPLEMENTED_ACCEPTED`、`final_acceptance_status=ACCEPTED`）**状态均未改变**；历史记录中描述当时状态的 `IMPLEMENTED_PENDING_USER_REVIEW` 逐字保留、未全局替换；既有 `PASS=113/FAIL=0/BLOCKED=2/NOT_RUN=0` 与 `DS-AC-104`/`DS-AC-108` 两个 `BLOCKED` 逐字保留；未写为 `PASS_17_OF_17`/`IMPLEMENTED_ACCEPTED`/`ACCEPTED`/生产可用；数据源管理 Feature 整体正式验收状态未改变；未修改业务代码/测试/配置/依赖/锁文件/SQL；未测试、未构建、未访问数据库/ZK/Kafka/业务源库/目标库；未启停任何服务；R1/R2 报告作为历史证据保持原样 | `DATA-SOURCE-CREATE-EDIT-TIME-SORT-FORM-UI-ADJUSTMENT-001-R3`（七份核心文档当前状态统一；纯文档任务） |
