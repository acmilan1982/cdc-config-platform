...
## 13. 列表展示全部状态、启用/停用及视觉微调设计（`DRAFT_PENDING_USER_REVIEW`，未实现）

> 本轮调整基线分层状态：`adjustment_document_status=DRAFT_PENDING_USER_REVIEW`、`adjustment_baseline_status=DRAFT_PENDING_USER_REVIEW`、`implementation_status=NOT_STARTED`、`implementation_authorization_status=NOT_GRANTED_IN_THIS_TASK`、`formal_acceptance_execution_status=NOT_RUN`、`new_adjustment_acceptance_status=ALL_NOT_RUN`。
>
> 任务：`DATA-SOURCE-LIST-ALL-STATUS-ENABLE-DISABLE-UI-ADJUSTMENT-BASELINE-001`（`task_type=FEATURE_ADJUSTMENT_BASELINE_DRAFT`，纯文档任务）。
>
...
- 事务边界：单条主表记录的**单条 `UPDATE`**，短事务；`enable` 在写入前**先读取**当前 `FG_ACTIVE` 以实施上表状态机；`disable` 可依赖 `UPDATE ... SET FG_ACTIVE='0' WHERE DATA_SOURCE_ID=?` 自身完成归一化。
...
- 并发：**不加锁**、**不使用**乐观版本字段、**不引入**新的并发控制；并发或先后请求最终**收敛**到其中一个请求的目标值；文档**不承诺**“至多一次成功”，也**不承诺**请求顺序与最终值一一对应（`DS-REQ-171`）。
...
| `UPDATE` 影响行数 ≠ 1 | `50002`（`STATUS_FAILED`，复用既有语义并**恢复启用**该码），事务**回滚**，不留中间状态 |
...
- `40400` → 提示目标数据源不存在，并按当前已应用条件刷新列表。
- `40250` → 提示该记录状态异常、只能先停用归一化；刷新列表。
- `50002`/`50000` → 提示操作失败、请重试；**保留**当前列表与已应用条件、恢复 busy。
...
- **成功/失败处理**：成功后按**当前已应用查询条件**重新查询（不读取尚未点击“查询”的草稿条件）；因列表展示全部状态，停用成功后该行**仍保留**并出现“停用”标识，启用成功后标识消失；失败时保留当前列表与已应用条件、恢复 busy、显示明确错误。
