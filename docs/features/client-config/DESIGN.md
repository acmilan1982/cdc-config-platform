...
| 第五轮弹窗间距／启停确认文案／列表单行选中调整基线状态 | `adjustment5_baseline_status=DRAFT_PENDING_USER_REVIEW`（依据**项目负责人已明确确认的三项决定**建立的**草案**；R0 提交 `3f1fdd4` 经 ChatGPT 远程复审、结论 `CHANGES_REQUIRED`，已由 R1 定向纠错任务 `CLIENT-CONFIG-DIALOG-SPACING-ROW-SELECTION-AND-CONFIRM-COPY-BASELINE-001-R1` 作三处定向修订；**尚未**获项目负责人批准，见 §17） |
| 第五轮调整批准状态 | `adjustment5_approval_status=NOT_APPROVED`（`adjustment5_approval_date=NOT_APPLICABLE`、`adjustment5_approved_reviewed_commit=NOT_APPLICABLE`；项目负责人的三项决定确认**不**等于本节草案已批准） |
| 第五轮调整实现状态 | `adjustment5_implementation_status=NOT_STARTED`（本轮为**纯文档**任务，**未**修改任何前端／后端代码或测试，**未**运行测试／构建／浏览器／服务） |
| 第五轮正式验收执行状态 | `adjustment5_formal_acceptance_execution_status=NOT_RUN`（`CCFG-AC-136~146` 共 11 条全部 `NOT_RUN`；本轮**仅定义验收，不运行正式验收**） |
| 第五轮现行下一入口 | `CHATGPT_REMOTE_CLIENT_CONFIG_DIALOG_SPACING_ROW_SELECTION_CONFIRM_COPY_BASELINE_R1_REVIEW`（R1 远程文档复审入口；其前身 `CHATGPT_REMOTE_CLIENT_CONFIG_DIALOG_SPACING_ROW_SELECTION_CONFIRM_COPY_BASELINE_REVIEW`（R0 草案复审）已给出 `CHANGES_REQUIRED`，属**历史入口**；复审通过后**仍需项目负责人批准本轮草案**，才能另行进入代码实现） |
...
| CCFG-DESIGN-018 | 删除流程（E5）：以探针 ID 定位，在短事务内直接物理 `DELETE` 该记录；不检查、不修改、不级联该探针与其他表/进程/ZooKeeper/Kafka 的关系；删除行数必须为 1，否则回滚并报“探针不存在或已删除”。删除成功后前端刷新（重新加载）列表（**【本轮定向修订 · 待批准】** 原“并清空选中”因本轮取消全部选择能力已不适用，由 §13 `CCFG-DESIGN-043` 与 `REQUIREMENTS.md` `CCFG-REQ-020/094` 取代；页面本无选中状态可清空）。 **【本轮（第五轮）定向修订 · 待批准】** 上句“删除成功后前端刷新（重新加载）列表”在**第五轮**恢复“点击固定单行选中视觉”后需明确选中口径：删除成功触发的重载**清除**此前的固定选中（由 §17 `CCFG-DESIGN-081`／`CCFG-REQ-146` 定义），删除**取消或失败**不改变原有固定选中；物理 `DELETE`、行数必须为 1 否则回滚报“探针不存在或已删除”、不检查不级联其他表／进程／ZooKeeper／Kafka 的既有业务口径**全部不变**。本项原文作为历史决定**保留不改写**，仅在此标注本轮新增的选中口径，不伪装为原规则从未存在。 | CCFG-REQ-026、CCFG-REQ-027、CCFG-REQ-028 | CCFG-AC-020、CCFG-AC-021 |
| CCFG-DESIGN-019 | 停用流程（E7）：二次确认由前端负责；后端在短事务内仅把目标记录 `FG_ACTIVE` 更新为 `0`，行数必须为 1；历史数据源异常（停用/不存在/含逗号）不阻断停用。 **【本轮（第五轮）定向修订 · 待批准】** 上句“二次确认由前端负责”**保留不变**；其确认**正文**由第五轮 §17 `CCFG-DESIGN-075`／`CCFG-UI-020` 定向收窄为 `确定停用探针 {探针ID} 吗？`（标题“停用探针”、按钮“取消／停用”），页面**不**再出现“停用后该探针不再按启用状态命中”一类表述，也**不**得声称停用会立即停止探针进程或采集任务。后端在短事务内**仅**把 `FG_ACTIVE` 更新为 `0`、行数必须为 1、历史数据源异常（停用／不存在／含逗号）不阻断停用的**既有口径全部不变**。本项原文作为历史决定**保留不改写**。 | CCFG-REQ-030、CCFG-REQ-032、CCFG-REQ-035 | CCFG-AC-023、CCFG-AC-024、CCFG-AC-027 |
| CCFG-DESIGN-020 | 启用流程（E6）：一般不弹确认；后端在单个普通短事务内、目标 `UPDATE` 前重新读取 `CDC_CLIENT_MULTIPLE` 全部记录：读取目标记录并校验状态（非 `0/1` 直接拒绝，见 CCFG-DESIGN-021），执行与新增/编辑相同的当次数据源唯一分配检查（自排除目标记录自身）；仅重复分配冲突阻断启用（`40941`），其他数据源异常不阻断 → 未发现阻断则仅把 `FG_ACTIVE` 更新为 `1`，行数必须为 1，任一步失败整笔回滚。该当次检查不消除检查与写入之间的并发竞态。防止历史异常记录绕过新增/编辑规则。 **【本轮（第五轮）定向修订 · 待批准】** 上句“**一般不弹确认**”由第五轮 §17 `CCFG-DESIGN-076`（`CCFG-REQ-141`）**定向修订**：本页启用**新增**一次二次确认（`确定启用探针 {探针ID} 吗？`，标题“启用探针”、按钮“取消／启用”），**取消确认不调用启用接口**；这**不是**替换一句已有文案，而是**新增一个确认步骤**，新旧适用时序对照为“**旧**：单击启用直接调 E6、无确认（`CCFG-REQ-031`／`CCFG-UI-018` 的既有免确认口径）；**新**：单击启用先弹确认，确认后才调 E6，一次点击**不得**触发多次请求”。其后所载后端口径——短事务、写前重读与状态校验、唯一分配检查、仅 `FG_ACTIVE` 更新为 `1`、行数必须为 1、任一步失败整笔回滚——**全部不变**。本项原文作为历史决定**保留不改写**，仅在此标注被修订范围，不伪装为原规则从未存在。 | CCFG-REQ-031、CCFG-REQ-032、CCFG-REQ-035、CCFG-REQ-072 | CCFG-AC-022、CCFG-AC-024、CCFG-AC-027、CCFG-AC-059 |
...
| CCFG-REQ-137 | CCFG-DESIGN-072、CCFG-UI-060 |
| CCFG-REQ-138 | CCFG-DESIGN-073、CCFG-UI-061 |
| CCFG-REQ-139 | CCFG-DESIGN-074、CCFG-UI-062 |
| CCFG-REQ-140 | CCFG-DESIGN-075、CCFG-UI-063 |
| CCFG-REQ-141 | CCFG-DESIGN-076、CCFG-UI-064 |
| CCFG-REQ-142 | CCFG-DESIGN-077、CCFG-UI-065 |
| CCFG-REQ-143 | CCFG-DESIGN-078、CCFG-UI-066 |
| CCFG-REQ-144 | CCFG-DESIGN-079、CCFG-UI-067 |
...
| CCFG-AC-136 | CCFG-DESIGN-072、CCFG-UI-060 |
| CCFG-AC-137 | CCFG-DESIGN-073、CCFG-UI-061 |
| CCFG-AC-138 | CCFG-DESIGN-074、CCFG-UI-062 |
| CCFG-AC-139 | CCFG-DESIGN-075、CCFG-UI-063 |
| CCFG-AC-140 | CCFG-DESIGN-076、CCFG-UI-064 |
| CCFG-AC-141 | CCFG-DESIGN-077、CCFG-UI-065 |
| CCFG-AC-142 | CCFG-DESIGN-078、CCFG-UI-066 |
| CCFG-AC-143 | CCFG-DESIGN-079、CCFG-UI-067 |
...
| CCFG-DESIGN-040 | 结果区头部组合与**取消全部选择能力**：结果区头部采用“左侧摘要 / 右侧操作”布局，“新增探针”按钮置于头部**最右侧**（与数据源管理参考页的右侧新增按钮位置一致）；移除原左侧工具栏写操作按钮；**取消**“删除所选”按钮及与之绑定的禁用判定与选中计数显示逻辑。同时按项目负责人本轮决定**取消全部选择能力**：主列表不提供任何行单选机制，不保留选择状态/选择事件/选择键/已选行集合/当前选中行概念，不渲染选中行背景、高亮、左侧强调线或其他“当前选中”视觉，也不显示“已选择：{探针ID}”或等价已选提示（原 `CCFG-REQ-020/022` 与 `PENDING_USER_CONFIRMATION` 处理一并取代）；删除、启用、停用一律从该行“更多”下拉进入，不以行选中为前提（`CCFG-DESIGN-041`）；查询/重置/重新加载/删除成功后不存在“清空选中项”动作，取消删除/停用确认后也不存在“保留选中项”语义。保留双击行编辑与探针 ID 键盘编辑，二者与取消单选互不冲突。 **【本轮（第五轮）定向修订 · 待批准】** 本条“**取消全部选择能力**”的决定由第五轮 §17 `CCFG-DESIGN-077`（`CCFG-REQ-142`）作**页面级定向修订**：仅**恢复**“悬停产生移走即消失的临时高亮 + 左键点击普通行内容切换**唯一一行**固定选中视觉”的**视觉定位状态**（三态切换、整个主列表同时最多一行、悬停与固定选中使用可区分的视觉层级）；**不**恢复本条已取消的**旧的批量或业务选择能力**（“‘删除所选’按钮及禁用判定与选中计数显示逻辑”“选择状态集合／选择事件／选择键／已选行集合／‘当前选中行’概念”“‘已选择：{探针ID}’或等价已选提示”“复选框／多选”“批量操作”“后端选择状态”）——删除、启用、停用**仍**一律从该行“更多”下拉进入、**不**以行选中为前提。本条其余口径（结果区头部“左侧摘要／右侧操作”布局、“新增探针”按钮置于头部**最右侧**、移除原左侧工具栏写操作按钮）**不变**；原文作为历史决定**保留不改写**，仅在此标注被修订范围，不伪装为原规则从未存在。 **【R1 定向修订 · 待复审】** 此处绝对禁止的“选择状态／选择键／当前选中行概念”**收窄为禁止上述旧的批量或业务选择能力**；本轮**允许**仅在 `/config/client` 当前页面会话内保存**一个可为空的单个探针 ID**（`selectedClientId` 或同义值）作为固定高亮的**本地**状态（`CCFG-DESIGN-082`／`CCFG-REQ-147`），该值不写入 URL／`localStorage`／`sessionStorage`／接口／数据库、不作为启用／停用／删除入口的前置条件。 | CCFG-REQ-021、CCFG-REQ-022、CCFG-REQ-093、CCFG-REQ-094、CCFG-REQ-020 | CCFG-AC-016、CCFG-AC-017、CCFG-AC-079、CCFG-AC-080 |
| CCFG-DESIGN-041 | 操作列与事件边界：主列表最右侧新增**固定**“操作”列，唯一文字入口为“更多”下拉；该入口及其下拉项的 `click` 与 `dblclick` 必须**阻止冒泡**到行，避免误触发 `@row-dblclick`（打开编辑弹窗）；因本轮已取消全部选择能力（`CCFG-DESIGN-040`），不存在 `@row-click` 改变选中的语义，也不得再描述为“点击更多后保持/切换选中行”。 **【本轮（第五轮）定向修订 · 待批准】** 上句“不存在 `@row-click` 改变选中的语义”由第五轮 §17 `CCFG-DESIGN-077`／`CCFG-DESIGN-079`（`CCFG-REQ-142`／`CCFG-REQ-144`）**定向修订**：新口径下重新存在“左键点击普通行内容”的点击处理，但**仅**用于切换前述**唯一一行固定选中视觉**，**不**恢复任何选择状态集合、批量入口、选中计数或后端选择语义（该部分维持本条取消口径）；本条“该入口及其下拉项的 `click` 与 `dblclick` 必须**阻止冒泡**到行，避免误触发 `@row-dblclick`”的边界**继续有效并扩展**为“**不得**触发行的点击固定选中切换（同时继续不得误触发 `@row-dblclick`）”。下拉命令按 `fgActive` 决定条目——`'1'` → {停用, 删除}，`'0'` → {启用, 删除}，非 `0/1`（`ABNORMAL`，见 CCFG-DESIGN-006/021）→ 仅 {停用, 删除}，不提供“启用”。删除 / 停用 / 启用命令沿用既有 E5 / E7 / E6 与二次确认语义，仅入口位置变化。 | CCFG-REQ-095、CCFG-REQ-096、CCFG-REQ-097、CCFG-REQ-098 | CCFG-AC-018、CCFG-AC-019、CCFG-AC-082、CCFG-AC-083 |
...
| CCFG-DESIGN-043 | 写接口复用（确定性方案）：删除复用既有 E5 删除接口、停用复用 E7、启用复用 E6；**不**新增批量删除接口，**不**新增任何批量删除的前端调用路径；删除成功后重新加载列表（**【本轮定向修订 · 待批准】** 原“并清空选中”因本轮取消全部选择能力已不适用，见 `CCFG-DESIGN-018/040`）；后端契约、错误码与写前校验（CCFG-DESIGN-023）零改动。 **【本轮（第五轮）定向修订 · 待批准】** 上句“删除成功后重新加载列表”在第五轮恢复固定选中视觉后需明确选中口径：删除成功触发的重载**清除**此前的固定选中（§17 `CCFG-DESIGN-081`／`CCFG-REQ-146`），删除**取消或失败**不改变原有固定选中；本项“**不**新增批量删除接口、**不**新增任何批量删除的前端调用路径”与后端契约、错误码、写前校验零改动**不变**。 | CCFG-REQ-094、CCFG-REQ-096、CCFG-REQ-098 | CCFG-AC-080、CCFG-AC-082、CCFG-AC-084 |
...
## 17. 弹窗间距、启停确认文案与列表单行选中调整（第五轮 · 2026-09-24 草案建立 · `CLIENT-CONFIG-DIALOG-SPACING-ROW-SELECTION-AND-CONFIRM-COPY-BASELINE-001`）

> 本节为**第五轮**新增／编辑探针弹窗**间距**、启用／停用**确认文案**与主列表**单行固定选中**调整新增设计项（`CCFG-DESIGN-072~082`，共 11 条），依据**项目负责人已明确确认的三项决定**建立**草案**。`/config/data-source`（数据源管理）新增数据源弹窗在本轮**仅作间距与节奏的视觉参照**，本节规则**只**作用于 `/config/client` 页面，**不**修改参考页的代码、文档、状态与行为，也**不**把本轮提炼为通用弹窗模板。 **【R1 定向修订 · 待复审】** 远程复审对 `3f1fdd4` 纯文档草案给出 `CHANGES_REQUIRED`，R1 **仅**作三处定向纠错（见下）：单个本地 `selectedClientId` 允许口径、`CCFG-DESIGN-081` 状态筛选可构造性、探针 ID 单击与编辑入口区分；编号／条数（`CCFG-DESIGN-072~082`，82 条）不变。
>
> **状态分层**：`adjustment5_baseline_status=DRAFT_PENDING_USER_REVIEW`、`adjustment5_approval_status=NOT_APPROVED`、`adjustment5_implementation_status=NOT_STARTED`、`adjustment5_formal_acceptance_execution_status=NOT_RUN`（`CCFG-AC-136~146`，11 条全部 `NOT_RUN`）。§13（`CCFG-DESIGN-038~046`）、§14（`CCFG-DESIGN-047~053`）、§15（`CCFG-DESIGN-054~060`）、§16（`CCFG-DESIGN-061~071`）此前已批准基线分别保持 `APPROVED`，其实现事实（含第四轮实现 R1 提交 `0c30b150343853c29ddf0329496dee3272d14cc0` 已通过 ChatGPT 远程代码复审、**尚未**获得项目负责人最终页面接受或正式验收）保持真实，本节**不改写、不抹除**。**项目负责人对三项决定的口头/聊天确认不等于本节草案已复审、已批准，更不等于调整已实现或验收通过。** 设计编号在本文件既有最大编号 `CCFG-DESIGN-071` 之后连续新增，**不**复用旧编号、**不**重排历史编号。
>
> **参考来源效力边界**：参考页 `/config/data-source` 的新增数据源弹窗**仅作纵向节奏与控件边界的视觉对照**；本节所列像素／运行期测量口径仅供实现阶段对照，**不是**测试通过证据；实现必须**以真实参考页样式与实际视口为准**（先测量、后修值，**不**凭截图写死像素）。本节**不**触碰 `API.md` 与 `DATABASE.md`（判定为无需变更：本轮为页面级表单布局、确认文案与前端选择视觉调整，**不**涉及接口契约、字段、状态值与数据库对象）。
>
> **与既有设计项的关系（定向修订并保留原文）**：`CCFG-DESIGN-020` 的“**一般不弹确认**”被 `CCFG-DESIGN-076` **定向修订**（本页启用**新增**一次二次确认，取消不调用启用接口；这不是替换已有文案而是新增步骤）；`CCFG-DESIGN-019` 的“二次确认由前端负责”**保留**，其确认正文由 `CCFG-DESIGN-075`／`CCFG-UI-020` 定向收窄；`CCFG-DESIGN-040` 的“**取消全部选择能力**”被 `CCFG-DESIGN-077` 作**页面级定向修订**（仅恢复点击固定单行**视觉**，**不**恢复“删除所选”、批量入口、已选文字、复选框或后端选择状态）；`CCFG-DESIGN-041` 的“不存在 `@row-click` 改变选中的语义”与 `CCFG-DESIGN-018`／`CCFG-DESIGN-043` 的删除后重载口径分别被 `CCFG-DESIGN-077`／`CCFG-DESIGN-079` 与 `CCFG-DESIGN-081` 定向修订。以上各项定义行**原文保留不改写**（仅在其项内标注被修订范围），**不**伪装为原规则从未存在。`CCFG-DESIGN-056` 的约 **900px** 目标宽度与窄视口安全边距、`CCFG-DESIGN-057` 的数据源双栏比例与受控滚动、`CCFG-DESIGN-058` 的标签排版令牌、`CCFG-DESIGN-066` 的反馈区稳定空间、`CCFG-DESIGN-067` 的视口水平居中、`CCFG-DESIGN-068` 的标签右对齐**全部继续有效**；本轮 `CCFG-DESIGN-072`~`CCFG-DESIGN-074` **仅在其上**补充水平间距、纵向节奏与调整后稳定性口径，**不**取代其中任何一条。`CCFG-DESIGN-039` 的“无刷新能力”与 `CCFG-DESIGN-041`／`CCFG-DESIGN-052` 的三点菜单事件边界**继续有效**，`CCFG-DESIGN-080` 仅补充“普通重载清除固定选中、重置不发起查询”的选中口径。
>
> **【R1 定向修订 · 待复审】** R1 在第五轮草案基础上作三处定向纠错，**不**新增／删除／重排设计项（仍 `CCFG-DESIGN-072~082`，共 11 条），既有 6 条定向修订行集合不变：① 将 `CCFG-DESIGN-040`、`CCFG-DESIGN-082` 中绝对禁止的“选择状态／选择键／‘当前选中行’概念”**收窄为仅禁止旧的批量或业务选择能力**，并明确**允许**仅在 `/config/client` **当前页面会话内**保存**一个可为空的单个探针 ID**（`selectedClientId` 或同义值）作为固定高亮的**本地**状态（`CCFG-DESIGN-077`／`082`），该值不写入 URL／`localStorage`／`sessionStorage`／接口／数据库，也**不**作为启用／停用／删除入口的前置条件；② `CCFG-DESIGN-081` 的“不再显示”以**状态筛选**构造（“启用”条件下停用、“停用”条件下启用），保留“全部”筛选下目标仍在结果中则按稳定探针 ID 固定该行；③ `CCFG-DESIGN-078`／`079` 明确**探针 ID 文字／所在普通单元格的普通左键单击属“普通行内容”**、按 `CCFG-DESIGN-077` 规则切换固定选中，仅 **Enter／Space 键盘编辑**与**行双击编辑**属编辑入口。以上纠错为第五轮**草案**范围内的措辞与可构造性修订，**不**改变第一至第四轮已批准基线，也**不**改变本轮三项已确认产品决定；**不**把绝对禁止“单行固定高亮”的历史口径伪装为从未存在，单个本地 ID 仅是本轮恢复的**视觉定位实现状态**。
>
> **后续入口（仅登记，不提前创建／批准）**：本轮**不**扩展为通用弹窗模板、**不**修改 `docs/baseline/**` 的模板级全局状态（保持 `NOT_STARTED` / `NOT_GRANTED` / `NOT_DECIDED`）。`CCFG-DESIGN-071` 登记的“新增／编辑表单弹窗模板”后续入口其前置条件变为“**第五轮页面实现完成并经项目负责人实际目测认可之后**”，本轮**不**提前创建／批准模板、**不**声称其他页面已接入。

| 设计编号 | 设计决定 | 覆盖需求 | 覆盖验收 |
|---|---|---|---|
| CCFG-DESIGN-072 | 新增／编辑探针弹窗**水平间距**：在保留 `CCFG-DESIGN-056` 约 **900px** 桌面目标宽度与窄视口安全边距、**不**扩大弹窗的前提下，配置项名称（“探针 ID”“探针描述”“采集数据源”）的**右边缘**与其**控件左边缘**之间统一增加约 **12px** 水平间距；标签的已批准字号、字重、颜色、必填红星与**右对齐**口径（`CCFG-DESIGN-058`／`CCFG-DESIGN-068`）**不变**；输入框、文本域与数据源双栏区的**控件左边界继续对齐**，控件**右边界保持当前布局位置**。改动通过**页面作用域的表单布局**（如标签列宽／间距令牌）完成，**不**为某一个输入框随意设置固定宽度，**不**改参考页、**不**改其他弹窗、**不**新增全局样式块、**不使用** `!important`。 | CCFG-REQ-137 | CCFG-AC-136 |
| CCFG-DESIGN-073 | 弹窗字段**纵向节奏**与反馈区保留：上述三个字段**之间**的可见纵向留白以 `/config/data-source`“新增数据源”弹窗的配置项节奏为视觉参照；实现前必须核对本页探针弹窗 `.cc-form` 的字段 `gap` 与 `.cc-field-feedback`（`CCFG-DESIGN-066` 的稳定占位）**共同**形成的间距，允许**收敛**字段间留白，但**不得**移除反馈区、**不得**以固定弹窗高度掩盖问题；数据源双栏内部（搜索、候选列表、已选区与受控滚动）维持当前布局（`CCFG-DESIGN-057` 不变）。 | CCFG-REQ-138 | CCFG-AC-137 |
| CCFG-DESIGN-074 | 间距调整后的**弹窗稳定性**：水平与纵向间距调整后，在同一视口内字段**提示与错误状态**切换（含未选数据源的中性提示↔红色错误态、`CCFG-DESIGN-064`／`CCFG-DESIGN-065`）**不得**造成弹窗高度或位置突变（继续满足 `CCFG-DESIGN-066`）；长错误文字必须**完整可读**、不得硬裁剪；允许依视口大小采用既有响应式布局与受控滚动（`CCFG-DESIGN-056`／`CCFG-UI-022`）。实现方式以真实视口测量确定，本设计**不**写死像素值、**不**构成待确认项。 | CCFG-REQ-139 | CCFG-AC-138 |
| CCFG-DESIGN-075 | 停用**确认文案收窄**：停用确认窗口的正文由 `确定停用探针 {探针ID} 吗？停用后该探针不再按启用状态命中。` 定向修订为 **`确定停用探针 {探针ID} 吗？`**，标题“**停用探针**”，按钮“**取消／停用**”；原有二次确认步骤与“后端仅把 `FG_ACTIVE` 更新为 `0`”的业务语义（`CCFG-DESIGN-019`）**不变**。页面**不得**出现“按启用状态命中”一类难解表述，也**不得**声称停用会立即停止探针进程或采集任务。 | CCFG-REQ-140 | CCFG-AC-139 |
| CCFG-DESIGN-076 | 启用**新增确认步骤**（定向修订 `CCFG-DESIGN-020` 的“一般不弹确认”）：本页启用操作改为**先二次确认、确认后才调用既有 E6**，确认正文 **`确定启用探针 {探针ID} 吗？`**，标题“**启用探针**”，按钮“**取消／启用**”；**取消确认不调用启用接口**，一次点击**不得**触发多次请求。启用既有的 `FG_ACTIVE` 更新为 `1`、服务端校验、错误处理与行级忙碌锁（`CCFG-DESIGN-042`）**保留**；异常状态行“更多”原有的安全操作口径（非 `0/1` 不提供“启用”，`CCFG-DESIGN-021`／`CCFG-DESIGN-041`）**不变**。 | CCFG-REQ-141 | CCFG-AC-140 |
| CCFG-DESIGN-077 | 主列表**悬停与点击固定单行选中**（页面级定向修订 `CCFG-DESIGN-040` 的“取消全部选择能力”）：**悬停**只产生**临时高亮**，鼠标移走即消失；**左键点击普通行内容**切换**固定选中**——未选中时固定当前行、再次点击同一行取消、点击另一行则只选中新行，整个主列表**同时最多一行**；鼠标移到其他行时固定选中仍须**清晰可辨**，悬停与固定选中使用**可区分的视觉层级**。仅恢复**视觉单行定位状态**（**实现为一个可为空的单个探针 ID（`selectedClientId` 或同义值）的页面会话内本地状态**，而非已选行集合），**不**恢复“删除所选”按钮及计数、批量操作、“已选择：{探针ID}”文字、复选框／多选或**后端选择状态**；删除、启用、停用仍由各行“操作”列三点菜单完成；**保留**双击行编辑与探针 ID 的 Enter／Space 编辑入口。 **【R1 定向修订 · 待复审】** 该单个本地 ID 不写入 URL／`localStorage`／`sessionStorage`／接口／数据库、不作为启停删除入口前提；**探针 ID 文字／所在普通单元格的左键单击属“普通行内容”**，按本条规则切换固定选中（见 `CCFG-DESIGN-078`）。 | CCFG-REQ-142 | CCFG-AC-141 |
| CCFG-DESIGN-078 | **双击与单击的事件协调**：行双击由两次 `click` 组成，实现必须使双击编辑**不**错误地取消已固定选中的行、也**不**留下与用户最后一次操作不符的选中状态；设计口径为“单击切换固定选中、双击进入编辑，二者以可执行的判定规则协调（如首次点击先按单击口径显示固定选中，双击达成时保持该行选中状态并进入编辑）”，具体判定方式以实现阶段可执行且可回归测试为准；探针 ID 的 Enter／Space 键盘编辑入口**保持**不变。 **【R1 定向修订 · 待复审】** 明确：**探针 ID 文字／所在普通单元格的左键单击属“普通行内容”点击**，按单击口径切换固定选中、**不**排除在被选中范围之外；**仅**对 ID 的 **Enter／Space 键盘编辑**与**行双击编辑**为编辑入口，二者进入编辑时**不得**因事件传播造成二次选中／取消。 | CCFG-REQ-143 | CCFG-AC-142 |
| CCFG-DESIGN-079 | 行内**事件隔离**：三点按钮（触发器）、其下拉菜单浮层、启用／停用／删除**确认窗口（含遮罩与按钮）**以及必要的其他**真行内交互控件**的 `click`／`dblclick` **必须阻止冒泡**到行，**不得**触发行的点击固定选中切换；本条延续并扩展 `CCFG-DESIGN-041`／`CCFG-DESIGN-052` 既有事件边界，原本“不得冒泡触发行双击编辑”的口径**继续有效**。 **【R1 定向修订 · 待复审】** 探针 ID 的**普通左键单击**（文字／所在普通单元格）**不属于**本条需隔离的“真行内交互控件”，应按普通行内容参与 `CCFG-DESIGN-077` 的固定选中切换；**仅**对 ID 的 **Enter／Space 键盘编辑**与**行双击编辑**属编辑入口（见 `CCFG-DESIGN-078`）。 | CCFG-REQ-144 | CCFG-AC-143 |
| CCFG-DESIGN-080 | **普通列表重载清除固定选中**：进入页面的首次加载、用户点“查询”、列表加载失败后点“重试”，以及其他**非启停成功触发**的重载，**均清除**此前的固定选中；页面既有“重置”只清空查询控件、**不发起查询**，因此单独点击“重置”**不**属于列表重载、**不**以此为由改变选中；页面**不**新增刷新按钮、自动刷新或轮询（`CCFG-DESIGN-039` 继续有效）；若未来确有外部刷新导致重新加载，同样清除此前选中。选中状态**不**写入 URL、`localStorage` 或 `sessionStorage`。 | CCFG-REQ-145 | CCFG-AC-144 |
...
| 2026-09-24 | 弹窗间距、启停确认文案与列表单行选中调整**草案**（`CLIENT-CONFIG-DIALOG-SPACING-ROW-SELECTION-AND-CONFIRM-COPY-BASELINE-001`，纯文档）：依据项目负责人已明确确认的**三项决定**，新增 §17 与 `CCFG-DESIGN-072~082`（11 条，追加在既有最大编号 `071` 之后，不重排历史编号）——`072` 弹窗水平间距（保留 `056` 约 900px 与窄视口安全边距、不扩大弹窗；标签右缘与控件左缘统一增加约 12px；标签字号／字重／颜色／必填红星／右对齐不变；输入框、文本域与数据源双栏控件左边界继续对齐、右边界保持当前位置；通过页面作用域表单布局完成，不为单个输入框随意设固定宽度，不加全局样式与 `!important`）、`073` 字段纵向节奏与反馈区保留（以参考页“新增数据源”弹窗节奏为视觉参照，实现前核对 `.cc-form` 字段 `gap` 与 `.cc-field-feedback` 稳定占位共同形成的间距、可收敛字段间留白，但不得移除反馈区、不得以固定弹窗高度掩盖问题；数据源双栏内部搜索／候选／已选与受控滚动维持当前布局）、`074` 间距调整后的弹窗稳定性（同视口内提示↔错误态切换不得造成高度或位置突变、长错误完整可读不硬裁剪、允许既有响应式与受控滚动、实现以真实视口测量确定不写死像素）、`075` 停用确认文案收窄（`确定停用探针 {探针ID} 吗？停用后该探针不再按启用状态命中。` → `确定停用探针 {探针ID} 吗？`，标题“停用探针”、按钮“取消／停用”；二次确认与仅改 `FG_ACTIVE` 语义不变；不得声称停用会立即停止探针进程或采集任务）、`076` 启用**新增确认步骤**（定向修订 `020` 的“一般不弹确认”：`确定启用探针 {探针ID} 吗？`，标题“启用探针”、按钮“取消／启用”；取消不调用启用接口、一次点击不得触发多次请求；保留 `FG_ACTIVE` 更新、服务端校验、错误处理与行级忙碌锁；异常行“更多”安全操作口径不变）、`077` 悬停与点击固定单行选中（悬停仅临时高亮、移走即消失；左键点击普通行内容切换固定选中三态、整个主列表同时最多一行；跨行悬停时固定选中仍清晰可辨、与悬停使用可区分视觉层级；仅恢复视觉单行定位状态，不恢复“删除所选”、批量操作、“已选择：{探针ID}”、复选框或后端选择状态；删除／启停仍走三点菜单；保留双击编辑与探针 ID 键盘编辑）、`078` 双击与单击事件协调（双击由两次 click 组成，不得误取消已固定选中行、不得留下与最后操作不符的状态；判定规则须实现阶段可执行且可回归）、`079` 行内事件隔离（三点按钮、下拉浮层、启停／删除确认窗口含遮罩与按钮、必要的其他行内交互控件阻止冒泡，不得触发行的点击固定选中切换；延续并扩展 `041`／`052` 既有边界）、`080` 普通列表重载清除固定选中（首次加载、查询、失败重试及其他非启停成功触发的重载均清除；“重置”只清空查询控件不发起查询故不改变选中；无刷新按钮／自动刷新／轮询；外部刷新同样清除；不写入 URL／`localStorage`／`sessionStorage`）、`081` 启停成功重载的唯一例外与删除结果（成功后目标探针仍在列表则按稳定探针 ID 固定选中、已因查询条件过滤则清除且不保留“看不见的已选行”；取消确认或启停失败不变；删除成功清除、删除取消／失败不变）、`082` 边界与不恢复项（选中仅在当前页面会话有效、不改启停/删除/编辑 API 契约、不引入额外持久化字段、不改过滤与排序；页面级口径不提炼通用模板、不改两套模板全局状态；明确不恢复选择状态集合／复选框／“删除所选”／批量操作／已选文字／后端选择状态）。**定向修订既有设计定义行 6 条**（保留原文并就地标注方向，不伪装为原规则不存在）：`CCFG-DESIGN-018`（删除成功重载清除固定选中、取消／失败不变）、`CCFG-DESIGN-019`（二次确认保留，确认正文由 `075` 收窄）、`CCFG-DESIGN-020`（“一般不弹确认”被 `076` 定向修订为新增确认步骤，给出新旧适用时序对照）、`CCFG-DESIGN-040`（“取消全部选择能力”被 `077` 作页面级定向修订，仅恢复点击固定单行视觉，不恢复已取消的选择状态／批量入口／已选提示／复选框／后端选择）、`CCFG-DESIGN-041`（“不存在 `@row-click` 改变选中的语义”被 `077`／`079` 定向修订，事件边界继续有效并扩展为不得触发行的点击固定选中切换）、`CCFG-DESIGN-043`（删除后重载的选中口径与“不新增批量删除接口／调用路径”不变）。**状态分层**：本轮草案 `adjustment5_baseline_status=DRAFT_PENDING_USER_REVIEW`、`adjustment5_approval_status=NOT_APPROVED`、`adjustment5_implementation_status=NOT_STARTED`、`adjustment5_formal_acceptance_execution_status=NOT_RUN`；§13、§14、§15、§16 此前已批准基线分别保持 `APPROVED`，其实现事实（含第四轮实现 R1 提交 `0c30b150343853c29ddf0329496dee3272d14cc0` 已通过 ChatGPT 远程代码复审、尚未获得项目负责人最终页面接受或正式验收）**保持真实、不改写、不抹除**。**编号与计数**：`CCFG-DESIGN-001~082`（82 条）连续、唯一、不新增/删除/重排历史编号；除上述 6 条定向标注外，既有 `CCFG-DESIGN-001~071` 设计定义行相对起始提交 `0c30b15` **逐字节零变化**；§12 追踪矩阵更新为 **147/147** 需求、**146/146** 验收；需求 147 条、验收 146 条（`CCFG-AC-136~146` 全部 `NOT_RUN`）、界面 70 条；`PENDING_USER_CONFIRMATION=0`。**不改** `API.md`/`DATABASE.md`、**不改**任何业务代码/测试/前端或后端源文件、**不改** `docs/baseline/**` 与两套模板全局状态（`NOT_STARTED`/`NOT_GRANTED`/`NOT_DECIDED`）、**不改**数据源管理参考页、**不**回写既有报告；下一入口 `CHATGPT_REMOTE_CLIENT_CONFIG_DIALOG_SPACING_ROW_SELECTION_CONFIRM_COPY_BASELINE_REVIEW`（草案复审入口，**不是**直接进入实现） | `CLIENT-CONFIG-DIALOG-SPACING-ROW-SELECTION-AND-CONFIRM-COPY-BASELINE-001`（项目负责人已确认的三项决定驱动的**纯文档草案**；未实现代码、未运行测试/构建/浏览器/服务、未访问数据库/ZooKeeper/Kafka、未执行正式验收或最终接受；项目负责人确认三项决定 ≠ 文档已批准 ≠ 已实现 ≠ 已验收） |
| 2026-09-24 | 第五轮草案 R1 定向纠错（ChatGPT 远程复审 `3f1fdd4` 结论 `CHANGES_REQUIRED` 驱动，**纯文档**）：**仅**三处定向修订，未改编号与条数。① 将 `CCFG-DESIGN-040`／`082` 中绝对禁止的“选择状态／选择键／‘当前选中行’概念”**收窄为禁止旧的批量或业务选择能力**，明确**允许**仅在 `/config/client` 当前页面会话内保存**一个可为空的单个探针 ID**（`selectedClientId` 或同义值）作为固定高亮的本地状态（`CCFG-DESIGN-077`／`082`），不入 URL／`localStorage`／`sessionStorage`／接口／数据库、不作为启停删除入口前提；② `CCFG-DESIGN-081` 的“不再显示”以**状态筛选**构造（“启用”条件下停用、“停用”条件下启用），保留“全部”筛选下目标仍在结果中则按稳定探针 ID 固定该行；③ `CCFG-DESIGN-078`／`079` 明确**探针 ID 文字／所在普通单元格的普通左键单击属“普通行内容”**、按 `CCFG-DESIGN-077` 规则切换固定选中，仅 Enter／Space 键盘编辑与行双击编辑属编辑入口。**编号与计数**：`CCFG-DESIGN-001~082`（82 条）连续、唯一，未新增／删除／重排；除 R1 修订标注外，既有定义行原文字节保留；§12 追踪矩阵仍 **147/147** 需求、**146/146** 验收；`adjustment5_baseline_status=DRAFT_PENDING_USER_REVIEW`／`adjustment5_approval_status=NOT_APPROVED`／`adjustment5_implementation_status=NOT_STARTED`／`adjustment5_formal_acceptance_execution_status=NOT_RUN` 保持；下一入口 `CHATGPT_REMOTE_CLIENT_CONFIG_DIALOG_SPACING_ROW_SELECTION_CONFIRM_COPY_BASELINE_R1_REVIEW`；不回写 R0 报告与第一至第四轮已批准基线 | `CLIENT-CONFIG-DIALOG-SPACING-ROW-SELECTION-AND-CONFIRM-COPY-BASELINE-001-R1`（ChatGPT 远程复审 `CHANGES_REQUIRED` 驱动的 R1 定向修订；纯文档任务，未实现、未运行测试/构建/浏览器/服务、未访问数据库/ZooKeeper/Kafka、未执行正式验收） |
