...
| 第五轮弹窗间距／启停确认文案／列表单行选中调整基线状态 | `adjustment5_baseline_status=DRAFT_PENDING_USER_REVIEW`（依据**项目负责人已明确确认的三项决定**建立的**草案**；**尚未**经 ChatGPT 远程复审、**尚未**批准，见 §17） |
...
| 第五轮现行下一入口 | `CHATGPT_REMOTE_CLIENT_CONFIG_DIALOG_SPACING_ROW_SELECTION_CONFIRM_COPY_BASELINE_REVIEW`（远程文档复审通过后**仍需项目负责人批准本轮草案**，才能另行进入代码实现） |
...
> 本节为**第五轮**新增／编辑探针弹窗**间距**、启用／停用**确认文案**与主列表**单行固定选中**调整新增设计项（`CCFG-DESIGN-072~082`，共 11 条），依据**项目负责人已明确确认的三项决定**建立**草案**。`/config/data-source`（数据源管理）新增数据源弹窗在本轮**仅作间距与节奏的视觉参照**，本节规则**只**作用于 `/config/client` 页面，**不**修改参考页的代码、文档、状态与行为，也**不**把本轮提炼为通用弹窗模板。
>
...
| CCFG-DESIGN-077 | 主列表**悬停与点击固定单行选中**（页面级定向修订 `CCFG-DESIGN-040` 的“取消全部选择能力”）：**悬停**只产生**临时高亮**，鼠标移走即消失；**左键点击普通行内容**切换**固定选中**——未选中时固定当前行、再次点击同一行取消、点击另一行则只选中新行，整个主列表**同时最多一行**；鼠标移到其他行时固定选中仍须**清晰可辨**，悬停与固定选中使用**可区分的视觉层级**。仅恢复**视觉单行定位状态**，**不**恢复“删除所选”按钮及计数、批量操作、“已选择：{探针ID}”文字、复选框或**后端选择状态**；删除、启用、停用仍由各行“操作”列三点菜单完成；**保留**双击行编辑与探针 ID 的 Enter／Space 编辑入口。 | CCFG-REQ-142 | CCFG-AC-141 |
| CCFG-DESIGN-078 | **双击与单击的事件协调**：行双击由两次 `click` 组成，实现必须使双击编辑**不**错误地取消已固定选中的行、也**不**留下与用户最后一次操作不符的选中状态；设计口径为“单击切换固定选中、双击进入编辑，二者以可执行的判定规则协调（如首次点击先按单击口径显示固定选中，双击达成时保持该行选中状态并进入编辑）”，具体判定方式以实现阶段可执行且可回归测试为准；探针 ID 的 Enter／Space 键盘编辑入口**保持**不变。 | CCFG-REQ-143 | CCFG-AC-142 |
| CCFG-DESIGN-079 | 行内**事件隔离**：三点按钮（触发器）、其下拉菜单浮层、启用／停用／删除**确认窗口（含遮罩与按钮）**以及必要的其他行内交互控件的 `click`／`dblclick` **必须阻止冒泡**到行，**不得**触发行的点击固定选中切换；本条延续并扩展 `CCFG-DESIGN-041`／`CCFG-DESIGN-052` 既有事件边界，原本“不得冒泡触发行双击编辑”的口径**继续有效**。 | CCFG-REQ-144 | CCFG-AC-143 |
...
| CCFG-DESIGN-081 | **启停成功重载的唯一例外与删除结果**：启用／停用**成功后自身触发**的列表更新，若目标探针仍在当前列表，按**稳定探针 ID** 固定选中该行（即使操作前另一行被选中）；若因当前查询条件而不再显示，则**清除**固定选中、**不**保留“看不见的已选行”。**取消确认或启停失败**不改变原有选中；**删除成功**触发的重载清除固定选中（`CCFG-DESIGN-018`／`CCFG-DESIGN-043`），**删除取消／失败**不改变原有选中。 | CCFG-REQ-146 | CCFG-AC-145 |
| CCFG-DESIGN-082 | **边界与不恢复项**：单行固定选中**仅在当前页面会话内**有效，**不**改启用／停用／删除／编辑的 API 契约、**不**引入额外持久化字段、**不**修改列表过滤或排序规则；本决策为 `/config/client` **页面级**口径，**不**提炼为通用模板、**不**修改两套模板的模板级全局状态（保持 `NOT_STARTED` / `NOT_GRANTED` / `NOT_DECIDED`）；明确**不**恢复选择状态集合、复选框、“删除所选”、批量操作、“已选择：{探针ID}”文字与后端选择状态。 | CCFG-REQ-147 | CCFG-AC-146 |
