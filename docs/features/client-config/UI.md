...
| 依据需求 | `CCFG-REQ-001~103`（`001~090` 已批准；`091~103` 为本轮调整草案，见 §15） |
...
| 设计编号 | `CCFG-UI-001 ~ CCFG-UI-035`（`001~026` 已批准；`027~035` 为本轮新增草案，见 §15），连续、唯一、不可复用；每个设计编号恰有一个定义行 |
| PENDING_USER_CONFIRMATION | `1`（本轮新增 1 项界面派生待确认，见 §15；`CCFG-UI-001~026` 范围内仍为 `0`） |
...
| 本轮下一入口 | `CHATGPT_REMOTE_BASELINE_REVIEW` |
...
| 设计编号 | 设计决定 | 覆盖需求 | 覆盖验收 |
|---|---|---|---|
| CCFG-UI-004 | **【本轮定向修订 · 待批准】** 工具栏（位于独立表格卡片顶部，含“新增探针”与唯一“删除所选”两个按钮）：新增探针为蓝色主按钮并带清晰加号图标；删除所选带删除图标，未选中任何行时灰色禁用，选中后为红色描边危险按钮（不使用大面积红色填充）；选中记录后按钮右侧显示“已选择：{探针ID}”，未选中时不得显示伪造 ID。工具栏不显示“双击记录可编辑”提示文字；取消提示不等于取消交互，双击行打开编辑与探针 ID 键盘聚焦后 Enter/空格打开编辑能力必须保留。删除前弹二次确认（文案见 §14），确认内容含将被删除的探针 ID；取消不删除；删除成功后刷新列表并清空选中状态。**【本轮定向修订 · 待批准】** 本项中“唯一‘删除所选’按钮”“未选中禁用 / 选中后显示‘已选择：{探针ID}’”及该按钮的删除确认入口已由 §15 `CCFG-UI-029`（取消“删除所选”、新增按钮移至结果区头部最右侧）取代；删除确认语义本身仍有效，入口见 `CCFG-UI-032`；行选中与“已选择”去留见 §15 `PENDING_USER_CONFIRMATION`。 | CCFG-REQ-021、CCFG-REQ-022、CCFG-REQ-023、CCFG-REQ-025 | CCFG-AC-017、CCFG-AC-018、CCFG-AC-019 |
...
## 15. 页面级模板选择性接入与列表调整（本轮新增 · `DRAFT_PENDING_USER_REVIEW`）

本节为 `CLIENT-CONFIG-QUERY-LIST-AND-LIST-TABLE-ADJUSTMENT-BASELINE-001` 的界面草案，
基于项目负责人在当前会话中逐项确认的 9 项页面调整决策。
**本节为草案，未获批准、未实现、未执行验收、未目测**；
`CCFG-UI-027~035` 与其覆盖的 `CCFG-REQ-091~103`、`CCFG-AC-077~089` 一致地处于
`DRAFT_PENDING_USER_REVIEW` / `NOT_STARTED` / `NOT_RUN`。
...
| CCFG-UI-029 | 结果区头部右侧放置“新增探针”主按钮（蓝色主按钮 + 加号图标），位置与数据源管理主列表的右侧“新增数据源”一致。**取消**原工具栏的“删除所选”按钮：结果区不再提供任何批量删除入口；行选中仍保留（单选与高亮语义见 `CCFG-UI-005` 与本节 `CCFG-UI-035`）。现有工具栏提示文字口径不变：不显示“双击记录可编辑”提示，双击行/键盘打开编辑的能力保留。 | CCFG-REQ-093、CCFG-REQ-094 | CCFG-AC-079、CCFG-AC-080 |
...
| CCFG-UI-031 | “探针 ID”列紧随“序号”列（第二列）。当该行 `fgActive='0'`（停用）时，在探针 ID 文本之后追加“停用”标记；`fgActive='1'`（启用）时**不**显示该标记。标记的视觉样式、颜色语义、与 ID 文本的间距及显示位置必须与数据源管理主列表“数据源 ID”后的“停用”标记保持一致（同一视觉语言，不另立一套样式）。探针 ID 自身的单行省略与完整值 Tooltip 不变。 | CCFG-REQ-101、CCFG-REQ-011 | CCFG-AC-009、CCFG-AC-087 |
| CCFG-UI-032 | 最右侧“操作”列为固定列，列内只有唯一文字入口“更多”（文字按钮或链接样式，带清晰悬停/聚焦反馈）。点击“更多”展开下拉菜单，条目按 `FG_ACTIVE` 决定：`fgActive='1'`（启用中）→ 下拉含“删除”“停用”；`fgActive='0'`（已停用）→ 下拉含“删除”“启用”；非 `0/1`（历史异常）→ 下拉仅含“删除”“停用”（不提供“启用”，按 `CCFG-REQ-034`/`CCFG-REQ-097`）。条目顺序为先“停用/启用”后“删除”，危险条目“删除”使用危险语义样式，“停用”使用警告语义样式；下拉内不出现状态标签、不出现批量操作。操作列与行内其他区域的事件边界：单击/双击下拉入口不得冒泡触发行编辑（`row-dblclick`），展开下拉不改变行选中态。 | CCFG-REQ-096、CCFG-REQ-097 | CCFG-AC-082、CCFG-AC-083 |
...
| CCFG-UI-035 | 历史异常 `FG_ACTIVE`（值非 `'0'`/`'1'`）的可见性**不得静默丢失**：取消原“状态”列后，该行的异常状态必须在主列表中仍可被用户看见（按当前 Feature 规则以异常状态呈现，且必须显示可辨识的原始值；具体承载位置与呈现形式由实现阶段在“探针 ID 列标记 + 操作列下拉条目”范围内确定，但不得隐藏、不得降级为与“启用/停用”相同的正常外观）。该项与 `CCFG-REQ-102`/`CCFG-AC-088` 对应，属于可见性保护要求，不是新增状态列。行单选中高亮与“已选择：{探针ID}”文本随“删除所选”取消后的存续，**按 `PENDING_USER_CONFIRMATION` 暂按保留处理**（`CCFG-REQ` 未编号、不影响 103/103 覆盖）。 | CCFG-REQ-102 | CCFG-AC-088 |
...
其被取代范围以本节为准。其他界面项（查询区 `CCFG-UI-003`、采集数据源列 `CCFG-UI-007~011`/`026`、
新增/编辑弹窗 `CCFG-UI-013~017`、自动生成 `CCFG-UI-015`、候选控件 `CCFG-UI-016`、
加载与空状态 `CCFG-UI-012`/`023`、文案表 `CCFG-UI-020`/`021`）**本轮不变**。

**本轮 `PENDING_USER_CONFIRMATION`（1 项）**：
`CCFG-UI-004` 的“已选择：{探针ID}”文本与行单选高亮，在“删除所选”按钮取消后是否仍然保留？
默认口径：**暂按保留**（行选中与高亮不随按钮取消而取消，仅去掉批量删除入口）。
该项为项目负责人本轮确认项，未编号、不影响需求/验收覆盖计数；
待项目负责人明确后在后续任务中定稿。
