...
| 本轮下一入口 | `CHATGPT_REMOTE_BASELINE_APPROVAL_CLOSEOUT_R2_REVIEW`（ChatGPT 远程独立复审本次 R2 状态/入口纠错的结果提交；历史入口 `CHATGPT_REMOTE_BASELINE_APPROVAL_CLOSEOUT_REVIEW` 已完成并返回 `CHANGES_REQUIRED`，原 `CHATGPT_REMOTE_BASELINE_R1_REVIEW` 已随 R1/R2/R3 复审闭环；基线经 ChatGPT 远程 R3 复审 `APPROVED` 后由项目负责人于 2026-09-22 批准）。**该入口已属已完成的历史入口（本行为第一轮页面调整基线的当时记录）**；**当前下一入口**见 §17：`CHATGPT_REMOTE_CLIENT_CONFIG_POPOVER_AND_DIALOG_VISUAL_BASELINE_R1_REVIEW` |
...
| 下一入口 | 2026-09-22 批准收口时的**历史入口**：`CHATGPT_REMOTE_BASELINE_APPROVAL_CLOSEOUT_REVIEW`（该远程复审已完成并返回 `CHANGES_REQUIRED`）；该记录时点的下一入口为 `CHATGPT_REMOTE_BASELINE_APPROVAL_CLOSEOUT_R2_REVIEW`（早已完成，仅作历史记录）；**现行下一入口**见 §17 与 `README.md` §5：`CHATGPT_REMOTE_CLIENT_CONFIG_POPOVER_AND_DIALOG_VISUAL_BASELINE_R1_REVIEW` |
...
## 17. `+N` 清单与新增／编辑弹窗视觉调整（第三轮草案 · 2026-09-23 草案建立 → ChatGPT 复审 `CHANGES_REQUIRED` → R1 定向纠错 · `CLIENT-CONFIG-POPOVER-AND-DIALOG-VISUAL-BASELINE-001`）

本节为第三轮（本轮）`+N` 清单与新增／编辑弹窗视觉调整的**界面设计草案**，依据项目负责人查看 `/config/client`（探针端管理）与 `/config/data-source`（数据源管理，**仅作视觉参考**）页面后**已明确确认的四项调整决定**建立。**状态分层**：本节草案基线 `adjustment3_baseline_status=DRAFT_PENDING_USER_REVIEW`、本轮实现 `adjustment3_implementation_status=NOT_STARTED`、本轮正式验收 `adjustment3_formal_acceptance_execution_status=NOT_RUN`；§15（`CCFG-UI-027~035`）与 §16（`CCFG-UI-036~042`）此前获批基线分别保持 `APPROVED`，其实现事实保持真实、不改写。**项目负责人对四项调整的聊天确认不等于本节草案已经正式批准，更不等于调整已实现或验收通过**；本节草案已经 ChatGPT 从远程 Git 独立复审、结论 `CHANGES_REQUIRED`，现由 R1 纠错任务 `CLIENT-CONFIG-POPOVER-AND-DIALOG-VISUAL-BASELINE-001-R1` 受理定向纠错，**尚未**获批、**尚未**实现。R1 仅定向修订验收条目 `CCFG-AC-113`（详见 `ACCEPTANCE.md`）与各文件当前状态/入口，**不**改动本节任何界面定义行，本轮四项产品决定不重新设计；`CCFG-UI-001~042` 的既有定义行除 `CCFG-UI-009`、`CCFG-UI-024` 两处**定向标注**外**逐字节不改**，本节与既有界面项冲突处一律按“本节定向修订并保留原文”处理。本节只作用于 `/config/client` 页面，**不**修改数据源管理参考页的代码、文档、状态、行为与模板级全局配置。编号在既有最大编号 `CCFG-UI-042` 之后连续新增，**不**复用旧编号、**不**重排历史编号。本节所列 `#hex`／像素数值均为**参考页实现事实**（供实现阶段核对），**不是**测试通过证据。
