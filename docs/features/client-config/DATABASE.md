...
| 创建日期 | 2026-09-03 |
| 设计编号 | `CCFG-DB-001 ~ CCFG-DB-020`，连续、唯一、不可复用 |
...
| CCFG-DB-003 | `CLIENT_DESC`：必填（去除首尾空白后非空），目标规则为 UTF-8 字节数 `<=1024`（`VARCHAR2(1024 BYTE)` 语义）；保存只写入最终描述文本，后端做必填/Trim/字节校验，不重新生成。 | CCFG-REQ-039、CCFG-REQ-059 | CCFG-AC-033 |
...
| CCFG-DB-007 | 两表无物理外键、无唯一约束、无规范化关联表；探针 ID 大小写不敏感唯一与“一个数据源只分配给一个探针”均为应用层业务规则，依托 §4 的表级互斥锁 + 锁内全量重查实现，不新增任何 DDL 对象。 | CCFG-REQ-068、CCFG-REQ-090 | CCFG-AC-056、CCFG-AC-076 |

## 4. SQL 逻辑形态（全部参数化绑定，禁止 `${}`/动态表名/拼接用户输入）

| 设计编号 | 设计决定 | 覆盖需求 | 覆盖验收 |
|---|---|---|---|
| CCFG-DB-008 | 列表读取：条件由 Mapper `<if>` 按绑定参数拼接，不拼接用户文本；无关键词且状态为 `ALL` 时为全表读取；关键字条件使用 `LOWER(CLIENT_ID)`/`LOWER(CLIENT_DESC)` `LIKE`（统一小写实现不区分大小写包含）；排序固定 `ORDER BY CLIENT_ID DESC`。候选/映射读取：`SELECT DATA_SOURCE_ID, DATA_SOURCE_ORG, DATA_SOURCE_NAME, DATA_SOURCE_TYPE, DATA_SOURCE_CATEGORY, FG_ACTIVE FROM CDC_DATA_SOURCE`（无密码列），候选附加 `WHERE FG_ACTIVE='1' AND UPPER(DATA_SOURCE_CATEGORY)='SOURCE' AND UPPER(DATA_SOURCE_TYPE)='ORACLE'`。 | CCFG-REQ-002、CCFG-REQ-005、CCFG-REQ-006、CCFG-REQ-010、CCFG-REQ-061 | CCFG-AC-002、CCFG-AC-004、CCFG-AC-005、CCFG-AC-014、CCFG-AC-049 |
| CCFG-DB-009 | 表级互斥锁（新增/编辑/启用的事务内、任何权威校验前获取）：`LOCK TABLE CDC_CLIENT_MULTIPLE IN EXCLUSIVE MODE WAIT 5`；等待超时（`ORA-30006`）映射错误码 `50050`，整笔回滚。 | CCFG-REQ-077、CCFG-REQ-085 | CCFG-AC-064、CCFG-AC-071 |
| CCFG-DB-010 | 锁内检查读取（获锁后在同一事务内执行，供应用层做 ID 大小写不敏感唯一与数据源占用映射）：`SELECT CLIENT_ID, DATA_SOURCE_ID, FG_ACTIVE FROM CDC_CLIENT_MULTIPLE`（无过滤全量，或按需 `ORDER BY CLIENT_ID`）；应用层对读取结果构建占用映射并判定；编辑自排除 `originalClientId`。 | CCFG-REQ-038、CCFG-REQ-068、CCFG-REQ-069、CCFG-REQ-070、CCFG-REQ-084 | CCFG-AC-030、CCFG-AC-056、CCFG-AC-057、CCFG-AC-070 |
...
| 操作 | 开启事务 | 表级互斥锁 | 锁内校验 | 写入字段 | 失败处理 |
|---|---|---|---|---|---|
| 新增（E3） | 是 | 是（`WAIT 5`） | 探针 ID ASCII 大小写不敏感唯一；全部数据源唯一分配且可用 | `CLIENT_ID/CLIENT_DESC/DATA_SOURCE_ID/FG_ACTIVE='1'` | 校验失败或超时整笔回滚 |
| 编辑（E4） | 是 | 是（`WAIT 5`） | 原记录存在（按 `originalClientId`）；新 ID 格式+唯一（自排除原 ID）；数据源唯一+可用（自排除原 ID） | `CLIENT_ID/CLIENT_DESC/DATA_SOURCE_ID` 一次原子更新 | 校验失败或超时整笔回滚，不部分写入 |
| 启用（E6） | 是 | 是（`WAIT 5`） | 原记录/状态（非 `0/1` 拒）；数据源重复分配（仅此阻断） | `FG_ACTIVE='1'` | 校验失败或超时整笔回滚 |
| 停用（E7） | 是（短） | 否 | 目标记录存在 | `FG_ACTIVE='0'` | 失败回滚 |
| 删除（E5） | 是（短） | 否 | 无（不做关联检查） | 物理删除该行 | 失败回滚 |
| 列表/候选读取（E1/E2） | 否 | 否 | — | — | 只读，无写锁 |

| 设计编号 | 设计决定 | 覆盖需求 | 覆盖验收 |
|---|---|---|---|
| CCFG-DB-016 | 事务与锁矩阵如上：新增/编辑/启用为“事务+表锁+锁内重查权威校验+写入”；停用/删除为不取表锁的短事务写；列表/候选为不加写互斥锁的只读。锁获取必须先于任何权威唯一性检查，锁等待超时整笔失败并回滚。写操作行数必须为 1，保证整体成功/整体失败，不产生部分写入。 | CCFG-REQ-049、CCFG-REQ-072、CCFG-REQ-074、CCFG-REQ-077 | CCFG-AC-040、CCFG-AC-059、CCFG-AC-061、CCFG-AC-064 |
| CCFG-DB-017 | Oracle 一致性读与显式表锁的关系：普通 `SELECT`（一致性读，经 undo 不取 `TM` 锁）在表锁持有期间照常可读、不被表锁阻塞，因此列表/候选读取设计为“不依赖表锁”；并发写入方（含本 Feature 其他写请求及其他模块对同一表的 DML）需获取行级/TM 锁，与 `EXCLUSIVE` 表锁互斥而等待，从而把配置写入串行化，堵住“先查后写”的竞态窗口（幻影插入）。锁的粒度针对本配置表低频写入，等待上限固定 5 秒。 | CCFG-REQ-077、CCFG-REQ-090 | CCFG-AC-064、CCFG-AC-076 |

## 6. 性能前提与 DDL 边界

| 设计编号 | 设计决定 | 覆盖需求 | 覆盖验收 |
|---|---|---|---|
| CCFG-DB-018 | 已批准数据库基线记录“`CDC_CLIENT_MULTIPLE` 当前业务模型下记录总数不超过 20 条”为既有事实（`CONFIRMED_HARD_LIMIT`），不等于页面新增业务上限；本 Feature 不设探针数/数据源数业务硬上限（字段物理容量仍校验）。设计前提：小表全量读 + 短事务表锁串行写入，开销可忽略且避免引入复杂分布式锁；无 N+1（一次读客户表 + 一次读数据源表）。不新增索引、外键、唯一约束、触发器、序列、关联表或迁移脚本；`DDL_STATUS=NONE`。 | CCFG-REQ-004、CCFG-REQ-090 | CCFG-AC-003、CCFG-AC-076 |
