...
| 第二轮调整实现状态 | `adjustment2_implementation_status=IMPLEMENTED_PENDING_CHATGPT_REVIEW`（本轮已批准基线已于 2026-09-23 由 `CLIENT-CONFIG-VISUAL-FOLLOWUP-IMPLEMENTATION-001` 完成前端实现并停在 ChatGPT 远程复审入口，见 §1.6；批准任务 `CLIENT-CONFIG-VISUAL-FOLLOWUP-BASELINE-APPROVAL-CLOSEOUT-001` 时点的 `NOT_STARTED` 属历史状态。**实现完成不等于已目测、已验收或已接受**） |
| 第三轮调整任务编号 | `CLIENT-CONFIG-POPOVER-AND-DIALOG-VISUAL-BASELINE-001`（2026-09-23，第三轮 `+N` 清单与新增／编辑弹窗视觉调整**草案**，对应报告见 §2 导航） |
| 第三轮调整基线状态 | `adjustment3_baseline_status=DRAFT_PENDING_USER_REVIEW`（本轮仅建立纯文档草案，待项目负责人与 ChatGPT 从远程 Git 复审后另行批准，见 §1.7） |
| 第三轮调整批准状态 | `adjustment3_approval_status=NOT_APPROVED`（本轮草案**尚未**复审、**尚未**批准） |
| 第三轮调整实现状态 | `adjustment3_implementation_status=NOT_STARTED`（本轮草案**尚未**实现任何代码；`CCFG-REQ-113~122`、`CCFG-AC-105~117`、`CCFG-DESIGN-054~060`、`CCFG-UI-043~049` 均未实现） |
...
独立实现任务 `CLIENT-CONFIG-VISUAL-FOLLOWUP-IMPLEMENTATION-001` 已按已批准基线实现 `/config/client` **主列表**的五项视觉调整（起始提交 `81bb8f172f612a2a6edf91181c59b4388da2c999`，对应报告见 §2 导航）。该实现任务完成时点的分层状态如下：
...
- **状态边界**：`adjustment2_implementation_status=IMPLEMENTED_PENDING_CHATGPT_REVIEW` 只表示**代码实现已完成并停在 ChatGPT 远程复审入口**；**不**表示已通过项目负责人目测、已执行正式验收或已获最终接受。全部 104 条验收（含本轮 `CCFG-AC-090~104` 15 条）仍**全部 `NOT_RUN`**，覆盖保持 112/112，`PENDING_USER_CONFIRMATION=0` 保持。
...
### 1.7 第三轮 `+N` 清单与新增／编辑弹窗视觉调整（2026-09-23 草案建立，待 ChatGPT 远程复审）

第三轮任务 `CLIENT-CONFIG-POPOVER-AND-DIALOG-VISUAL-BASELINE-001`（纯文档）依据项目负责人**已明确确认的四项调整决定**，为 `/config/client` 的 `+N` 完整清单弹层与新增／编辑探针弹窗建立新一轮调整**草案**，供 ChatGPT 从远程 Git 复审并由项目负责人后续另行批准。该草案**不实现任何代码、不声明验收**。当前分层状态如下：
...
- **状态分层与草案边界**：本轮为**纯文档草案**，`adjustment3_baseline_status=DRAFT_PENDING_USER_REVIEW`、`adjustment3_approval_status=NOT_APPROVED`、`adjustment3_implementation_status=NOT_STARTED`、本轮验收 `NOT_RUN`（`CCFG-AC-105~117`，13 条新增，全文件 117 条全部 `NOT_RUN`）。**建立草案 ≠ 已批准 ≠ 已实现 ≠ 已验收**。
- **与已批准第二轮的关系**：第二轮的 `adjustment2_baseline_status=APPROVED` 与实现 `IMPLEMENTED_PENDING_CHATGPT_REVIEW` **保持真实不改写**；本轮为独立的第三轮草案，具有独立的草案状态，**不**改写第二轮历史、**不**自动继承第二轮批准。
- **定向修订（保留原文）**：本轮对既有规则中与本轮决定冲突的条目（`REQUIREMENTS.md` 的 `CCFG-REQ-016`、`ACCEPTANCE.md` 的 `CCFG-AC-011`、`UI.md` 的 `CCFG-UI-009` 与 `CCFG-UI-024`）采取**目标标注 + 修订**：保留原决定与变更轨迹、不改写为“从未存在”，并指明由 `CCFG-REQ-115`/`CCFG-AC-109`/`CCFG-UI-044` 承接；这些标注**在本轮获批前不生效**，实现仍以现行已批准规则为准。
