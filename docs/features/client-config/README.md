...
| `DESIGN.md` | 逻辑设计（`CCFG-DESIGN-001~034`，含并发/锁方案、追踪矩阵） | `DRAFT_PENDING_USER_REVIEW`（2026-09-03 设计草案，未正式复审批准） |
| `API.md` | 接口契约（`CCFG-API-001~014`，接口 E1~E7、错误码表） | `DRAFT_PENDING_USER_REVIEW`（2026-09-03 设计草案，未正式复审批准） |
| `UI.md` | 界面设计（`CCFG-UI-001~024`，布局/交互/文案） | `DRAFT_PENDING_USER_REVIEW`（2026-09-03 设计草案，未正式复审批准） |
| `DATABASE.md` | 数据库使用设计（`CCFG-DB-001~020`，SQL 形态/事务锁矩阵） | `DRAFT_PENDING_USER_REVIEW`（2026-09-03 设计草案，未正式复审批准） |
...
- `DESIGN.md`/`API.md`/`UI.md`/`DATABASE.md` 已于 2026-09-03 建立设计基线草案（`CLIENT-CONFIG-DESIGN-BASELINE-001`），状态均为 `DRAFT_PENDING_USER_REVIEW`、`PENDING_USER_CONFIRMATION=0`，尚未经 ChatGPT 正式设计复审与项目负责人批准；本 Feature 尚未进入实现阶段。四份设计文档是草案，不得写成已批准基线；不因建立设计草案而改变已批准需求/验收，也不修改任何数据库基线。
...
下一入口为设计复审（`CHATGPT_FORMAL_DESIGN_REVIEW`）：由 ChatGPT 对 `DESIGN.md`/`API.md`/`UI.md`/`DATABASE.md` 四份设计草案进行正式设计复审；设计草案须经 ChatGPT 正式复审通过并经项目负责人批准后才能进入实现阶段，不得跳过设计直接实现代码。
