...
- **CDC 节点状态**：单页面，通过"节点类型"筛选切换客户端节点、探针节点及其他节点类型。不拆分多个菜单或独立页面。
- **日志查询**：单菜单、双 Tab（错误日志 / 正确日志）。错误日志对应 CDC_LOG_ERROR，正确日志对应 CDC_LOG_CORRECT。
- **第一版范围**：4 张表 + ZooKeeper 节点状态，全部只读。前端轮询刷新，不使用 WebSocket。不提供导出功能。
- **暂不纳入范围**：
  - 启停 CDC 任务
  - 重置采集进度
  - 修改 ZooKeeper 节点
...
- 支持从运行监控页面通过链接跳转到对应的配置管理页面。跳转仅当运行数据能够唯一定位到一条配置记录时显示入口，无法唯一定位时不显示入口。
  - **状态**：项目负责人已确认。
