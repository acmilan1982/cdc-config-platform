...
| C-24 | 事件卡片按时间倒序排列；已闭环默认展开最新事件，未闭环展开当前处理事件，异常链事件默认展开 | 044 第二条 |
...
一次故障过程中的每条 `CDC_JOB_FAILURE_EVENT` 使用一张独立卡片，按 **FAILURE_TIME 降序**（时间倒序）排列：

```text
事件 3 → 事件 2 → 事件 1（最新 → 最早）
```
...
| 场景 | 默认展开 |
|------|----------|
| 已闭环故障 | 展开**最新**失败事件（倒序后第一个，含 STABLE_CHECK_PASSED） |
| 未闭环故障 | 展开**当前正在处理**的事件（倒序后第一个） |
| 存在异常链 | 异常事件默认展开（即使不是最新事件） |
| 其他事件 | 默认折叠 |
| 单事件故障 | 始终展开 |
...
| 事件卡片展开 | 已闭环：默认展开最新事件；未闭环：默认展开当前事件；异常事件默认展开；卡片按时间倒序排列；手动展开不因 CLOB 懒加载而改变 |
...
| 2026-07-28 | v1.2 | 依据 043 视觉验收结论全面修订：引入两级状态体系（Job 当前状态 2 种 + 故障过程状态 5 种）；客户端标题栏增加正常/异常数量统计；主列表删除 dataSourceOrg/机构/业务库字段；历史故障从分页改为时间范围查询（1d/1w/1m）；增加最大故障过程数量保护；指定故障详情独立页面；物理 Job 演变链、异常链提示、CLOB 三行摘要懒加载；事件卡片时间倒序（044 修订）；扁平边框风格（无 glassmorphism）；全文 DTO、JSON 示例、算法、状态枚举同步修订 |
